...
- 正例要包含正式、口语和简略表达；是否覆盖未点名正例，由 `admission.json` 与 `agents/openai.yaml` 的调用策略决定，评估器通过准入脚本统一读取，不另维护例外名单。候选或禁止隐式调用的 Skill 只接受显式点名正例，并须保留未点名负例；候选定向评估不表示安装或生产准入。
...
- 文件、入口和章节行数只作诊断，不扣分、不阻塞准入，也不强迫拆分。静态引用质量继续检查渐进读取结构；可检索性分数只衡量渐进读取头与任务索引覆盖，将原有 22:28 导航信号归一到 100，不能冒充真实检索效果或新旧行为改善。拆分依据是独立任务、重复权威或索引不能隔离所需上下文，而非机械篇幅。
...
运行配置只进入非盲 key 与报告，不给 Judge 暴露条件；仅有这些自报 metadata 不能证明运行时确实采用了该配置，采集轨迹仍需独立核对。旧案例可不提供该字段，但不能据此声称推理、工具、权限与环境已对齐。指令行为对照应检查不必要确认、未读来源误报完成、授权扩张和实际验证范围；发现阻塞或偏离时按 `AGENTS.md` 回指具体条款，不按篇幅或 `MUST` 数量判断质量。
...
- 允许隐式触发的 Skill 至少有一个正例不直接点名 Skill；只允许显式调用的 Skill 则要求所有正例点名，并至少保留一个未点名负例。
