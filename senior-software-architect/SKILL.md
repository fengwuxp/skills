...
2. **【强制】禁止吞异常和丢上下文** – 不得空 catch、只打印日志后继续、事务内捕获异常不回滚；包装异常必须保留 cause 和关键业务上下文。
3. **【强制】禁止裸日志和敏感信息泄露** – 不用 `System.out.println`、`e.printStackTrace()`；日志不得输出密码、token、密钥、身份证、银行卡、手机号等敏感信息原文。
4. **【强制】金额、时间、ID 不得随意处理** – 金额不用浮点和 `new BigDecimal(double)`；时间必须明确时区和精度；ID 生成必须考虑唯一性、可追踪和并发。
5. **【强制】公共契约不可随意破坏** – 对外 API、DTO、枚举、错误码、配置项、消息体、数据库字段的破坏性变更必须给兼容方案、迁移路径和废弃周期。
6. **【强制】不得返回模糊契约** – 公共 API 的空值、异常、权限、幂等和分页语义必须明确；内部 Java 空值契约遵循 JSpecify，列表默认返回空集合，不以 `null` 表示空列表。
7. **【强制】不得把业务规则塞进错误层次** – Controller 不写领域规则，Mapper/Repository 不写业务决策，通用工具类不沉淀业务规则。
8. **【强制】不得为了复用而复用** – 不为 DTO 构建、一行代码或偶发重复提取公有方法/工具类；不得为了“少写”省略必要边界、错误处理、安全/权限/资金兜底或测试；公有方法参数超过 5 个必须先和用户确认。
9. **【强制】不得引入无主依赖** – 新增依赖、starter、中间件、代码生成器或运行时代理前必须说明必要性、替代方案、版本风险和维护责任。
10. **【强制】Java/Spring/Wind 模型与持久化约规不可绕过** – 对外 API 和跨模块契约使用 DTO、Request、Query，不暴露 Entity；模型转换优先使用 MapStruct；查询禁止 `LambdaQueryWrapper`，使用 MyBatis Flex `XxxRefs`；写库默认使用 selective 方法；不得擅自修改或删除有明确用途的注释代码。
11. **【强制】业务代码不得用内存版 Service 冒充生产实现** – 除缓存能力、测试替身/fixture、沙盒模拟或明确 demo 外，生产源码路径不得出现 `InMemoryXxxService`、`FakeXxxService`、`MockXxxService`、Map/List 存储型业务实现或只在进程内保留状态的应用服务来承载真实业务能力。
12. **【强制】不得用注释替代可读性重构** – AI 生成或人工代码应优先通过命名、方法抽取、常量/枚举、强类型和值对象、测试表达意图；注释只补充代码无法自表达的业务约束、设计取舍、外部规则、历史坑点或 Why not。
13. **【强制】接口和服务拆分必须有真实业务职责** – 接口、Service、ApplicationService、Facade、Adapter 的拆分必须对应明确用例编排、业务规则、事务边界、权限/审计、状态转换、跨资源协调或对外稳定契约；不得为了分层好看强行设计只透传调用、只改名转发、一行包装或似是而非的抽象。
14. **【强制】模型对象不处理运行时依赖** – DTO、Request、Query、Command、Context 等模型只表达契约字段、输入范围和确定性自有派生；不得依赖 Service、Supplier、Cache、当前用户/租户解析器、远程端口或数据库查询。运行时派生变量由 Service、ApplicationService 或 Adapter 在用例边界组装，有值才写入上下文，不写无业务语义的 `null` 占位。
15. **【强制】编码前先做约规归位检查** – 在 opt-in Wind/Nobe 项目修改 Service、模型或 DAL 前，先确认项目 `AGENTS.md` 和 Wind 约规：face 放稳定契约，impl 放实现和 DAL；组合、生命周期、应用层或外部适配服务优先依赖稳定服务契约。若调用方需要跨实体读写，先复用或补强对应基础服务，不把 Entity、Mapper、QueryWrapper 泄到对外接口或编排服务。
16. **【强制】不得保留无业务语义的单行透传方法** – 公有或私有方法若只是原样转调、原样传参、改名包装或返回下游结果，且没有新增业务命名、边界校验、权限/审计、事务语义、异常转换、观测埋点、稳定契约或可测试隔离，应直接删除并内联调用；不得用这类方法制造调用跳转、伪抽象或 AI 生成噪声。
17. **【强制】业务逻辑与控制逻辑不得混杂** – 不让一个方法同时负责取数、遍历、分支、改状态、异步调度和业务决策；Controller、Listener、Webhook、回调函数只做协议解析、校验、转换和转交，不承载核心业务规则；业务状态必须显式建模、可测试、可追溯，不用 flag、隐式顺序或跨线程副作用表达。
