...
7. **验证优先**：原则必须落到模块结构、接口契约、测试、静态检查、监控指标或评审清单；不得以口号替代工程验证。
...
8. **【强制】正式设计不混过程稿**：系分、技术方案、ADR 和架构交付物只保留最终设计结论、有效取舍、风险、待确认和验证；讨论过程、迭代草稿、AI 推理轨迹、被拒方案和评审流水进入评审报告、任务计划、Decision Log 或中间任务文档。
