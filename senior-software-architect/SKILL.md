...
本文中的项目规则均指消费项目；不要求继承 Skills 源仓库 `AGENTS.md`。Java 任务需要运行时可达的 `wind-coding-conventions`；缺失时只暂停依赖该规则的代码写入并说明缺口，不猜测正文或自动安装。
...
6. **Web API 先按 RESTful 语义审查**：资源 URI、HTTP 方法、状态码、查询参数、幂等 / 并发和兼容例外必须与真实契约相符；动作型命令要说明例外理由。既有固定协议优先保兼容，不为形式重命名；详细规则读取 `references/project-governance-service-api-modeling.md` 的“HTTP API / RESTful 契约裁决”。
7. **围绕变化拆分、通过组合协作**：责任、数据、事务和变化原因一致时优先内聚，边界不同则通过契约组合；继承须满足行为可替代性，不按单实现、短方法或接口数量裁决。
8. **守住不变量、让变化局部发生**：结合业务阶段、团队与运行约束，只为有证据的变化轴设置边界；以修改影响、兼容成本、恢复和退出能力判断演进质量。
9. **以真实结果验证、达到目标即收口**：选择能验证使用者结果、边界与失败责任的证据，区分事实、推断和待验证；满足验收后停止扩展，保留理由链和必要的复核条件。

设计、接口与方法首次写回前、代码 CR 和架构审查共用 `references/project-governance-service-api-modeling.md` 的“能力价值与架构裁决”，无需用户再次点名；详细契约与实现边界也归该 reference。简单任务用简短判断直接推进，不逐条展示九项原则或新增审批。
...
4. **【强制】按入口核对任务契约**：收到 Product Context Card、Engineering Handoff Card 或生产交付卡时核对其内容；用户直接提出工程任务时，从用户要求和项目事实核对目标、约束、写入范围、验收和待确认，不以缺少交接卡为由展开额外协调流程或阻塞已授权工作。不重开已确认的产品流程，不把交接卡当成 Execution Grant、测试通过、Git 授权或上线审批。
...
2. **【强制】核心业务隔离外部细节** – 独立领域模型不依赖 Controller、Mapper、ORM Entity、第三方 SDK、MQ、缓存或 HTTP Client；用例通过稳定业务契约组合能力。按消费项目实际分层映射责任，不能仅凭 `Service` 类名认定它是纯领域层。Wind 的承责基础 `ServiceImpl` 可以封装自身 Mapper、Entity 和事务，具体边界读取 Wind 约规“2. 服务层”；不为套用 DDD 另造透传 DomainService、Repository 或包装接口。
3. **【强制】禁止循环依赖和反向依赖** – 独立 core/domain 不依赖 web/infrastructure/bootstrap；web 不直接访问 Repository/Mapper；外部系统通过承责模块的稳定契约隔离，内部实现按项目架构选择适配方式。
