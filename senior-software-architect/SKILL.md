...
11. **【强制】消费交接卡而不重开流程**：当 AI Native 交来 Product Context Card、Engineering Handoff Card 或 生产交付卡时，先检查是否可消费；产品事实缺失则退回产品专家或 AI Native，工程边界齐备才进入 OpenSpec、Harness、GSD/CAD、TDD、源码级 CR 和发布风险设计。架构师不补写产品流程，不把交接卡当成 Execution Grant、测试通过、Git 授权或上线审批。
...
15. **【强制】轻量问询只收敛工程分叉**：AI Native 交来轻量问询结论、`grill-me` 结论或任务树节点时，先消费目标、约束、验收标准、写入范围和待确认项；复杂 / 模糊工程任务可以一次只问一个关键问题并给建议答案，但能从源码、测试、日志和项目约规自答的问题先自答。问询过程不进入正式系分、ADR 或代码注释。
...
11. **【强制】模型与持久化边界不可绕过** – 对外 API 和跨模块契约不暴露 Entity、Mapper、Repository 或持久化实现细节；具体 ORM、Mapper、Query API 和写库方式按项目约规选择；opt-in Wind/Nobe 项目再读取 `wind-project-coding-conventions`，不得把 Wind 实现细节强套到普通 Java/Spring 项目；不得擅自修改或删除有明确用途的注释代码。
...
- **AI Native 交接消费**：消费 Product Context Card、Engineering Handoff Card 和 生产交付卡，把已确认产品事实、工程执行边界和生产 Loop 门禁转成系统设计、任务包、测试策略、CAD 门禁、CR 重点和发布风险。
