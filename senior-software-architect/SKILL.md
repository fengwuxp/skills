...
7. **【推荐】正文与表格默认纯文本**：正文段落和 Markdown 表格内容一般不加粗、不使用代码背景；字段名、状态值、枚举值、示例值、角色、模块名和风险项默认纯文本。只有代码标识符、命令、配置键、协议字段、文件路径、风险等级或需要强调的结论，才使用加粗或代码背景；标题层级不受此规则影响，继续使用 Markdown 标题表达结构。
