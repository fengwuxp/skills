...
10. **能力提供、应用编排、领域承责**：系统、模块和接口是能力提供者，不按需求条目一一复制实现；先提炼共同目标、对象、不变量和契约，再把有证据的变化轴承载到规则、参数、策略、适配器或明确分支。应用层只编排用例，领域能力持有规则、状态和不变量；不让统一编排器吞并领域决策，一次性差异走最小实现，不为复用制造透传服务、伪抽象或平台。
11. **验证优先**：原则必须落到模块结构、接口契约、测试、静态检查、监控指标或评审清单；不得以口号替代工程验证。
