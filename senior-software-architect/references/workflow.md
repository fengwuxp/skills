...
| 复杂需求、关键设计未定或实现偏离设计 | `设计就绪与代码写入` | 不以代码骨架代替定性、分析和设计 |
...
`Clarify -> Design -> Plan -> Build -> Verify -> Review/Ship` 用于检查工程责任是否覆盖，不要求每项都成为独立文档、顺序阶段或用户审批点。目标、范围、关键设计、实现授权和验证清楚时直接实现；多步骤可用简短计划组织依赖，无需等待完整计划批准。高风险任务仍须保留确认点、测试设计、回滚或监控边界。
...
- **形成可审查设计**：按 `project-governance-service-api-modeling.md` 的“能力价值与架构裁决”和“契约与实现对齐”说明当前改动的结果责任、稳定承诺、内部封装、兼容影响、拟修改范围及可观察验收；只覆盖会影响当前切片的项目，已有决定引用原记录。文件清单、操作步骤、类名列表或“已想清楚”不能代替设计结论，不要求另建平行文档或暴露内部推理过程。
- **先说明依据，再编码**：复杂任务首次写代码前，向用户说明采用的设计依据、当前切片和验收；已确认设计及实现授权足够时，说明后继续，不重复审批。授权内可自行决定的可逆实现细节按项目事实选择；会改变业务取舍、公共契约或风险边界且未获裁决授权的事项先返回有依据的选项，只暂停依赖它的实现。
...
- 在当前任务说明或已有载体中简要保留已读取的文件/章节、本轮适用约束和验证命令；这不是新审批或新卡片。规则未读、来源冲突或必要依据缺失时，先补齐该项，再写依赖它的代码；已确认依据且范围未变时不重复全量读取。
