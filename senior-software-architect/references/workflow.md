...
- 【强制】CAD Mode 启动前必须完成 Git 自动提交权限预检：确认工具层、沙箱、Git hook、签名和仓库状态是否允许非交互式 `git add` / `git commit`；如需要额外权限，应在 Round 0 提前请求，避免每轮提交时被中断。
- 【强制】自动提交必须使用非交互式命令传入提交信息，例如 `git commit -m` 或等价方式；不得启动交互式编辑器等待人工输入。
- 【强制】若自动提交因权限、hook、签名、沙箱或仓库状态失败，允许按工具规则重试一次；仍失败时必须暂停 CAD Mode，改为输出已验证文件清单、建议提交信息和失败原因，不得声称已经提交。
