...
| 核对类似系统调研与适配依据 | `微信公众号文章：AI时代，究竟怎么做架构设计？` | 不把历史案例、技术栈或作者性能数字当成本项目事实 |
...
| 核对代码坏味与架构体检来源 | `已参考的公开来源：架构健康扫描与复杂度热点`、`应用记录` | 不把启发式阈值和工具输出写成 CR 结论 |
...
## 微信公众号文章：AI时代，究竟怎么做架构设计？

- 来源：`https://mp.weixin.qq.com/s/pHZCyEVIQeNnBeNnr_QPdg`；标题：`AI时代，究竟怎么做架构设计？（2/100）`；作者：`58沈剑`；公众号：`架构师之路`；发布时间：2026-09-15 08:16 Asia/Shanghai；读取日期：2026-09-15。
- 读取状态：常规网页抓取失败后，通过浏览器读取完整正文和补充阅读链接。文章是架构实践观点，正文未引用研究论文；不归因于 Google 或 WikiSkill。
- 作者补充案例：`https://mp.weixin.qq.com/s?__biz=MjM5ODYxMDA5OQ==&mid=2651971182&idx=1&sn=d09db09078469cfebcb0cff4ee55e73a&scene=21#wechat_redirect`；标题：`1万属性，100亿数据，每秒10万吞吐，架构如何设计？`；作者：`KG沈剑`；发表于 2022-09-01 22:46 Asia/Shanghai；2026-09-15 通过浏览器读完正文。只参考存储、属性和检索职责的演进依据；性能数字未独立复现。
- 独立核验：SEI ADD 3.0 官方介绍 `https://www.sei.cmu.edu/library/add-30-rethinking-drivers-and-decisions-in-the-design-process/`，页面日期 2015-04-28；2026-09-15 已读介绍正文，支持参考架构、框架与迭代设计的既有方法依据。演示 PDF 两次获取超时，不声明已读全文。
- 独立核验：SEI ATAM 沿用本文“已参考的公开来源：架构验证方法组”的既有链接；2026-09-15 已读方法概要，支持按业务目标与质量场景分析取舍；未逐篇读取集合内报告，不引入完整 ATAM 组织流程。
- 适配反例：MongoDB 官方案例 `https://www.mongodb.com/resources/solutions/use-cases/mongodb-case-study-craigslist`；2026-09-15 已读正文，叙述 2011 年历史归档迁移，不能证明 Craigslist 当前在线主链路选型；厂商自述不等于独立性能审计。
- 许可证与时效：微信文章未见明确复用许可；上述来源均只保留链接、日期、方法转述和适用边界，不复制正文、图片、代码或宣传表达。涉及当前产品能力或工程选型时重新核验版本与本地条件。

应用记录：`architecture.md` 第 2.1 节维护类似系统调研与适配；`coding-review-deep-dive.md` 按条件路由到该节。新增架构调研行为题集覆盖场景错配、未知来源、重复抽象、简单任务、CR 与验证停止边界；题集结构通过不代表行为收益。
...
## 已参考的公开来源：架构健康扫描与复杂度热点
- 来源/读取：`https://mp.weixin.qq.com/s/q7aKFiBeldJcjEhZ7v2Smg`；标题：`寻找你代码中的臭味：一个让 AI 帮你嗅出架构腐化的开源 Skill`；作者：`鸟窝`；发布时间：2026-05-28 14:06；读取日期：2026-07-18。`web.open` 未取得正文，随后通过 Chrome UA `curl` 公开 HTML 读取标题、作者、发布时间和正文。
- 来源/读取：`https://mp.weixin.qq.com/s/zCrGpuFddVZU1fum6uFYsQ`；标题：`代码在发臭：一个能"闻"出坏味道的 AI 技能，我拿它扫了最新的开源代码`；作者字段：`鸟窝`；账号字段：`鸟窝聊技术`；发布时间：2026-07-03 15:18；读取日期：2026-08-02。`web.open` 无法打开，随后通过 Codex 应用内浏览器 Playwright 读取标题、作者、账号、发布时间和正文。本文继续指向同一开源实现，只补充真实项目演示和重构后复扫思路。
- 开源实现：`https://github.com/smallnest/goal-workflow`，读取 `master` 分支的 `skills/smell/SKILL.md`；仓库标注 MIT；读取日期：2026-07-18。已核对扫描范围、启发式分类、报告结构、并行探索和快速模式；未安装、未同步、未复制其 Skill 或脚本。
应用记录：
- `scenario-routing.md` 增加最近改动 / diff、指定模块、全仓和仅架构层的范围契约，并把快速体检作为默认强度。
- `coding-review-deep-dive.md` 增加候选信号与确认问题分层、确定性证据优先、跨语言复杂度热点及输出边界。
- `coding-review-deep-dive.md` 增加同范围、同规则复扫的可比性门禁；复扫只作辅助证据，不替代测试、运行证据和独立 Review。
未吸收内容：
- 不吸收固定行数、方法数、参数数阈值，不把启发式命中或工具输出直接写成缺陷、级别或重构处方。
- 不默认启动多 Agent，不照搬外部命令、报告目录、健康分数、数量排行榜、长期路线图或自动重构流程。
- 不复制文章对单个开源项目的统计、扫描结论、图片或路线图，也不把案例展示当成扫描精度和重构收益证明。
- 不把外部文章、开源 Skill 或扫描器的结论写成当前项目事实、CR 准出、测试通过、执行授权或生产审批。
