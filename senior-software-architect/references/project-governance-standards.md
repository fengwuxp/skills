...
- 具体 Java 编码规约优先读项目本地规范和 `wind-coding-conventions` 通用层；Wind 专项按依赖或上下文启用。
...
- Java 项目先读 `wind-coding-conventions` 通用层；项目声明、依赖、包名、类型或模块上下文命中 Wind 时再读其专项规则。
...
| Wind 专项信号 | `wind-coding-conventions` Skill | 没有声明、依赖或源码上下文证据时不加载 Wind 专项 |
...
- `wind-coding-conventions` Skill：所有 Java 项目的通用约规入口；命中 Wind 后补充 face/impl 边界、服务分层、模型归属、分包、DAL、外部集成和 CR 清单。
