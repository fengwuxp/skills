...
- 任务需要跨会话恢复或跨责任人交接，或当前上下文已无法可靠保持目标、决策、阻塞项与验证证据。
- 用户已授权多 Agent / Wave 分工，或项目明确要求可恢复的阶段编排。
...
- 目标、范围、授权和验证清楚，当前会话可完成的任务；不因文件数、模块数或预计时长而启用。
...
- 只补当前恢复、交接或编排实际需要的状态，复用项目原生载体；读取本文件不要求交付全部阶段或表单。
- 恢复或交接保留目标、授权、写入范围、决策、证据、未决项和恢复入口；只读第 12 节即可满足时，不追加 Wave / Loop。
- 实际启用 Wave 才补任务依赖与写入隔离，实际启用 Loop 才补反馈、验证者、预算和停止条件；高风险确认点按原授权保留。

## 需要继续读取的 reference

- AI 协作总纲读 `ai-assisted-engineering.md`。
- 端到端 AI Native 产品到研发流程、角色编排和 GSD / 工程执行准入判断按 `SKILL.md` 工作原则核对目标、约束、授权和验收；有既有准入结论时核对其有效事实，否则使用已确认的用户与项目事实，并在已经进入工程侧大项目编排后细化任务包、上下文账本、Wave 和验证矩阵。产品侧上下文缺口回到 `product-architecture-expert/references/ai-native-product-context.md`。
...
| GSD + 长任务 + Loop 进入工程 | 先核对已有项目执行规范、授权和任务契约；有既有编排结论时消费其有效事实，再读 6、7、10 | 不创建运行时 Goal，不把 Loop 当整个 Roadmap 授权 |
...
AI Native 产品到工程的端到端链路仍由当前 Agent 负责；已有 GSD / 工程执行编排准入结论时核对其有效事实。直接工程任务从已确认的用户与项目事实核对同等输入，不因缺少协调 Skill 或交接卡而阻塞。进入本文件时，只保留工程侧最小链路：
...
先匹配真实恢复、交接或编排需要，再读取对应部分；以下信号不是开启整套流程的充分条件：

| 信号 | 启用理由 |
| --- | --- |
| 任务跨多个模块、服务、接口、状态机或测试层级 | 先判断依赖与验证是否已清楚；当前会话可完成时直接执行。 |
...
| 修改涉及公共契约、资金、权限、生产行为、数据迁移或安全 | 保留适用的人工确认、验证与回滚边界，不自动启用阶段编排。 |
| 需要 thread automation 或 side panel artifact 持续迭代 | 需要明确 verifier、停止条件、状态位置和人工确认点。 |

不满足启用门槛时，按用户与项目约定直接工作；不要求轻量 OpenSpec，也不为了“流程完整”创建额外文档。
...
端到端 GSD / 工程执行准入、GSD Round 0 缺口、Wave/Atomic Task 候选和 Plan Grant / Execution Grant 缺口按 `SKILL.md` 工作原则从已确认事实核对；已有准入包时核对其有效事实。本节说明工程任务契约齐备后，如何把工程侧大项目编排和受控工程执行 Loop 组合起来。
...
暂停前更新项目已有状态载体，`03-state.md` 只是已采用整套编排时的示例；以下按实际需要填写，不创建空字段或并行状态源：
...
1. 项目本地 `AGENTS.md`。
2. 项目已有规格或已确认的用户任务契约。
3. 项目原生状态载体中的授权、决策、阻塞与证据。
4. 实际启用 Wave 时读取当前任务包和验证矩阵。
