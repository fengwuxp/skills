...
| GSD-CAD 组合判断 | 先消费 `delivery-collab/references/planning-execution-admission.md` 的准入结论，再读 8 和 `cad-mode.md` | 不对整个大项目直接开 CAD |
| GSD + Goal + Loop 进入工程 | 先消费 `delivery-collab/references/delivery-execution-control.md` 和 `planning-execution-admission.md`，再读 6、7、10 | 不把 Loop 当整个 Roadmap 授权 |
| AI Native 编排结论进入工程 | 8A，并消费 `delivery-collab` 或产品侧交接材料 | 不把业务 MVP 直接当 CAD 授权 |
...
AI Native 产品到工程的端到端链路由 `delivery-collab` 维护，GSD/CAD 编排准入由 `delivery-collab/references/planning-execution-admission.md` 产出。进入本文件时，只保留工程侧最小链路：
...
产品上下文包回答“这个产品候选是否值得工程化、工程化必须保留哪些业务事实”。`delivery-collab` 回答“端到端流程如何流转、是否进入 GSD/CAD 编排候选、谁负责、何时停止”。GSD-like 编排回答“哪些阶段和任务可以被执行”。CAD Mode 只回答“当前选中的原子任务是否可以自动执行”。四者不得互相替代。
...
端到端 GSD/CAD 准入、GSD Round 0 缺口、Wave/Atomic Task 候选和 Plan Grant / Execution Grant 缺口先由 `delivery-collab/references/planning-execution-admission.md` 输出。本节只说明资深架构师拿到准入包后，如何把工程侧大项目编排和 CAD Mode 组合起来。
...
当输入来自业务方或产品侧 AI 生成原型、MVP、Product Builder 方案、业务 dogfooding 结果或“放下 PRD”的流程改造时，先确认是否已有 `delivery-collab` 或 `产品架构专家` 给出的产品侧交接结论。没有 Hardened Candidate 或产品侧交接条件时，不进入 GSD/CAD，只回到产品上下文补齐。
...
- 架构师绕过 `delivery-collab` 和产品侧交接结论，直接按原型补接口和表结构。
