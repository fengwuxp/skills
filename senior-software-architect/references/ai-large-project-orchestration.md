...
产品上下文包回答“这个产品候选是否值得工程化、工程化必须保留哪些业务事实”。知止者持续持有端到端目标与状态，并按需装载产品、工程和验证能力。GSD-like 计划层回答“哪些阶段和任务可以被执行”，受控工程执行 profile 只回答“当前选中的原子任务如何在 Loop 内连续执行”。状态载体、能力和执行机制不得互相替代。
...
脚本只做结构完整性检查；通过不代表可以进入受控工程执行 Loop，也不代表用户已授权 Git、联网、部署或生产操作。
...
| 受控工程执行 Loop | 在单个任务包或阶段切片内按 Pick -> Build/Test -> Review -> Verify -> Record -> Continue/Pause 推进。 | 不消费整个 Roadmap，不跨越写入范围，不处理未选定任务包。 |
| Plan Grant / Execution Grant | 明确当前任务允许的写入范围、验证命令、禁止事项、人工确认点和撤销方式。 | 不扩大为跨阶段、跨 Wave、跨任务链的永久授权。 |
...
- 已检查工作区状态，能区分用户已有改动和本轮改动。
- 已准备适用授权：Plan Grant 覆盖当前任务，或单任务 Execution Grant；不要求两者并存。涉及 Git 写操作、外部访问、依赖安装、Docker/服务启动、数据库迁移或生产行为时必须显式列出。
- 已准备提交切片；默认 `summary_only`，若要求阶段提交或自动提交，必须在适用授权中写明 `commit_after_verified_task`、提交范围、验证条件和失败回退。

工程执行输出必须回写阶段状态、验证矩阵和 handoff：完成内容、验证结果、失败/跳过原因、残余风险、Git 处理结果、下一任务建议。只有原子任务包满足工程执行 Loop 门禁时才进入；门禁不满足时，回到本文件的 Round 0、阶段拆分或任务包补齐。
...
- AI Native 编排结论：当前成熟度、下一步 owner、GSD / 工程执行编排准入结论、是否进入 OpenSpec、是否需要 Harness/GSD、GSD Round 0 缺口、Wave/Atomic Task 候选、工程执行 Loop 缺口和 Plan Grant / Execution Grant 缺口。
...
进入受控工程执行 Loop 前必须再次确认：已选定单个 Task ID 或阶段切片，关键决策冻结，写入范围、验证命令、状态载体、反馈源、人工确认点和停止条件明确。产品上下文包、Hardened Candidate、AI Native 编排结论或 GSD Roadmap 都不是 Plan Grant / Execution Grant；只有 AI Native 输出 `Plan Grant: Active` 时，才默认推进任务计划内低风险本地动作。
...
- 若任务包标记为工程执行 Loop 候选，是否已继续读取 `cad-mode.md` 并确认 Plan Grant / Execution Grant 覆盖当前任务。
...
- 只有用户明确要求并在 Plan Grant / Wave Grant / Execution Grant 中写明 `commit_after_verified_task` 或等价策略，才在任务验证通过后执行本地 `git add` / `git commit`；提交前必须检查 `git status` 和 `git diff`，确认只包含授权范围内且已验证的变更，混入用户已有改动、验证失败、跳过验证或范围不清时不得提交。
