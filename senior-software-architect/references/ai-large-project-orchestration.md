...
| GSD + Goal + Loop 进入工程 | 先消费 `ai-native-engineering-workflow/references/agent-loop-engineering.md` 和 `gsd-cad-admission.md`，再读 6、7、10 | 不把 Loop 当整个 Roadmap 授权 |
...
端到端 GSD/CAD 准入、GSD Round 0 缺口、Wave/Atomic Task 候选和 Plan Grant / Execution Grant 缺口先由 `ai-native-engineering-workflow/references/gsd-cad-admission.md` 输出。本节只说明资深架构师拿到准入包后，如何把工程侧大项目编排和 CAD Mode 组合起来。
