...
- Java 代码实现细则读项目本地规范和 `wind-coding-conventions` 通用层；Wind 专项按依赖或上下文启用。
...
- Java 编码约规读项目本地规范和 `wind-coding-conventions` 通用层；Wind 专项按依赖或上下文启用。
...
| 任务 | 优先读取 | 跳过 |
| --- | --- | --- |
| Java/Spring 测试底座、最小上下文、校验边界前移、Maven / Surefire 与 offline 证据 | `testing-practices-java-spring-common.md` | 具体 Controller / Service 模板 |
...
- `testing-practices-java-spring-common.md`：Java/Spring 测试底座、最小上下文、外部依赖替身、公共测试基础设施、校验边界前移、Maven / Surefire 真实执行与 offline 子进程证据。
