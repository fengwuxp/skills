...
- AI Native 已交付 Product Context Card、Engineering Handoff Card 或 Production Loop Card，需要架构师判断是否可消费并继续工程设计、编码、TDD、CR 或发布风险。
...
- 写入范围、禁止事项、验证命令和停止条件。
- AI Native 交接卡消费结论：Product Context Card 是否足够，Engineering Handoff Card 是否可执行，Production Loop Card 是否满足持续推进门禁。
...
| AI Native 三卡交接后进入工程 | 1D 交接卡消费协议、3 OpenSpec、5 Harness、8 风险分级；命中 CAD 时继续读 `cad-mode.md` | 不让架构师猜测产品事实或把交接卡当授权 |
...
反模式：把 AI 只当更快的代码助手，长期修补生成代码；逐行复写却没有定义系统边界、测试和质量标准；只追求 PR 数量、代码量或自动化轮数，不维护上下文账本、验证矩阵和风险门禁；让实现 Agent、Review Agent 和测试 Agent 共用同一模糊上下文，导致互相迎合或漏掉真实风险。

## 1B. AI 进入旧系统的架构门禁
当 AI 产品、Agent 入口、企业协作 AI 或“主动服务”能力要接入已有 SaaS、协作平台、交易系统、后台系统或多端客户端时，架构师必须先把 AI 当作会穿透旧系统的生产能力，而不是可插拔的智能入口。外部文章、离职复盘或公开转述只能作为反模式参考；具体设计必须回到当前系统、源码、权限和验证证据。架构门禁包括 Context 架构（来源、权限、裁剪、过期、脱敏、追踪和回放）、权限与权力边界（AI 能看/做/确认/申诉/关闭/担责）、旧系统技术债（消息、权限、多组织、客户定制、端侧差异、历史兼容和数据质量）、多端一致性、任务闭环、成本与稳定性、可观测与审计、演进切片。AI 旧系统接入 OpenSpec 补充至少写清真实业务任务、Context 来源 / 裁剪 / 权限、AI 可执行动作、人工确认 / 兜底、多端一致性、成本 / 延迟 / 稳定性指标、审计与可解释、灰度 / 回滚 / 退场。反模式：把 AI 能汇总/提醒/生成等同于系统已经能完成任务，把产品原型或发布会演示路径当成旧系统生产接入复杂度已经解决，只讨论模型能力而不讨论权限、审计、端侧同步、失败兜底和人工责任，或用全量上线倒排架构改造且缺少可回滚的窄场景切片。

## 1C. 可开发系统工程化门禁
当产品侧输入来自模糊需求、内容平台、多端原型或高保真 HTML 时，架构师先检查它是否已形成可开发系统秩序：目标/非目标、前后台和多端边界、业务流、对象状态、规则权限、数据安全、接口候选、测试验收和发布路径。若只看到页面愿望、原型截图或功能列表，应退回产品专家补产品上下文；若已具备秩序，再把它转成 OpenSpec、领域模型、接口契约、数据模型、测试矩阵和 Harness 任务。反模式：从“做一个平台/系统”的一句话直接生成接口、数据库和页面，或把高保真原型当作系统边界已经清楚。

## 1D. AI Native 交接卡消费协议
架构师被 AI Native 调用时，不重开产品流程，也不把流程交接当执行授权；先检查三张卡是否可消费。

AI Native 交接卡消费结论只需说明：Product Context Card 是否足够，Engineering Handoff Card 是否可执行，Production Loop Card 是否满足持续推进门禁，以及下一步是退回产品专家、退回 AI Native、进入 OpenSpec / Harness / GSD 任务包 / CAD 候选，还是只做只读侦察。

消费规则：Product Context Card 只提供产品事实和验收种子，缺目标、对象、不变量、流程规则或验收 owner 时退回产品专家或 AI Native；Engineering Handoff Card 缺 Goal、Spec/AC、Wave/Task、写入范围、验证命令、停止条件、失败回写或授权策略时不进入 CAD；Production Loop Card 缺状态载体、隔离执行、Maker / Checker、反馈源、独立验证、预算、无进展检测、观测审计、人工接管或发布/回滚时不进入自动循环。三卡都不是 Plan Grant / Execution Grant、测试通过、CR 结论、生产审批或 Git 授权；可消费后再映射为 OpenSpec、领域模型、接口/数据契约、测试矩阵、Harness Plan、GSD Atomic Task、CAD 候选、源码级 CR 重点、发布/回滚和残余风险。
