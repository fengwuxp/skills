...
| 任务 | 优先读取 | 跳过 |
| --- | --- | --- |
| 判断是否需要 AI 协作流程 | 使用时机、读取后必须产出、1 核心定位、2 适用场景、8 风险分级 | 工程执行 Loop 细节 |
...
| 代码库教程 / 架构培训 / 技术早报 | Codex 运行时协作模式、工程知识生产 | 不把内容产量当工程价值 |
...
- 浏览器原型局部修改消费 UI 的 `Prototype Change Slice`，把 target_slice、reads、writes、preserved_invariants 和 regression_scope 映射到真实文件、组件与测试；检查全局样式、共享路由、状态和数据结构没有越界污染，写入范围重叠时改为串行，合并后执行目标与非目标回归。
...
工程任务收口后，如果 bug 根因、架构坑、工具决策、测试失败模式或 CR 高频问题会复用，输出最小经验卡：症状、根因、解决方案、预防策略、适用条件、验证证据。优先沉淀到项目 `AGENTS.md`、ADR、测试、fixture、脚本或既有知识库；不把聊天记录、一次性 workaround 或未验证猜测当工程记忆。
...
外部 AI Skill 方法论只作为工程纪律参考，不作为本技能的新能力菜单。实际代码写入默认消费 `llm-coding-hygiene` 作为静默护栏，显式专项审查也由它处理；本技能仍持有工程实现、TDD、诊断闭环、源码 CR 和生产风险，不复制该 Skill 的规则或来源。
...
| 是否参数过多 | 参数数量只作职责与契约可读性的 Review 信号；需要新业务类型或改变公共契约时才确认 |
...
- **规划层经验**：只有当同类任务反复出现稳定阶段、依赖顺序、分支条件或停止条件时，才沉淀到 `scenario-routing.md`、`workflow.md`、工程执行 / Harness 门禁或触发 fixture。
...
senior-software-architect/scripts/check_harness_plan.py --kind engineering-loop --file 04-harness-plan.md
```
...
受控工程执行 Loop 的详细工程规则只读 `cad-mode.md`，本文只保留入口边界。`CAD` 仅作为该文件的内部路由标识；不得把“用户说继续”解释为扩大范围、跳过 Plan Grant / Execution Grant、验证或高风险人工确认点。

进入前必须确认单个任务已选定、关键业务与架构决策冻结、写入和验证边界清楚、预计需要至少两轮、Loop 状态与反馈源可回写，并已有覆盖当前任务的 Plan Grant / Execution Grant。任一门禁缺失时，只列缺口并回到相应权威位置；若已有 `Plan Grant: Active`，范围内低风险本地任务不再逐任务索要 Execution Grant。

工程 profile 的准入、轮内动作、工程停止条件和回写以 `cad-mode.md` 为唯一详细规则源；Loop 状态读 `wise-agent/references/delivery-execution-control.md`，授权读 `wise-agent/references/planning-execution-admission.md`，Git 与平台权限始终服从项目和工具层规则。

## 10. Plan Grant / Execution Grant：执行授权包

Plan Grant / Execution Grant 是受控工程执行 Loop 消费的权限边界，只能约束当前任务链，任务完成、用户暂停或授权被撤销后失效；不得被理解为永久授权或跨任务授权。

最小授权结构、默认低风险操作、必须显式列出的操作和禁止静默执行事项由 `wise-agent/references/planning-execution-admission.md` 维护。AI 协作总纲和 `cad-mode.md` 不复制这些细节。
...
- 若启用受控工程执行 Loop，Plan Grant / Execution Grant 是否覆盖当前任务且本轮操作未越权。
- 代码是否符合现有架构边界和项目风格。
