...
- Java 编码读项目本地规范和 `wind-coding-conventions` 通用层，Review 读 `coding-review-deep-dive.md`；Wind 专项按依赖或上下文启用。
...
- 中大型 AI 编码、长任务上下文衰减或多 Agent/Wave 编排读 `ai-large-project-orchestration.md`。
...
| Java/Spring/Wind 设计或 Review | 项目本地规范、`wind-coding-conventions` 通用层、`coding-review-deep-dive.md`、`project-governance-standards.md`；Wind 专项按依赖或上下文启用 | 通用能力解释 |
...
| 编码质量与 Java/Spring/Wind | 命名、异常、日志、契约、MapStruct、MyBatis Flex | 项目本地规范、`wind-coding-conventions` 通用层、`coding-review-deep-dive.md`；Wind 专项按依赖或上下文启用 | Review 结论、整改建议、验证命令 |
...
| “代码这样写对不对” | 是否违反项目规范、契约、边界、异常日志、测试红线 | Java 项目：本地规范 -> `wind-coding-conventions` 通用层 -> `coding-review-deep-dive.md`；Wind 专项按依赖或上下文追加 |
...
| “AI 写到后面越来越乱 / 长任务上下文太重” | 是需求不清还是上下文衰减，是否需要阶段状态、原子任务计划和 Wave 依赖 | `workflow.md` -> `ai-assisted-engineering.md` -> `ai-large-project-orchestration.md` |
...
| Wind 编码规则 | `wind-coding-conventions` |
