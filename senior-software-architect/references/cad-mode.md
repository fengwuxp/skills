...
| 从 GSD-like 编排进入 CAD | 2、3、4、8，并回读 `ai-large-project-orchestration.md` 的 GSD-CAD 双层协议 | 不消费整个 Roadmap |
| 从 AI Native 产品上下文或 MVP harden 进入 CAD | 2、3、4、8，并回读 `ai-large-project-orchestration.md` 的 AI Native 产品到工程 hardening | 不把产品上下文包、Hardened Candidate 或业务 MVP 当 Execution Grant |
...
中大型项目中的 CAD 只建议作用于一个已选定的原子任务包或阶段切片。GSD-like 编排负责大项目上下文、Stage、Wave、任务包和恢复入口；CAD 不直接消费整个 Roadmap，不跨 Wave 自动推进，不把 GSD 计划解释为 Execution Grant。
...
- Harness Plan 已形成：任务拆分、写入范围、只读范围、依赖顺序、验证命令、停止条件和交接方式清楚。
...
- 若来自 AI Native 产品上下文、Product Builder、业务 dogfooding 或 MVP/原型 harden，必须已经通过产品侧 Hardened Candidate 门禁，并由架构师转成 OpenSpec、Harness Plan、验证矩阵和单个 CAD 候选任务；不得把业务 MVP、PRD、产品上下文包或 GSD Roadmap 直接当 Execution Grant。
