...
出现以下任一情况，必须停止自动推进并回到人工确认：数据破坏、资金损失、权限绕过、敏感信息泄露、生产不可用、仓库状态不可控或不可逆风险；验证命令出现无法解释的严重失败；OpenSpec 与代码/测试/业务规则冲突；公共 API、DTO、枚举、错误码、数据库结构需要破坏性变更；需要新增依赖或引入新的稳定业务类型；涉及资金、权限、租户、审计、生产数据、密钥、外部接口或真实支付/资金通道；工作区混有用户改动；需要选择业务口径、产品规则、兼容策略、迁移路径或风险取舍；达到最大轮次、预算 / 时间盒 / 上下文预算上限，或触发无进展检测。
