...
结束时必须输出已执行验证、未执行验证、残余风险和最终是否仍需用户执行 Git 操作。专项审查覆盖：资深架构师审查 OpenSpec / 架构红线 / 编码约规 / 测试要求；复杂业务时路由产品架构专家；CAD 专项审查门禁、授权、每轮闭环、自动提交和用户已有改动；GSD-CAD 联动审查必须回写阶段状态、验证矩阵和 handoff，说明当前 Task ID 是否完成、是否满足退出条件、是否可以进入下一任务包或必须暂停。
