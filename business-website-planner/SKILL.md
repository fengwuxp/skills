...
description: |
  用户要求从零规划或重构企业官网，并需基于真实业务确定定位、内容模块、可信证据、指标示例和多屏素材边界时触发。单纯 UI/Figma、建站编码、商城交易、SEO 或法律文本不触发。
---
...
本 Skill 只持有官网业务契约，不负责 UI 绘制、Figma 写入、图片生成、代码实现、法律文本定稿或发布准入。跨阶段目标仍由当前 Agent 持有，授权范围仍以用户要求为准；能力装载遵守当前消费项目实际生效的规则，不依赖 Skills 源仓库 `AGENTS.md`。
