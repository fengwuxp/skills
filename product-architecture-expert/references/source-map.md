...
- 微信公众号文章《不愧是腾讯挖过来pm，这需求分析说的太好了》：`https://mp.weixin.qq.com/s/DbwaevfER4t32I9bS7qtUg`。页面由 `PMTalk产品经理社区` 发布，发布时间为 2026-08-30 09:00，文末另标“作者：Gali-AI产品版”“来源网络公众号”；2026-09-02 普通网页读取失败，随后通过本机 Chrome / Playwright 等价浏览器实际读取标题、发布时间和公开正文。公开内容用于参考需求追问，但该文只作为二手案例，吸收失败类型拆分、存量 / 增量目标、成功 / 失败 / 降级和半年工作系统变化等检查项；未取得具名当事人、腾讯内部材料或原始方法来源，不把标题、人物故事、四层漏斗名称、示例数字或“腾讯实践”写成已验证事实。
- OpenAI 官方《Model optimization》与《File search》：`https://developers.openai.com/api/docs/guides/model-optimization`、`https://developers.openai.com/api/docs/guides/tools-file-search`。2026-09-02 已读取当前公开正文；前者用于核验先建立代表性 eval，再按反馈调整提示、上下文或条件式训练，且当前 fine-tuning 可用性具有明显时效；后者用于核验文件检索基于已进入知识库的文件，不自动证明实时性、权威性或完整性。本仓库只吸收 eval-first、上下文与检索边界和时效复核条件，不把 OpenAI 当前产品状态写成跨供应商永久规则，也不把 RAG、微调或小模型设为单轴默认答案。
...
- 微信公众号文章《产品经理做需求分析，可以先用这几类 Skill》：`https://mp.weixin.qq.com/s/A6fMUljyFuI9dg6sQIiaPg`。账号 / 作者字段为 `鲨域`，发布时间为 2026-07-28 20:40；2026-09-02 通过 Codex Browser / Chrome Playwright 等价浏览器实际读取标题、作者、发布时间和完整公开正文。公开内容用于参考问题、证据、范围、风险和指标的需求分析短链，并只补强 `product-insight-analyst.md` 的逐项原始证据回链、`product-deliberation-workflow.md` 的承重假设红队和 `product-prd-operations-and-data.md` 的护栏 / 反指标；不复制原文、图片、提示词、固定数量或观察窗口，也不把文章的插件归类、安装命令或二手解释写成当前产品权威。
...
- GitHub 仓库 `phuryn/pm-skills`：`https://github.com/phuryn/pm-skills`。2026-07-02 已通过 GitHub raw 读取公开 `README.md`；2026-08-21 进一步读取 `pm-product-strategy/skills/product-strategy/SKILL.md`、`pm-product-discovery/skills/opportunity-solution-tree/SKILL.md`、`pm-product-discovery/skills/identify-assumptions-existing/SKILL.md`、`pm-product-discovery/skills/prioritize-features/SKILL.md` 和 MIT License；2026-09-02 复核公开 `v2.1.0` / commit `18468a95b427e70e258b51389796367c6f684e7d` 的 README、`discover`、假设识别 / 排序、访谈总结、`create-prd`、`strategy-red-team`、`north-star-metric`、仓库测试和许可证。当前公开结构为 9 个 plugins、68 个 skills、42 个 workflows，指标职责分布于 discovery、marketing-growth 和 data-analytics；仓库测试主要覆盖 manifest、版本、计数、引用和 frontmatter 一致性，不证明产品判断行为质量。本仓库只吸收从目标结果到机会、方案、假设与验证的判断链，以及价值、可用性、商业成立、可行性风险、承重假设红队的条件式检查；不安装该仓库，不复制固定画布、固定数量、slash commands、plugins 或 PM toolkit，也不把外部优先级公式、默认联网 / 写文件行为或结构测试替代本仓库证据、owner、验收、授权和停止条件。
...
- NN/g 文章《User Journeys vs. User Flows》：`https://www.nngroup.com/articles/user-journeys-vs-user-flows/`。2026-09-01 已读取公开正文。公开内容用于校准场景化端到端旅程与产品内具体流程的粒度差异，以及两者只在解决不同阅读问题时组合；本仓库据此把场景内推进合入业务场景，并把跨场景流程保留为条件视图，不复制文章图示、案例、课程材料或作者表达。
- GOV.UK Service Manual《Map and understand a user's whole problem》与 Local Government Association《Service blueprinting》：`https://www.gov.uk/service-manual/design/map-a-users-whole-problem`、`https://www.local.gov.uk/our-support/transformation/transformation-capability-framework/design-blueprinting`。2026-09-01 已读取公开正文。公开内容用于参考端到端任务、线上线下触点、后台流程、跨组织责任和支撑能力在同一分层视图中的表达；不复制英国政府流程、成熟度模型、工具清单、图示或组织治理要求，也不把服务蓝图强制为每份 PRD 的固定产物。
