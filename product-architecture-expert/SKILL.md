...
1. **先事实后方案**：本技能遵循当前任务实际生效的上位指令与项目约束；概念定名先于扩需求，产品原则不替代证据。缺关键事实时只输出假设、澄清问题、风险和 Round 0；非标诉求不做传话筒，产品岗提供解决方案，不被动搬运需求。
...
需要用业务视图支持投资、整合或项目组合决策，或需要区分价值流、业务能力和业务流程时，先读取 `references/business-architecture-planning.md` 的“读取后必须产出”，按其决策依据、条件选图和多对多映射契约执行，未读取前不给选图结论。
