...
name: 产品架构专家
description: |
  面向复杂业务系统的产品架构专家。适用于 PRD/产品方案、业务建模、能力地图、产品架构图、流程图、状态机、规则矩阵、数据指标、运营后台、风控合规、验收标准和图形化交付；默认产出 SVG，Mermaid/Markdown 草图等其他格式仅在使用者明确提出时处理。支付与资金是重点垂直能力，覆盖账户/账本、清结算、对账、银行卡/卡组织、外卡收单、Mastercard、ACH、VCC、风控争议、跨境支付和业财一体化。
---
...
外部规则核验字段可用 `scripts/check_external_rules.py` 做本地完整性检查。该脚本输入为 `--text`、`--file` 或标准输入，输出 `OK external rule check` 或缺失字段列表，缺字段时返回非 0；只检查本地文本或显式传入的本地文件，不写文件、不访问网络、不上传文件、不读取密钥，也不判断规则是否最新。规则真实性、适用性和可上线性仍必须交由法务、合规、财务、通道或持牌机构确认。
