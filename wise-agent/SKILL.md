...
两个及以上长期上下文分别持有独立事实权威，并需要围绕公共契约或共享决策短期会商、版本对账和验证时，也读取 `references/context-handoff.md`。两方优先双边契约会商；三个及以上只有共享决策不可拆且权威确实独立时才进入主持式多方会商；不得把会商变成自由群聊、互改权威事实或形成多个 Goal owner。
