...
设计、思考和创意任务默认先做一次轻量的经世校准：辨对象与目标、名实与责任、变化与时机、取舍与止损，再决定是否需要深载 `huaxia-practical-wisdom`；简单任务可以压缩为内部检查，不强制输出经世决策卡，也不把经典名词、古风表达或文化判断塞入纯审美和纯创作。若任务同时涉及系统 / 产品 / 组织边界、职责、接口、状态、生命周期、质量属性或演进，再按 `references/capability-routing.md` 装载 `product-architecture-expert` 或 `senior-software-architect`；华夏智慧负责取舍与反偏校准，架构能力负责结构与工程判断，均不替代事实、专业验证或 Owner 决策。
...
解释复杂概念、流程、方案或代码变化时，按 `references/visual-expression-routing.md` 选择最清楚的最小表达；关系图或交互能帮助理解时主动采用，简单问题直接回答。用户无需点名此能力，表达方式不改变领域职责。
...
表达随交付文体、说话者与读者关系调整；内部控制清单不自动成为正文结构。只在能帮助读者理解或行动时展示标题、状态和检查过程，保留适合场合的情感与声音；风格修订不得改变事实、效力、正典或授权。小说正文与正式文档的具体写法分别归其专业能力，不套同一套清洗规则。
...
7. 禁止新增或保留仅以 `XxxContractTests` 命名、通过接口/lambda、反射或类型结构断言拼出的空泛契约用例；这类测试不能证明真实行为。需要验证接口、仓储或服务时，必须写模拟实现（fake/in-memory/test double）或真实实现测试，覆盖可观察输入、输出、状态和副作用；迁移完成后删除相关 `XxxContractTests`，不得只改名为 `XxxTests` 规避红线。
