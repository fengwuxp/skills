...
- 已经选定单个原子任务并要求判断是否进入受控工程执行 Loop；详细规则交给 `senior-software-architect/references/cad-mode.md`。
...
- 单个原子任务的受控工程执行 Loop、轮内动作和工程停止细则交给 `senior-software-architect/references/cad-mode.md`；Execution Grant 仍由本文定义。
...
| 拆 Wave / Atomic Task 候选 | 3、4 | 不写工程执行每轮动作 |
| 判断工程执行 Loop 候选缺口 | 5、6 | 不把候选当 Execution Grant |
| 优化 GSD / Loop 授权策略或阶段提交 | 5、6、6A、6B | 不把全部动作默认通过 |
| 梳理 GSD / Loop 协同状态和交接卡 | 1、2、6、7、7A | 不直接进入工程执行 |
| 判断 GSD 是否可接入 Agent Loop | 1、2、3、6，再读 `delivery-execution-control.md` | 不把 Loop 写成 Execution Grant |
...
GSD 的目标是交付生产可用能力，不是让 AI 随机推进一组看上去有进展的模拟模块。所有 GSD Round、Wave、Atomic Task 和工程执行 Loop 候选都必须回答：它服务哪个真实业务目标，落在哪个生产边界或真实入口，如何由验收种子、测试、CR、发布/回滚条件或人工确认来证明可用。回答不了时，只能做 Round 0 补齐或只读侦察。
...
事实边界红线：GSD / 工程执行候选不得基于无根据猜测、模型脑补、工具总结、外部文章观点或超出用户目标的功能扩张。凡是没有用户目标、来源材料、源码锚点、验收种子或验证证据支撑的内容，只能写入 `推断 / 待确认 / 范围外不做`，不能写成任务、实现、准入结论或授权缺口。
...
| 工程执行 profile | 已选定的单个原子任务如何在 Loop 内受控推进。 | `资深架构师` |
...
| 成熟度 | 结论 | 典型动作 |
| --- | --- | --- |
| 想法 / 文章观点 / 工具宣传 | 不进入 GSD 或工程执行 Loop。 | Round 0 补目标、主体、证据、风险、验收。 |
...
- 当前授权策略是否清楚：只读、计划内低风险执行、Plan Grant、Wave Grant、Execution Grant、Codex 替我审批，还是需要显式确认。

缺任一关键项时，只输出补齐清单，不建议进入工程执行 Loop。
...
不满足准入卡时，只能输出 Round 0 补齐清单或回到产品专家，不得生成 Wave、Atomic Task 或工程执行 Loop 候选。
...
- 是否可能成为工程执行 Loop 候选，以及阻断原因。
- 是否可能成为 Loop 候选，以及状态载体、反馈源、验证者、预算 / 最大轮次、无进展检测和停止条件缺口。
- 建议授权模式：只读、计划内低风险执行、Wave Grant、Execution Grant 或显式确认。
...
工程执行 Loop 候选只写“候选”和“缺口”，不写成执行授权。判断时逐项检查：

- 是否只有一个已选定的 Task ID 或阶段切片。
- 写入范围是否足够窄，且能区分只读参考。
- 是否有可运行或可替代的验证命令。
- 是否有停止条件：规格不清、验证失败、权限不足、风险升级、用户中断。
- 是否已经区分事实、推断、待确认和范围外不做；未区分时不能进入工程执行。
- 是否已有测试策略、Review 和完成前验证门禁，或明确说明当前任务为何不需要；缺少时只能标为工程执行 Loop 候选缺口。
...
- `不是工程执行 Loop 候选`：缺任务边界、验证或风险确认。
- `工程执行 Loop 候选但缺授权策略`：任务边界和验证清楚，但缺默认推进范围、显式确认边界或工具层权限。
- `Execution Grant 候选`：任务边界、低风险写入范围、验证命令、停止条件和默认审批通道清楚，可在 Grant 有效期内默认推进。
- `可交给架构师检查工程 profile`：已有单个原子任务、验证和授权草案，下一步读 `cad-mode.md`。
...
交接给 `资深架构师` 时，应形成一页式 `Engineering Handoff Card`；没有这张卡，不得把 GSD / Loop 候选直接写成工程执行任务：
...
- 需要单个任务工程执行门禁：交给 `senior-software-architect/references/cad-mode.md`。
...
- 架构师收到三卡后，先检查是否可消费；可消费才进入 OpenSpec、Harness、GSD 任务包、测试策略、工程执行门禁、源码级 CR 或发布风险设计。

## 8. 质量 / 测试门禁联动

GSD / 工程执行准入必须同时给出质量门禁位置：
...
没有验证矩阵或测试结果交接路径时，不建议进入工程执行 Loop。

## 9. 反模式

- 把整个 Roadmap、GSD 计划或 Wave 清单当成工程执行授权。
...
- 把无根据猜测、模型脑补、工具总结、外部文章观点或超出用户目标的功能扩张写成 GSD Wave、Atomic Task、工程执行 Loop 候选或实现建议。
