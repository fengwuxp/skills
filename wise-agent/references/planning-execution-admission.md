...
只回答当前准入问题，按命中场景使用下文章节；读取本文不要求生成全套卡片。

- 目标、授权与验证清楚：直接执行或给出当前可执行切片；需要组织多步依赖时使用简短计划。
- 真实缺口：给出影响、证据入口、owner 和依赖它的暂停动作；只在路线仍模糊且需要跨轮恢复时输出决策寻路地图。
- 实际采用 GSD、Wave、工程执行 Loop 或 AI 产品工程化：补对应章节要求的输入、边界与证据，已有授权不退回候选。
- 跨责任人或跨会话交接：复用已有载体记录当前状态、写入范围、验证、停止条件和下一 owner；项目要求三卡交接时核对三卡，不并建重复文档。
...
授权策略不是让每个任务都重新问一次，也不是让所有动作无条件通过。GSD / 工程执行应把授权前移到 项目执行规范 任务计划、Wave 或 Execution Grant 层：用户明确说 `GSD + 项目执行规范 按任务计划推进`、`按任务计划自动推进` 或等价意图时，如果任务计划已经列清目标、写入范围、验证命令、停止条件和显式确认边界，可以形成 Plan Grant；在 Plan Grant 有效期内，低风险原子任务直接推进，不再逐任务索要 Execution Grant。验证失败按第 6 节判断是否可在范围内修复；越界或触发未授权高风险动作时暂停相关动作并升级确认。

Codex 的“替我审批”只能作为当前会话已经由用户开启的运行时审批模式：它可以承接低风险、可回滚、范围内的工具审批噪音，但不能被 Skill 自行开启，也不能绕过项目规则、sandbox、用户授权、Git/联网/生产等硬门禁。

Agent Loop 只能建立在 项目执行规范、GSD、Harness 和授权策略之上。旧 `/goal` 输入必须先迁移到项目执行规范；`/loop`、auto mode、后台 Agent 或多 Agent 监督若没有状态载体、反馈源、验证者、预算 / 最大轮次、无进展检测和停止条件，只能作为待补齐的 Loop 候选，不能进入自动推进。
...
`Plan Grant Active` 是授权状态，不是 执行状态；工程执行 Loop 候选是准入结论，不是可写状态。跨责任人、跨会话或项目明确要求时，按第 7 节使用 `Engineering Handoff Card` 承接；同一 Agent 在已授权范围内切换阶段，可以直接消费已有任务依据，不另补交接卡，也不能把 Loop 当作授权。
...
一次只向 Owner 提出一个主 blocker；可查事实和已确认决策可同轮收敛，不机械限制决策关闭数量。按节点性质复用现有能力：
...
地图存在阻断 Destination 的开放 Frontier 或 Not yet specified 时，整体项目执行规范 保持 `Draft`，不得把依赖未决判断的内容写成已就绪 Spec、Wave、Atomic Task、Plan Grant 或执行方案；边界独立、已确认且已授权的切片仍可推进。路线清晰后才进入 Spec、最小计划或 项目执行规范 Ready；若初始梳理没有迷雾，立即停止建图并回到当前任务的最短交付路径。
...
- 写入范围是否足够窄，且能区分只读参考。
- 是否有可运行或可替代的验证命令。
- 是否有停止条件：规格不清、验证失败且无法在范围内收敛、权限不足、风险升级、用户中断。
...
| Plan Grant | 用户明确要求按任务计划推进，且 项目执行规范、任务计划、写入范围、验证命令、停止条件和显式确认边界已列明。 | 任务计划内低风险本地读写、测试、lint、文档/Skill/业务代码小步修改、Ledger/状态更新。 | 未授权 Git 提交、联网、依赖安装、生产/密钥/部署/不可逆操作、高风险业务、计划外写入、验证失败且无法在范围内收敛。 |
...
授权来自用户原话与其指向的已确认范围，不来自 Grant 名称或表单。普通已授权任务无需另建 Grant；采用计划、Wave 或 Loop 授权时，以下字段从现有任务载体复用，只有实际启用 Loop 才需要 Loop 预算。

验证失败先读取证据：预期 TDD RED、原因明确且可在范围内修复的失败，继续最小修复并验证；不得把失败当作扩范围的授权。无法确认契约、连续无进展、需要越权或扩大风险时，暂停依赖该结果的动作并交还相应 Owner；无关既有失败单独报告，不认领修复。
...
用户说“继续”“按建议推进”时，回指本轮上下文中已提出且已确认的目标、范围与动作；授权充分就执行，不把它降为流程意向或再索要固定格式的 Grant。若所指建议不明或仍有会改变行动的责任决策，只澄清该缺口，继续不依赖它的已授权工作。“自动跑起来”也不自动授权后台循环、高风险动作或范围扩张；实际采用 Plan Grant 时，范围内任务不再因缺少逐任务 Execution Grant 停止。
...
失败处理: 按第 6 节处理验证失败；无法在范围内收敛、范围漂移、用户改动冲突或风险升级时暂停相关动作并交还用户
...
- 实际采用 Loop 时，Plan Grant 已绑定 Loop 预算：状态载体、反馈源、验证者、预算 / 时间盒、最大轮次、无进展检测和失败回写位置清楚。
...
- 能区分用户已有改动和本轮改动，并按第 6 节处理验证失败，不覆盖用户改动或扩范围。

触发以下任一情况时，相关动作暂停并升级显式确认，不撤销无冲突部分的既有授权：计划外文件、未批准的公共契约破坏性变更、跨 Wave 扩张、验证失败且无法在范围内收敛、用户改动冲突、工具要求额外审批、未授权 Git/联网/生产/密钥/部署/不可逆动作、高风险资金/权限/租户/审计/合规决策。

## 7. 交接给架构师

需要向独立执行者、后续会话交接或项目明确要求时，使用一页式 `Engineering Handoff Card`，字段可复用已有任务文档；同一 Agent 按需装载 `资深架构师` 不构成交接。真实交接仍须核对范围、授权、验证和停止条件，不能把 GSD / Loop 候选直接写成执行授权：
