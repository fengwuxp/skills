...
- 需求仍处于问题发现阶段时，不直接进入 Harness 或工程执行 Loop。
- 小范围低风险修改不需要完整 GSD；可用目标、写入范围和验证命令轻量执行。
- 不用本文替代 GSD / 工程执行编排准入、`资深架构师` 的 `ai-assisted-engineering.md`、`ai-large-project-orchestration.md` 和 `cad-mode.md`。
...
- 若用户要求升级 SDD / Superpowers / Harness 版本，必须说明当前采用的内部 Harness 契约版本、吸收的上游方法、不吸收的外部脚本和授权缺口。
...
| 工程执行 Loop 准入 | 先读 `planning-execution-admission.md`，再读 `4. 受控工程执行 Loop 只处理原子任务`、`6. 权限边界` | 不把整个项目交给工程执行 Loop |
| AI 原生工具接入 | `6. 权限边界`、`7. 工具角色` | 不把工具能力当组织授权 |
| AI 代码库理解与影响结论包 | `3. Harness 最小契约`、`5. Wave 和交接`、`7. 工具角色` | 不把 AI 总结、可视化或上下文生成当测试或 CR 结论 |
| AI 代码交付闭环 | 先读 `3. Harness 最小契约`，再读 `code-delivery.md` | 不只优化编码速度或加厚 Spec |
| 流程治理评审 | `8. 治理清单`、`9. 反模式` | 不只看效率指标 |

## 1. 分级模型

默认直接执行。下表按实际采用的机制补充约束，不能用风险、文件数、跨模块或任务时长自动触发先写文档的流程：

| 级别 | 适用场景 | 最小门禁 |
| --- | --- | --- |
| 直接执行 | 目标、范围、授权明确，包括多文件、跨模块和多轮任务。 | 读取事实与约规、实施、验证；不要求规划文档。 |
| OpenSpec | 用户或项目明确采用 OpenSpec 并要求对应交付。 | 目标、范围、非目标、规则、验收、技术约束。 |
| Harness / GSD | 用户明确要求该编排，或确有独立执行者派发和恢复需要。 | Task ID、owner、写入范围、依赖顺序、验证矩阵、交接。 |
| 工程执行 Loop 候选 | 单个边界清楚、验证明确且需要多轮反馈的原子任务。 | 冻结决策 + 写入范围 + 验证方式 + 状态载体 + Grant + 停止条件。 |
...
- **派发前核对**：派发独立任务前核对输入、跨任务接口、写入冲突、验证与授权；已有计划可以作为输入，不要求先编写或批准计划文档。
...
- 本仓库不再复制 Superpowers 上游 Skill 或 helper；上游 release、Codex 市场标识、manifest 版本和工具支持状态在实际比较或升级时分别核验。
- 外部 release、marketplace、bootstrap、脚本和工具支持变化只作为带日期的 source-map 事实，不改变 Harness v3 默认执行边界。
- 不默认创建 `.superpowers/`，除非项目规则或用户明确授权并说明 gitignore、清理、恢复和隐私边界。
- 不把上游 v6 的性能数字、PR 数、token 降幅或 harness 支持列表写成当前项目收益；本仓库只采用可验证的流程约束。
...
本节只在用户显式调用 `$wise-agent` 后用于该任务；未显式调用时不得仅因任务复杂、跨阶段或可并行而启用这套模型分工。

```text
选择原因:
角色: implementer / batch_worker
运行时路由: agent_type / model + reasoning_effort
目标:
当前阶段: 只读侦察 / 执行
允许读取:
允许写入:
...
- 只读侦察可以返回候选范围，但不得写入。
- 执行阶段必须显式列出写入文件；发现需改未列出文件时停止并返回 `SCOPE_GAP`。
- `implementer` 只接冻结方案后的实现，`batch_worker` 只接无需业务判断的固定规则任务；主代理等待结果并独立验收。
- 优先显式传入 `agent_type=implementer` 或 `agent_type=batch_worker`；`task_name` 只命名任务，不选择角色、模型或推理强度。
- 当前角色绑定为 `implementer` 使用 `gpt-5.6-terra` + `high`，`batch_worker` 使用 `gpt-5.6-luna` + `medium`；模型绑定是时效性运行时适配。`agent_type` 不可用时，只有运行时明确开放目标模型覆盖才用隔离任务包回退；目标模型不可选或调用失败时不得猜测替代模型。
- 运行时既不支持 `agent_type` 也不支持模型覆盖时，回退为主代理直接执行；若模型分工本身是验收条件则停止，并明确说明额度优化未生效。
- 子代理不得扩大当前会话、项目和任务包授权；角色或模型选择不授予联网、Git、安装、部署、生产、密钥、删除或不可逆权限。
- 路由证据优先使用工具返回的实际角色、模型和推理强度；运行时不提供时标记为未确认，不以任务名、提示词或子代理自述代替，也不扫描用户目录补证。
...
受控工程执行 Loop 不是“整个大项目自动驾驶”，只适合已具备以下条件的原子任务：

- 目标、预期行为和必要业务上下文清楚，业务与架构决策已经冻结。
- 写入文件范围明确且可独立 Review。
- 依赖顺序清楚，不会和其他任务互相踩踏。
- 有可运行测试、编译、lint、静态检查或人工验收方式。
- 预计至少需要两轮实现、反馈、修正或验证；一次可完成时使用轻量执行或标准工程流程。
- Loop 状态载体、反馈源、验证者、预算 / 最大轮次和无进展检测可用。
- 停止条件明确：测试失败、契约不清、权限不足、风险升级、用户中断。
- 用户已明确授权当前任务的执行范围；授权不自动外推到提交、联网、部署或生产操作。
- 当前任务若属于已批准的 Wave Grant 或 Execution Grant，且只触发低风险、本地、可验证、可回滚动作，可以按默认授权推进；命中显式确认边界时停止。
...
- `Execution Grant` 只覆盖一个原子任务的 Pick / Build/Test / Review / Verify / Record 本地动作。
...
- 是否区分轻量任务、GSD Wave 和工程执行 Loop 候选。
