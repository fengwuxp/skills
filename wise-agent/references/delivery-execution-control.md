...
规格完成与单次运行完成必须分开。`单次运行完成信号` 使用可观察证据说明本轮确已达到结果，例如检查结果、退出状态、已记录状态变化或外部对账，不以 Brief 已生成或 Agent 自述代替；`完成动作` 说明检查点之后如何归档、标记、转交、发布或保持待处理，并在可能产生部分结果时写清部分结果是否算成功。`失败策略 / 恢复或回退` 先区分未执行、部分成功与状态未知：状态未知时不得直接重试；已经发生部分副作用时，先回读事实，再按对象契约选择对账、隔离、补偿、回滚或人工修复。恢复事实明确后，才按实际失败代价选择有界重试、跳过、暂停或人工升级；重试边界由外部契约、重复副作用和 Owner 决策确定，不设跨场景固定次数。结果显然且无外部副作用的一步流程可各用一句话表达，不为字段完整增加无主机制。
