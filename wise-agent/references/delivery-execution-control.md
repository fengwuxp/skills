...
计划生成准入先于计划审查：完整方案或实施计划仍夹带未决 Owner 判断时，装载独立 `grill-me` 关闭最高价值 Decision；完整盘问、问题台账、历史去重、自决和红线规则读取其 `SKILL.md` 与 `references/question-ledger.md`。决策快照经 Owner 确认后，才生成与已确认选择一致的最小计划。
