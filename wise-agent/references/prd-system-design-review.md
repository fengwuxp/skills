...
本文定义知止者流程中的 PRD / 系分预审门禁。它吸收 MAGI 三角色与多视角虚拟评审的可迁移方法，但不要求真实启动多个 Agent，也不替代正式产品评审、架构评审、代码 CR 或专业审批。
...
- 对已被工程事实证伪的描述性现状假设，停止依赖该假设的系分与实现，把原产品假设重新标为 `PENDING`，记录冲突证据和影响并交产品 / 业务 owner 裁决。
- 裁决或工程处置确认后，只在描述性假设被纠正或产品契约确有变化时更新 PRD，并同步更新受影响的系分、验收种子和 Decision Log，再恢复下游执行；不得让工程静默兼容、绕过 owner，或把当前实现和历史数据直接升级为新的业务规则。
