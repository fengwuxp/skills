...
预审报告和决策日志是过程资产，不是 PRD 或系分正文。进入正式文档时，只吸收 `ACCEPT` 后的最终结论、`PENDING` 待确认项、当前风险和验证要求；`REJECT` 的长理由、角色争论、AI 推理轨迹、多轮草稿和旧版本流水留在报告、Decision Log、项目执行规范 或任务计划中，通过链接追溯。
