...
下列阶段用于用户要求的生命周期覆盖分析，不是执行任务的必经顺序；当前目标需要实现时直接读取事实、实施、验证，不先补齐上游文档。
...
按当前目标选择所需工作，跨阶段不自动触发规划、文档审批或交接卡。Loop 的价值是让行动与验证持续闭合。
...
实际交给其他责任人或恢复执行时，复用已有记录补充必要事实；同一 Agent 的阶段切换不填卡、不先写文档。项目明确要求交接卡时，按需使用以下字段：
...
真实编码以业务入口、有效授权、当前范围、适用约规和可验证结果为依据。独立 Checker、持久状态、提交切片与回滚方案按实际风险和交接需要增加，不作为所有编码任务的统一开工条件。
