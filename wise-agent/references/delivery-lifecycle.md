...
- 现有流程只强调 AI 自动执行、GSD / 工程执行/项目执行规范/Loop 机制，但缺少每一阶段 owner、交接物、门禁和停止条件。
...
按当前任务涉及的生命周期责任给出结论，不因读取本文展示全套角色链、控制卡或反馈图。

- 当前交付内容、验证证据，以及仍需承接的阶段责任；能力 owner 直接引用 `capability-routing.md`。
- 存在真实未决判断时，说明缺口 owner、影响和暂停点；证据充分且已授权时直接进入下一步。
- 真实跨责任人、跨会话或项目要求的交接，复用已有产品上下文卡、工程执行交接卡或生产 Loop 交接卡，只补必要变化。
- 涉及生产生效时，按变更类型给出相应验证、发布 / 回滚证据和残余风险；方法图与控制卡仅在需要审查对应机制时展开。
...
| 进入知止者 | `1. 端到端阶段`、`1A. 阶段与能力衔接`、`1.2 角色视角顺序`、`3. 阶段交接门禁`，具体主能力与协同能力回到 `capability-routing.md` 判断 | 不把 GSD / 工程执行/项目执行规范 当主流程 |
...
决策澄清门禁只处理会改变当前行动的未决判断，复杂或跨阶段本身不触发问询。关键分叉、含糊回答、半答或连续返工时按需装载 `grill-me`；完整盘问、问题台账、历史去重、自决和决策快照由该 Skill 负责。本生命周期消费已有确认和新增证据，不把问询过程写进正式 PRD / 系分正文。

Loop 推进中适时装载 `grill-me`；盘问、问题去重、shared understanding、决策快照和红线记录读该 Skill，执行前对账读 `delivery-execution-control.md`。

每个阶段的小闭环完成后，先自问再问人：用现有事实、源码、测试、约规和交接物核对下一步；证据足够且仍在授权内时继续，实际需要恢复或交接时只回写变化。缺少 owner 责任判断时，只询问阻塞当前行动的问题；证据仍在收敛且预算允许时继续取证；越权、无进展或无法验证时停止相关动作并交接。
...
规划到执行消费已有目标、授权、范围与验证依据；同一 Agent 可直接推进已就绪任务，不把完整计划或 Execution Contract 文档作为统一前置。跨责任人、跨会话或项目明确要求时，复用原生载体承接 owner、状态、写入范围、验证命令、Checker、停止条件和回写位置，不靠旧摘要猜测。CR 或测试发现代码与 Spec 漂移时，先判断是代码偏离、Spec 缺失还是需求变化；代码偏离修代码，Spec 缺失先补 Spec，需求变化走影响识别。
...
| 反馈回流 | 知止者 + 对应 owner | 整理复盘、知识归位和 fixture 缺口 | Decision Log、项目执行规范、Skill / fixture / 脚本更新建议 | 不把未经验证经验沉淀为规则 |
...
2. 再按 `capability-routing.md` 选择一个主能力、必要的受控协同能力、明确不加载的能力和独立 Checker。
...
- 用 GSD / 工程执行/项目执行规范 等机制名替代角色协作，导致设计、评审、TDD、编码、CR、安全 / 可用性评估和发布门禁混在一个 Agent 输出里。
