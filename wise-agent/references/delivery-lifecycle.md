...
本文定义知止者的交付生命周期：从业务意图、需求收集、产品 / 交互设计、设计评审、TDD / 测试设计、真实编码、编码评审、可用性 / 安全性 / 可靠性评估、验证发布到运行支持、维护退役和反馈回流。它用于跨角色交付编排，不替代产品专家 PRD、架构师系分、源码级 CR、测试实现、发布审批或生产 owner 决策。
...
| 控制不同 Skill 完成任务 | `1.1 阶段能力与约规来源 / 角色协作判断矩阵`、`3. 阶段交接门禁` | 不让知止者直接代写专项产物 |
...
| 进入知止者 | `1. 端到端阶段`、`1.1 阶段能力与约规来源 / 角色协作判断矩阵`、`1.2 角色视角顺序`、`3. 阶段交接门禁` | 不把 GSD/CAD/Goal 当主流程 |
...
知止者在各阶段持续持有目标、上下文和最终交付。阶段名只是工作坐标，不是能力来源；每个阶段必须回指实际装载的主能力，缺少能力时先补输入或安全接入能力。本矩阵用于约束能力选择：先定 Loop 节点，再定人类责任 Owner、主能力和独立 Checker，工具和框架只能补一个缺口。
...
| 发布准出 | 发布 owner / 架构师 | 知止者 + `资深架构师` | GStack `/ship`、Harness 证据链、验证发布 reference | 生产交付审查卡、生产生效验证卡、监控 / 回滚 / 人工接管证据 | 不替代 Git / PR / merge / 部署授权 |
| 知识回流 | 知止者 + 对应 owner | 知止者 | Trellis 思想、Context System、source-map、fixture | reference / fixture / 用户指南 / source-map 更新建议 | 不沉淀未验证经验 |
...
- **知止者编排者**：决定当前阶段、owner、交接物、验证门禁和停止条件；不替代 PRD、系分、代码、测试、CR 或发布审批。
