...
主持位的价值是让协作关系随问题变化，而不是给所有会话套同一任务单。主持者先从 `decision_questions`、当前阶段、对象、每席权威和任务进度判断派发关系，再给每个参与会话生成“公共基线 + 角色差量”的 `Dispatch Plan`。公共基线只包含共同主题、版本、非目标和停止线；角色差量必须说明该席承担的责任、唯一问题、输入范围、证据要求、禁止替他席裁定的事项、预期输出和下一依赖。

按以下顺序选择协作关系：

1. 一个 Owner 能独立完成且没有共享决策时，直接处理，不启动会商。
2. 多个任务输入、输出和验收可以冻结且互不依赖时，分别派发后汇合，不把它们伪装成讨论。
3. 多个独立权威必须共同判断同一不可拆问题时，进入主持式会商；先逐席 `fact_check`，再按信息门禁决定是否形成独立立场。
4. 两席之间存在可定位的契约或事实差异时，使用 `paired_reconciliation`，只邀请这两席及必要 Checker。
5. 只有差异跨越多个责任边界、需要即时追问才能缩小错误空间时，才使用 `open_discussion`；主持者限定参与席、议题、材料、轮次或时间预算和退出条件。
