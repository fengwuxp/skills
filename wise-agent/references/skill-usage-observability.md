...
首次受控试点至少包含未点名时默认加载、显式调用 `$wise-agent`，以及用户明确禁用该 Skill 的对照任务；简单任务核对直接回答和无多余装载。JIT 配对任务还应覆盖明确标题、歧义查询、跨章节依赖和高风险红线四类，使用相同重复次数观察方差；未形成当前项目基线前不预设收益阈值。原始 Hook / OTLP 请求只能在进程内或临时目录出现，不得进入账本或仓库。
