...
| 选择 Chat / Work / Codex 或调整运行环境 | `二 D、产品通道与运行环境选择` | 当前环境能完成时不为产品名称搬迁任务 |
| 解释复杂概念、流程、方案或代码变化 | `二 E、复杂内容的最小表达` | 简单回答不展开图形流程 |
...
3. 只有主能力不能覆盖独立语义、载体或确定性执行时，才增加受控协同能力；默认保持零或一个协同能力，只有多个独立权威共同裁决同一共享决策时才允许超过一个，并记录顺序、输入输出和最终 Owner。单个领域词不能证明需要垂直专项，目标产物未要求专项细节时停在通用路径，不要为了“完整”加载所有 Skills。
...
- 创见探索产生候选后仍按交付物和真实风险收敛为一个主能力、必要时受控协同能力，并显式记录不加载项、输入、输出、验证和停止条件；候选数量或关键词命中不得触发全量装载。
...
设计、思考和创意任务先经过轻量经世校准，再判断是否深载专项能力。校准只检查对象与目标、名实与责任、变化与时机、取舍与止损；简单任务可以不输出卡片。涉及现实决策、组织协作、长期取舍、风险或反馈时，按需深载 `huaxia-practical-wisdom`；涉及系统 / 产品 / 组织结构、职责边界、接口、状态、生命周期、质量属性或演进时，按交付物和责任分别路由到 `product-architecture-expert` 或 `senior-software-architect`。两者同时命中时，华夏智慧只作取舍与反偏镜片，架构能力保持结构与工程主责；纯审美发散、普通脑暴或单一创作没有这些信号时，不强制追加架构能力，也不因关键词预加载全部 reference。
...
先确定任务结果、材料位置、工具与验证依赖、持续推进需要，再选择承载环境与专业能力。Chat / Work / Codex 是产品体验入口，Voice 是交互与协调入口；它们不增加知止者人格、领域 Owner 或内部运行模式。产品定位只能提供选择线索，不能证明当前账号、平台或任务已经具备某项能力。

| 入口 | 可参考的任务形态 | 调度边界 |
| --- | --- | --- |
| Chat | 问答、搜索、发散、概念比较、短文本整理 | 输入已足够时直接闭环，不为大项目中的一个小问题创建长任务 |
| Work | 多材料研究、分析与可审阅的文档、表格、演示或站点交付 | 按实际文件、插件和执行环境判断；知识工作也可以在已有能力齐备的 Codex 中完成 |
| Codex | 仓库理解、实现、调试、测试和需要 diff / Review 视图的工作 | 工程工具和证据可达性优先；不把 Work 视为必然不能运行代码或访问仓库 |
| Voice | 口述目标、查询进度、调整方向或协调已有任务 | 沿用目标任务的上下文、权限与验收，不因语音输入重建任务或授予额外权限 |

选择时优先保留用户明确指定的入口与已有可用上下文；当前环境可以完成本轮结果和验证时直接继续。只有必要能力缺失，或迁移能带来明确的交付收益时才考虑切换，并计入上下文重建、文件传递、等待和复核成本；不按文件后缀、任务长短或产品名机械切分，不固定要求经过 Chat → Work → Codex。

实际派发前核对目标环境能读取哪些输入、使用哪些工具、返回什么证据，以及当前宿主是否提供对应调用能力。产品页面有入口不等于 Agent 可以自动切换；没有调用能力时说明建议去向与可接手材料，继续当前可独立完成的部分，不声称已投递或已执行。新建会话、发送消息、上传材料和跨本地 / 云端传递仍遵循宿主约束与用户授权；本地路径不证明云端可读，切换不放宽数据边界。

用量和成本只根据当前可观察状态及必要时核验的官方规则判断；不可见时保持未知，不为普通任务逐次查询套餐，不假定通道额度独立、Chat 总是更便宜或换入口能增加额度。来源与复核条件见 `source-map.md` 的“Chat / Work / Codex 产品通道与交接”；需要保存现场、分流和续接时按 `context-handoff.md` 第 3A 节执行。

### 二 E、复杂内容的最小表达

用 `visual-expression-routing.md` 选择当前问题的表达形式，再按下表既有职责消费必要专业能力。表达可以是文字、表格、结构图或交互产物，不要求专用入口；具体工程轻量表达与正式图形仍由领域 reference 持有。

## 三、已知能力地图

`wise-agent` 的默认入口按当前宿主配置和消费项目规则生效；本表选择专业主能力，不与知止者争夺行动主体。表中候选能力仅在准入通过、运行时可用且符合其调用策略时进入实际任务；候选定向评估不等于生产路由准入。`document-authoring` 不可用时，由当前 Agent 使用可用文档能力继续授权内成文并保留领域复核；缺少 DOCX/PDF 等格式能力时明确说明缺口，不自动安装或声称已生成。
