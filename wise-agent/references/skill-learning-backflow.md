...
| 对候选文本反复调优或开展优化实验 | `6.1 优化实验的固定契约与留出集` | 普通修复不建立优化数据集 |
...
只有用户明确开展 Skill 文本的反复调优实验，或反复依据开发样例调整候选并要据此声明泛化收益时使用本节。普通 Bug 修复、补测试、措辞、格式或引用修订沿用必要回归，不创建训练集、留出集、`optimization/` 目录或新的审批步骤。

- **固定边界再调文本**：在既有试验记录中明确可改文本，以及本轮不可改的目标、授权、证据底线、评分规则和准出条件，绑定基线与这些依据的版本或指纹。源仓库的 `scripts/skill-evolution-control.py` 已支持 `frozen_sections / editable_sections`，按需复用；消费项目没有该脚本时按同一边界检查实际 diff，不假设它随 Skill 安装。其章节检查不证明外部评测规则未变，Checker 仍需对照试验依据。
- **开发集与留出集分开**：开发集用于定位失败和调优；留出集由独立评审方保管，仅在候选版本固定后用于评估。记录两组案例的身份、内容指纹、持有者和使用状态，按场景检查重复及语义近似泄漏。调优者不得读取留出题目、答案或逐题反馈；仅改文件名、换集合 ID 或盲化 A/B 标签都不构成隔离。
- **不把用过的题重新称为未见题**：一旦根据留出题目或反馈继续改候选，该集合对后续候选转为开发证据；再声明泛化收益须换用未参与调优的案例。保留历史结果，不刷新摘要、删除失败项或重命名集合来恢复留出资格。
- **验实际结果**：沿用同一 runner/model、来源绑定、独立盲评与 release gate。涉及工具或写入时核对原始轨迹、真实产物和副作用；回答包含“不覆盖”或“已验证”不能替代证据，也不以禁止“我无法”等固定措辞惩罚如实说明缺口。关键越界、伪造事实或权限失败不能靠平均分提升抵消。
- **目标变化另记依据**：目标或验收确需改变时，由 Owner 明确新依据并重新建立基线；不能为了使当前候选通过而降低门槛，再把结果描述为同一契约下的改进。留出证据不足只停止泛化或晋升结论，授权内的开发与局部验证可继续。

本节不启动模型、训练或发布，不授予新的读取与写入权限；source / case digest 只能绑定证据，不能证明数据未泄漏。真实隔离和留出使用记录由试验执行者与独立 Checker 验证。
