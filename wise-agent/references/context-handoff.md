...
- 下一会话只是按既有 项目执行规范、决策集合、恢复入口和下一动作继续工作，且没有运行环境变化或未对账的中断事项。
...
| 跨产品通道、运行环境或中断后续接 | `3A. 跨通道交接与临界点续接` | 不复制完整会话、不固定创建检查点文件 |
...
通道选择由 `capability-routing.md` 的“二 D、产品通道与运行环境选择”负责；本节只规定必要交接与恢复。沿用 `Continue / Branch / Worker / Checker` 和既有状态载体，不要求问题卡、证据图谱、检查点各建一个文件，也不新增调度服务。

- **交接可消费的产物**：在现有任务记录中按需保留目标与验收、已确认与待确认项、输入和产物的来源 / 版本、已执行动作与验证结果、尚未完成的动作、授权与停止条件、下一步及承接者。接收方实际可读的最小材料才可传递；本地路径在目标环境不可访问时，只说明缺口或使用已授权的必要摘录，不默认上传、复制完整历史或假设对方已读。立即消费的短内容可内联，跨轮产物保留可回读指针。
- **临界点先保存现场**：用户要求暂停、实际用量 / 时间 / 工具状态表明当前切片可能无法继续，或确定需要上下文迁移时，在安全边界停止新增探索与派发，优先记录上述状态。保留进行中动作的任务或执行标识、部分结果及未知结果；未读到完成证据就保持未完成，不能为存档盲目中断动作，也不能在恢复后重复执行结果不明的副作用。普通阶段切换、固定 token 数和猜测的剩余额度不自动触发此流程。
- **只分流独立部分**：在已有任务记录中维护依赖、Owner 与下一动作；会商中继续使用主持台账。只转移输入足够且不依赖受阻环境的证据整理、文本或验证准备；依赖本地源码、工具结果或原 Owner 裁决的工作保持 `PENDING`，写明唤醒条件。检查点不是任务完成或自动恢复调度的证据。
- **恢复先核对差量**：先回读当前权威和产物版本、接收方真实访问范围、未完成动作及已有回执，再消费分流返回的增量。沿用 `delivery-execution-control.md` 的认知完整性对账；摘要不补造事实、测试结果或授权。缺口只阻塞依赖它的切片；已经闭环的步骤不重跑，也不因更换入口重复请求仍有效的授权。
...
单点压力来自上下文、工具或可观察的用量限制时，主持者按第 3A 节保存现场、判断可转移部分并对账，继续复用本台账，不另建通道状态源。
