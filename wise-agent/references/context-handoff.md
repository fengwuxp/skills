...
| 双边契约会商 | `1. 正名与准入`、`4.1` 至 `4.4` | 不把双方变成平级 Goal owner，不永久互聊 |
...
两个及以上长期上下文分别拥有独立事实权威，且需要围绕公共契约或共享决策短期共同发现时，使用“主题对齐 -> 信息交换 -> 充分性门禁 -> 定契 / 立场 -> 验证 -> 裁决 -> 退场”。这是既有 Goal 下的场景路由，不是 Branch、handoff、新控制机制、群聊模式或常驻多 Agent 组织。
