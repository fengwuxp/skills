...
Plan-to-Goal Bridge 是进入 Goal / Plan Grant 前的桥接检查，不是新的场景视图、模式或独立 Skill。它用于把“先计划，再创建 Goal / 按计划推进 / 验证完成”这类诉求收敛进知止者的产研交付视图。
...
- 持续优化单一指标时，在既有 `非目标 / 停止条件 / 验证矩阵` 中至少写一个反证条件或护栏指标；指标上升不能单独证明 Goal 达成。
