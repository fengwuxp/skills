...
check(
    ...
    and has_all(
        wind_skill_conventions,
        [
            "服务、接口、策略、工厂、状态机、规则层和配置化必须来自真实责任或变化轴",
            "业务规则、状态行为、外部通道、平台差异、技术选型",
            "不为套设计模式新增浅服务",
        ],
    )
    and has_all(
...
check(
    ...
    has_all(
        ...
        [
            ...
            "输入 / 输出",
            "Owner",
            "详细类型、来源矩阵、归位和候选验证见 `resource-capability-distiller/references/distillation-contract.md`",
            "创建规范见 `skill-creator`",
            "高风险操作仍须先通知、确认、再执行",
        ],
    ),
...
check(
    ...
    has_all(
        product_prd_template,
        ["参考资料与证据索引（按需）", "链接或路径", "按资料名称、链接和用途定位"],
    )
    and has_all(
        system_analysis_template,
...
check(
    ...
    and has_all(
        ...
        [
            ...
            "不得新增没有独立责任的透传包装或改名转发",
            "既有服务能在自身责任、数据与事务边界内承接时，优先复用、增强或合并重复职责",
        ],
    )
    and has_all(
        review,
...
check(
    ...
    has_all(
        ...
        [
            ...
            "只有用户明确进入命名迁移任务时才改名",
            "信息传递流程",
            "背景与问题 -> 目标与非目标 -> 定性与范围 -> 概要设计 -> 详细设计（业务场景与流程、能力对象、产品需求陈述）-> 业务规则 -> 风险与待确认",
            "完整性框架不等于可见目录",
            "描述可靠性必须可见",
            "表达结构选择",
            "图形化视图选择",
            ...
            "核心对象生命周期和禁止迁移",
            "用例图不表达先后顺序和异常分支",
            "每张图必须说明图形目标、目标读者、对应 PRD 章节、关键假设、待确认项和正文定位",
            "表格只用于比较、矩阵或追踪",
            "真实问题",
            "证据与可信度",
            "优先级口径",
            ...
            "附录按需展开",
            "精简输出规则",
            "产品结果、关键边界、异常和红线直接写入场景、需求或规则",
            "详细验收及执行证据进入已有测试质量文档或执行计划",
            "需要独立承诺时按能力逐条表达",
            "模板占位符使用 `〈...〉`",
            "每个 P0/P1 需求仍须能回到问题、目标、场景、规则和测试质量文档",
            "正式 PRD 以最终标准版本为主",
...
check(
    ...
    and has_all(
        ...
        [
            ...
            "可就近说明或按需独立展开",
            "关键概念必须说明定义、适用范围、反例、责任方和与对象/规则/测试质量文档的关系",
            "场景、需求和规则直接写清可观察结果与完成边界",
        ],
    )
    and has_all(
        product_routing,
...
check(
    ...
    has_all(
        ...
        [
            ...
            "正式 PRD 是最终交付标准版本",
            "不能只写功能名",
            "P0/P1 需求能从正文回到问题、目标、场景、能力或功能、规则和测试质量文档",
...
check(
    ...
    and has_all(
        ...
        [
            ...
            "本轮止于可继续生长的文学虚构候选",
            "显式要求会商或头脑风暴",
            "先发散，收敛前至少做一次去同构与反方复核",
            "只有作者要求选择、成形、写入故事或检验可用性时",
            "可以退回天马行空重新发散",
            "幻想归来（成真）",
            "归来目的",
            "幻想核、现象发生、触发与代价、具体事件、跨地点或时代通则",
            "异常只新增一项可观察现象",
            "不能自动鉴定文书真伪、权利归属或责任",
            "作者明确确认的内容只在对应粒度进入小说正典",
            "未确认的现象、规则和事件仍为候选",
            "故事归来",
            "人物归来",
            "世界归来",
...
check(
    ...
    and has_all(
        ...
        [
            ...
            "novelist-imagination-should-brainstorm-before-converging",
            "novelist-imagination-should-not-turn-anomaly-into-rights-oracle",
            "同一 runner/model",
            "静态 fixture 只固定预期行为",
            '"source_profiles"',
            '"id": "no-skill"',
