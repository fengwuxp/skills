...
check(
    ...
    has_all(
        ...
        [
            ...
            "目标、材料、边界、风险、验证",
            "### 2. 按交付物选入口",
            "产品语义 / 业务架构规划 / 产品判断动作链 / PRD / Backlog / 验收",
            "战略意图、业务能力、项目组合、能力-项目-系统映射和知识库回流落点",
            "系分 / 架构 / 代码 / Bug / 测试 / CR / 发布",
            "跨角色产研交付 / 目标拆解 / 任务计划 / Spec / Harness / 质量发布 / 生产交付审查 / 知识生产闭环",
            "默认进入角色协作 Loop",
            "目标层、计划层、原子执行层和执行契约是内部层",
            "角色协作阶段只是编排坐标，不是能力来源",
            "能力来源：产品 / 交互设计和验收种子回到 `产品架构专家`",
            "业务架构规划、能力地图、项目组合和知识库回流计划也回到 `产品架构专家`",
            "系分、TDD、编码、最小正确实现检查、CR、安全可靠和发布回到 `资深架构师`",
            "结构化 Java Service 生成回到 `java-service-code-generator`",
            "角色协作 Loop 的处理步骤",
            "Ask-or-Decide",
            "复杂或模糊任务一次只问一个主 blocker",
            "按你建议推进",
            "`grill` / `grilling` 有两种用法",
            "直接用 `grilling`",
            "AI Native 里的 `grilling` 盘问门禁",
            "接受建议 / 改答案 / 补材料 / 停止",
            "已确认选择、被排除方案、待确认项、下一阶段输入",
            "Auto-Decide Next",
            "Auto-Ask Owner",
            "Auto-Loop",
            "Stop-Handoff",
            ...
            "### 提示词示例",
            "可直接复制这些示例",
            "用 AI Native Engineering Loop 进入产研交付视图",
            "用 AI Native Engineering Loop 进入角色协作 Loop 问询推进",
            "用 AI Native Engineering Loop 做生产交付审查",
            "用 AI Native Engineering Loop 进入只读理解视图",
            "用产品架构专家做业务架构规划",
            "不覆盖现有文件",
            "### 最佳实践",
