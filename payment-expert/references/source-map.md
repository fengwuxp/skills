...
| `S10` | 金墨数客 2026-08-17 至 2026-08-26 发布的四篇跨境支付系列文章；2026-08-26 从公开 HTML 读取标题、作者、发布日期和共 24,007 字符正文；本机 Chrome headless 等价浏览器尝试首篇时退出码 134，故为 `read-with-limitations`；同日以 Airwallex Global Accounts / Transfer statuses 与 Stripe Bank transfers 当前官方正文交叉核对有限厂商事实 | VA / Global Account 名实评估、提交后路由承诺边界、RFI 与迟到失败的 D1 问题样例；官方资料只支持目标厂商当前页面中的有限差异 | 四篇文章不构成四个独立来源；不能支持 SWIFT/ISO 精确报文映射、VA 通用法律性质、百分比归因、跨银行静默切换、固定费率/时效/覆盖/金额阈值、RFI 自动改轨、监管合规或生产能力 |
...
- 微信公众号跨境支付系列：《跨境支付状态机设计：如何处理「没有终态」的交易》`https://mp.weixin.qq.com/s/fNXY-KK7FfLBchtVqFQb0A`、《跨境支付四种模式深度拆解：信息流与资金流》`https://mp.weixin.qq.com/s/YKEhYACuiRgoE5k4QcVdRQ`、《虚拟账户（VA）深度解析：为什么它是跨境收款的基石》`https://mp.weixin.qq.com/s/RT9tI2ViKOUKWf7tS4NMHw`、《深度解密全球支付网关架构：VA、智能路由与「收管付」的底层设计哲学》`https://mp.weixin.qq.com/s/cpn3ULjfZOUdRGL8NC4Q5w`。作者均为“金墨数客”，页面日期依次为 2026-08-26、2026-08-24、2026-08-19、2026-08-17；2026-08-26 公开 HTML 可读取完整正文，本机 Chrome headless 等价浏览器验证首篇时退出码 134，读取状态为 `read-with-limitations`。同日重新读取 Airwallex Global Accounts `https://www.airwallex.com/docs/global-treasury/get-started/global-accounts`、Airwallex Transfer statuses `https://www.airwallex.com/docs/payouts/transfers/create-a-transfer/transfer-statuses` 和 Stripe Bank transfers `https://docs.stripe.com/payments/bank-transfers` 公开正文：仅用来交叉核对 Global Account 与 Wallet 分层、目标版本 `PAID` 可迟到失败、虚拟银行账号辅助对账但仍存在多付/少付/未核销等厂商事实。系列文章只提供 VA 名实、路由承诺边界和异常处理的问题样例；精确协议、法律关系、数字指标、跨银行容灾、RFI 自动改轨及生产结论均不吸收。

- 微信公众号文章《多国 KYB/KYC 差异化校验：一套系统如何适配全球资质要求》：`https://mp.weixin.qq.com/s/Oa7zfx2uuNC8Zr_U6ecmgw`；作者 / 账号：金墨数客；页面发布时间：2026-09-14 08:00 +0800；读取日期：2026-09-14；常规网页读取失败后，通过 Codex 应用内浏览器 DOM 读取标题、作者、发布时间和正文，图片独有内容未逐图核验，状态为 `read-with-limitations`。公开内容用于参考：差异规则组合、持续复审和配置变更责任的 D1 设计候选。结合下列官方反例形成的仓库判断归入 `regulatory-baseline.md`、`payment-method-cards.md` 与 `../../product-architecture-expert/references/product-design-and-prd.md`，不代表原文逐字表述；不吸收全球固定 UBO 门槛、固定税号正则、自动冻结政策、供应商排名、报价或周期，不构成模型行为、专业批准或生产证据。
- FATF《Guidance on Beneficial Ownership of Legal Persons》，2023-03：`https://www.fatf-gafi.org/content/dam/fatf-gafi/guidance/Guidance-Beneficial-Ownership-Legal-Persons.pdf.coredownload.pdf`；2026-09-14 已读取第35–38段及第45段：用于反证所有法域采用同一持股门槛及只看持股比例的推断；不将国际指引直接写成目标机构的法定义务，适用规则与专业确认仍为 `PENDING`。
- 印尼税务总局2024-07-01公告：`https://pajak.go.id/en/node/108460`；2026-09-14 已读取正文：引入16位 NPWP，并按当时服务范围安排新旧格式过渡；只支持格式、主体与服务适用性需要区分，不证明2026年所有服务仍接受15位，当前项目兼容范围与专业确认仍为 `PENDING`。
- 巴西 COAF 2026-07-27公告：`https://www.gov.br/coaf/pt-br/sistemas/siscoaf/comunicados-siscoaf/atualizacao-do-portal-da-pessoa-obrigada-para-adequacao-ao-cnpj-em-formato-alfanumerico`；2026-09-14 已读取正文：新登记引入字母数字 CNPJ、既有数字号码继续有效；只作为旧正则不能覆盖新输入的官方反例，不生成可直接部署的校验规则，目标服务兼容范围与专业确认仍为 `PENDING`。
