...
name: payment-expert
description: |
  用户要求设计、评审或解释支付资金产品，并涉及跨主体资金责任、资金账户/支付账本、清结算对账、异步事件或支付轨道（ACH、VCC）时触发。孤立的账户、订单或退款词，以及库存、一般会计、代码、UI 和普通 PRD 不触发。
---
...
- 跨产品、工程、验证或多轮任务仍由当前 Agent 持有目标和交付责任，能力装载遵守当前消费项目实际生效的规则，不依赖 Skills 源仓库 `AGENTS.md`；本 Skill 不定义协调流程或扩大授权。
...
命中任务索引或唯一标题时，先读取完整语义章节；存在歧义、跨节依赖、资金风险语境不全或节省不足时，再扩大到父节或整文件。普通支付任务不加载 `source-map.md` 或方法卡全集。
...
| 完整收单平台、Payment API 完整性、商户入网到银行到账 | `M10` 收单平台完整性与准入门禁 | 商业模式、法域、商户责任、轨道、有效能力状态、资金映射或完成证据不全 |
...
- 任务从支付语义转为资金安全、账户接管、内部滥用、指令完整性、止损恢复或残余风险准出时，交 `security-engineering-expert` 主责安全结论；本 Skill只提供支付对象、资金责任与清结算事实。
