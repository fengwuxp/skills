...
本文把仓库级 `AGENTS.md` 的 Skill 类型接入门禁映射到产研协同体系产品到研发流程。它不新增顶层 Skill，而是帮助 `delivery-collab` 判断当前任务属于哪类能力、谁负责落地、需要什么交接物和验证证据。
...
- 需要判断是否新增 Skill、只扩展 reference、补脚本 / fixture，还是保留产研协同体系编排并路由到产品专家或架构师。
...
- Agentic Engineering 治理读 `engineering-governance.md`。
...
- 三卡交接读 `product-to-engineering-lifecycle.md` 的 `3A. 三卡交接协议`，产品侧回 `Product Context Card`，工程侧回 `Engineering Handoff Card`，持续推进回 `生产交付卡`。
...
| 代码质量、Runbook、发布 | `1. 路由矩阵`、`4. 架构侧细化`，再读架构师 reference | 不让产研协同体系做源码级判断 |
| 流程可用性 CR | `5. 回流与验证` | 不只更新说明文字 |
| 产品/产研协同体系/架构师三卡交接 | `1. 路由矩阵`、`3. 产品侧细化`、`4. 架构侧细化` | 不让任一卡替代 Execution Grant、测试通过或上线审批 |

## 1. 路由矩阵

| Skill 类型 | 产研协同体系负责 | 产品专家负责 | 架构师负责 | 典型验证证据 |
| --- | --- | --- | --- | --- |
...
4. **新建顶层 Skill**：满足以下全部条件才考虑：触发词稳定、产物独立、owner 不属于现有产品/架构/代码生成 Skill、验证脚本明确、不会和产研协同体系主入口抢职责。

已拆出的稳定能力包：

- `wind-project-coding-conventions`：项目通过 `AGENTS.md` opt-in 后，作为 Wind/Nobe Java 项目编码约规的规则权威；产研协同体系只负责流程编排，`资深架构师` 负责源码级设计、TDD、CR 和验证，`java-service-code-generator` 负责结构化代码生成。

专项 Skill 拆分先走 V 字判断：

- **向下拆清职责**：先确认它解决的是稳定能力包，而不是产研协同体系流程里的一个阶段名；输入、输出、owner、停止条件和验证证据必须能单独成立。
...
- **运行一段再抽离**：质量 / 测试门禁、Spec 治理、知识生产、产品评审、编码评审等先作为产研协同体系编排位或专家 reference 运行；只有反复独立触发、现有 owner 明显过载、验证脚本清楚时，才升为顶层 Skill。

不建议拆分的情况：

- 只是产研协同体系流程中的一个门禁位，例如质量 / 测试门禁、理解门禁、授权门禁。
- 只是产品专家或架构师已有能力的细化，例如验收种子、TDD、Runbook、发布检查。
- 只是外部文章中的工具名、hooks、marketplace 或组织实践，当前仓库没有对应运行时能力。

## 3. 产品侧细化

产品专家被产研协同体系调用时，应优先补三类能力：
...
产品专家不负责把验收种子实现成测试，也不负责判断 GSD/CAD 准入；它只把产品事实交给产研协同体系编排和架构师承接。

产品侧交接物统一收敛为 Product Context Card / 产品上下文交接卡：业务目标、非目标、owner、事实证据、核心对象、状态、不变量、流程规则、验收种子、风险、待确认和专业确认方。产品专家不生成 Engineering Handoff Card、生产交付卡、Plan Grant、Execution Grant 或 CAD Grant。

## 4. 架构侧细化

架构师被产研协同体系调用时，应优先补四类能力：
...
架构师不负责补产品语义，也不把产研协同体系的流程判断写成 Execution Grant；写入、Git、联网、生产和不可逆操作仍按仓库授权边界执行。

架构侧消费 Engineering Handoff Card 和必要时的 生产交付卡：先检查 Goal、Spec/AC、Wave/Task、写入范围、验证命令、停止条件、失败回写、授权策略、状态载体、隔离执行、Maker / Checker、独立验证、预算、人工接管和发布/回滚。可消费后才进入 OpenSpec、Harness、GSD 任务包、CAD 候选、TDD、源码级 CR 或发布风险设计。
...
- 是否把源码级职责判断归还给架构师或项目约规，产研协同体系只保留路由、回流和验证门禁。
- 是否有验证证据，而不是只有“流程更完整”的描述。
- 是否避免把 Claude Code / 外部 Skill 的内部机制写成本仓库默认能力。

## 6. Skill 治理三问

吸收 Anthropic / Claude Code Skills、Perplexity Agent Skills 或其他外部经验时，先用三问收口，避免 Skill 越改越大：

1. **循名责实**：`name`、`description`、`SKILL.md` 和 reference 是否名实相符？`description` 只写触发场景和用户会怎么说，不写长流程、能力清单或外部工具承诺。
2. **庖丁解牛**：内容是否放在最小正确层级？定位、路由和红线留在 `SKILL.md`；长清单、模板、gotchas 放 `references/`；重复、确定、容易写错的解析 / 生成 / 校验放 `scripts/`；正负触发样例放 `fixtures/`。
3. **治未病**：新增规则前是否先补可验证样例？至少覆盖一个正向触发、一个邻近 Skill 负例和一个不吸收边界；没有 fixture、validator 或 CR 证据时，只记录待确认，不把外部观点写成仓库默认能力。

阴阳平衡：阳是能力复用和自动化，阴是上下文节制、owner 边界和验证证据。只增不删会“阳强不能密”，只禁不行会“阴气乃绝”；每次改 Skill 都应同时回答“能少读什么、能少猜什么、能少做错什么”。
