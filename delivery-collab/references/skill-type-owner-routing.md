...
| 专业文档或汉字训诂任务 | `1A. 专业文档与训诂路由` | 不让文档 Skill 改写领域证据，不把单一字书当权威 |
...
| 任务 | 主 owner | 交接与边界 |
| --- | --- | --- |
| 正式报告、制度、手册、研究说明、材料合并和文档审校 | `document-authoring` | 负责专业文档撰写、引用、修订和载体检查；事实正确性仍由领域 owner 负责。 |
| PRD、产品方案和产品验收 | `产品架构专家` -> `document-authoring`（按需） | 产品专家保持语义和正文主责；文档 Skill 只在结论稳定后处理同一权威版本的结构、表达、引用和载体，完成后重新运行产品或架构交付物检查。 |
| 系分、ADR、技术方案和代码文档 | `资深架构师` -> `document-authoring`（按需） | 架构师保持工程正文主责；文档 Skill 不得改写接口、字段、状态、规则编号和验证语义，完成后重新运行产品或架构交付物检查。 |
| 汉字学与训诂、字源、甲骨文、金文、小篆、通假和异体 | `hanzi-philology` | 形、音、义、辞例、时代和来源互证，不设单一权威书；《说文解字》只作为汉代字书和传统解释材料之一。 |
| 产品名相引用古文或存在训诂争议 | `hanzi-philology` -> `产品架构专家` | 前者只是可选证据协作者，提供训诂证据卡中的命名证据；后者结合业务事实、用户理解、领域统一语言和 owner 决策确定名称。普通 API、类名、字段名不触发训诂。 |
| 训诂研究报告、DOCX 或 PDF | `hanzi-philology` -> `document-authoring` | 前者先交付训诂证据卡，后者不得改变证据等级、争议状态或补写未证结论。 |
...
- `wind-coding-conventions`：作为所有 Java 项目的唯一编码约规入口；项目声明、依赖、包名、类型或模块上下文命中 Wind/Nobe 时叠加专项规则。纯约规检查只由该 Skill 主责；`资深架构师` 单独主责源码级设计、TDD、CR、修复和验证并消费规则，`java-service-code-generator` 负责结构化代码生成。
