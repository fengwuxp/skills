...
- GSD/CAD 编排准入、GSD Round 0、Wave/Atomic Task 候选、CAD 候选缺口和 Execution Grant 缺口读 `planning-execution-admission.md`。
- Goal 组合、GSD + Goal、CAD + Goal、状态机、Ledger、预算 / 时间盒和跨轮交接读 `goal-governance.md`。
...
| 升级 SDD / Superpowers 6.x / Harness 版本 | `2. OpenSpec / Superpowers / Harness`、`2A. 交付执行契约 v3`，再读 `superpowers-skill-library.md` 和 `code-delivery.md` | 不把 v6 脚本或 `.superpowers/sdd/` 写成默认安装态 |
...
| 多 Agent / GSD 编排 | 先读 `planning-execution-admission.md`，再读 `3. Harness 最小契约`、`5. Wave 和交接` | 不直接开 CAD |
| PRD / 系分合议预审 | 先读 `prd-system-design-review.md`，再读 `2. OpenSpec / Superpowers / Harness`、`3. Harness 最小契约` | 不把预审结论写成 Execution Grant |
| CAD 候选判断 | 先读 `planning-execution-admission.md`，再读 `4. CAD 只处理原子任务`、`6. 权限边界` | 不把整个项目授权给 CAD |
...
- **authorized workspace policy**：文件化 handoff 优先使用项目已有任务文档、Goal Ledger、Harness Plan、ADR、验证矩阵或 issue；只有用户或项目规则明确授权时，才运行 `task-brief`、`review-package`、`sdd-workspace` 或创建 `.superpowers/sdd/`。

v3 的不吸收边界：

- `external-superpowers/subagent-driven-development` 已替换为 Superpowers v6.0.3 vendored skill；上游 latest 已到 v6.1.0，但本地仍以 v6.0.3 SDD helper 为执行基线。`task-brief`、`review-package`、`sdd-workspace` 只在明确进入 SDD 执行且用户授权写入工作区时运行。
- v6.1.0 的 Codex marketplace、bootstrap 压缩、SessionStart hook 移除和 Gemini CLI 支持移除只作为 source-map / 工具准入事实，不改变 Harness v3 默认执行边界。
- 不默认创建 `.superpowers/sdd/`，除非项目规则或用户明确授权并说明 gitignore、清理、恢复和隐私边界。
...
- 对任何产研协同体系编排，Harness 摘要必须说明哪些来自用户目标或源材料、哪些只是模型推断、哪些需要确认、哪些超出目标不做；不能把无根据猜测、外部文章观点、工具总结或模型脑补写成任务、实现或授权。
