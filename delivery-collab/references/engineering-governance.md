...
- `external-superpowers/subagent-driven-development/upstream-skill.md` 只保存 Superpowers v6.0.3 的本地审查基线，不作为可安装 Skill；上游版本、Codex 插件和工具支持状态在实际比较或升级时重新核验。`task-brief`、`review-package`、`sdd-workspace` 只在明确进入 SDD 执行且用户授权写入工作区时运行。
- 外部 release、marketplace、bootstrap、hook 和工具支持变化只作为带日期的 source-map 事实，不改变 Harness v3 默认执行边界。
