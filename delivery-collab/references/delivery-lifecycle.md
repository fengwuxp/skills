...
本文定义产研协同体系的交付生命周期：从业务意图、需求收集、产品 / 交互设计、设计评审、TDD / 测试设计、真实编码、编码评审、可用性 / 安全性 / 可靠性评估、验证发布到运行支持、维护退役和反馈回流。它用于跨角色交付编排，不替代产品专家 PRD、架构师系分、源码级 CR、测试实现、发布审批或生产 owner 决策。
...
| 控制不同 Skill 完成任务 | `1.1 阶段能力与约规来源 / 角色协作判断矩阵`、`3. 阶段交接门禁` | 不让产研协同体系直接代写专项产物 |
| 判断角色 / Skill / 工具 / 框架协作方式 | `1.1 阶段能力与约规来源 / 角色协作判断矩阵`、`1.1A 外部框架 / 工具协同策略` | 不让工具或框架替代主 Skill |
| 选择外部框架或工具策略 | `1.1A 外部框架 / 工具协同策略`、`1.2 角色视角顺序` | 不把工具名变成主流程或默认依赖 |
| 校准三层反馈 Loop | `1.0A 三层反馈节奏`、`1.2 角色视角顺序`，必要时读 `decision-calibration.md` | 不让内层 AI 自测替代产品判断或真实用户反馈 |
| 进入产研协同体系 | `1. 端到端阶段`、`1.1 阶段能力与约规来源 / 角色协作判断矩阵`、`1.2 角色视角顺序`、`3. 阶段交接门禁` | 不把 GSD/CAD/Goal 当主流程 |
...
SDLC 只作为生命周期覆盖框架，不替代产研协同体系，也不与 Loop 并列。审查时确认当前任务覆盖了需要负责的构想 / 发现、开发、运行 / 支持、维护 / 演进和退役阶段；采购 / 供应仅在任务实际涉及外部软件、服务或依赖时纳入。安全、权限、风险、可追溯性和知识回流横贯所有阶段，不作为发布前一次性补项。
...
轻量问询优先于长方案审查：决策澄清门禁是小闭环总门禁，`grill-me` 是命中升级条件后的升级盘问。复杂或模糊任务不要先生成大段方案让用户审；先沿设计树一次只问一个关键问题，每问给出建议答案、依据和影响。先做 Facts / Decisions 分流：Facts 用已有材料、源码、测试、日志或约规自答，Decisions 才交给 owner 等待反馈。问询结论必须写入产品上下文卡、工程交接卡、任务树 / 计划切片或下一阶段输入，不进入正式 PRD / 系分正文的讨论过程。

问询推进只处理一个主 blocker。每次问询都写清问题、建议答案、依据、影响和默认暂停点；用户接受建议后，关闭当前 blocker、写入下一阶段输入，再挑下一最高价值 blocker。

Loop 推进中适时进入 `grill-me`，问询结论必须写入产品上下文卡、工程交接卡、任务树 / 计划切片或下一阶段输入。自动触发、问题去重、shared understanding、决策快照、红线记录和执行前对账的完整规则统一读 `delivery-execution-control.md`。
...
节奏规则：先让外层反馈定方向，中层把方向翻译成 Spec，内层再执行和验证；如果只有内层绿灯，只能说明实现对当前 Spec 更接近，不能说明产品方向正确或可以发布。需要老祖宗视角时，回 `decision-calibration.md` 用“五德终始问时序、一张一弛问节奏、循名责实问证据”校准。

| 阶段 | 主责 owner | AI 角色 | 交接物 | 门禁 / 停止 |
| --- | --- | --- | --- | --- |
| 意图收集 | 业务 owner / 产品专家 | 整理事实、问题和不确定性 | 原始意图、证据、影响面、成功/失败信号 | 缺少主体、场景、证据或验收方时停止 |
| 自我挖掘与确认边界 | 产研协同体系编排，业务 / 产品 / 架构 owner 确认关键判断 | 读取现有材料、代码、测试、历史反馈和约规，形成候选需求与边界 | 自主交付控制卡、事实/推断/待确认、人工确认点 | 无来源、冲突事实、目标取舍、范围扩张或高风险动作时停止转人工 |
| 产品 / 交互设计 | 产品专家 / UED | 辅助梳理流程、用例、交互状态和验收种子 | 产品上下文卡、流程 / 用例 / 页面状态、验收种子 | 产品只传话、未解释业务问题或影响面时停止 |
| 设计评审 | 产研协同体系编排，产品 / 架构 owner 决策 | Design Reviewer / MAGI / Checker 提出挑战与风险 | ACCEPT/REJECT/PENDING、风险清单、owner | AI 预审不能替代 owner 决策 |
...
| 反馈回流 | 产研协同体系 + 对应 owner | 整理复盘、知识归位和 fixture 缺口 | Decision Log、Goal Ledger、Skill / fixture / 脚本更新建议 | 不把未经验证经验沉淀为规则 |

## 1A. 能力来源与协同矩阵

### 1.1 阶段能力与约规来源 / 角色协作判断矩阵

产研协同体系只负责调度和门禁。阶段名只是编排坐标，不是能力来源，每个阶段必须回指主 Skill；缺少能力来源时，只输出分派建议和补齐清单。本矩阵也是专项 Skill 控制矩阵：先定 Loop 节点，再定主责角色和主 Skill，工具和框架只能补一个缺口。

| Loop 节点 | 主责角色 | 主 Skill | 可选工具 / 框架 | 交接物 | 不替代 |
| --- | --- | --- | --- | --- | --- |
| 意图 / 需求澄清 | 产品 owner / 产品专家 | `产品架构专家` | `grill-me`、GStack `/office-hours`、Wisdom Lens | 产品上下文卡、问题定义、验收种子 | 不替代产品取舍或 owner 决策 |
...
| 纯 Java/Wind 约规检查 | 规则 owner | `wind-coding-conventions` | 项目本地规范、按需 Wind 示例 | 适用层级、规则偏差和建议改法 | 不进入源码设计、CR、TDD 或修复 |
| Java/Wind 源码设计、CR、TDD、修复和验证 | 架构师 | `资深架构师`，消费通用 Java 与按需 Wind 规则 | Open Code Review、Ponytail、测试与源码证据 | P 级发现、最小修复、测试和验证结论 | `wind-coding-conventions` 不成为第二 owner |
...
| 发布准出 | 发布 owner / 架构师 | 产研协同体系 + `资深架构师` | GStack `/ship`、Harness 证据链、验证发布 reference | 生产交付审查卡、生产生效验证卡、监控 / 回滚 / 人工接管证据 | 不替代 Git / PR / merge / 部署授权 |
| 知识回流 | 产研协同体系 + 对应 owner | 产研协同体系 | Trellis 思想、Context System、source-map、fixture | reference / fixture / 用户指南 / source-map 更新建议 | 不沉淀未验证经验 |
...
产研协同体系默认按下列视角切换。当前任务不涉及的视角可以跳过，但不能把 Maker 和 Checker 合并成自证。
...
GStack 的 `/office-hours`、`/plan-ceo-review`、`/plan-eng-review`、`/plan-design-review`、开发、`/review`、`/qa`、`/ship` 只作为角色链审查模板接入产研协同体系。它解决“同一任务要被不同角色视角连续挑战”的问题，不新增对外主流程、不要求用户记命令，也不替代产品专家、资深架构师、质量门禁或发布 owner。

GStack 的价值不是命令名，而是连续逼问：先问为什么做，再砍范围，再审工程和交互，最后用独立 CR、QA 和发布证据准出。

| GStack 模板段 | 产研协同体系阶段 | 主责与能力来源 | 必须回答的问题 | 准出 / 停止 |
| --- | --- | --- | --- | --- |
...
| `/plan-ceo-review` 范围收敛 | 产品方案 / 目标层 Review | 产研协同体系编排 + 产品 owner | 是否过大、是否可逆、MVP 是否足够、非目标和成功指标是否清楚？ | 业务取舍或优先级不明时回 owner。 |
...
| `/qa` QA 验证 | 质量 / 测试门禁 | 产研协同体系质量门禁 + 资深架构师 testing 能力 | 正常、边界、失败、回归、安全、可靠性验证是否足够？ | 失败用例、残余风险或不可复现结果必须回写。 |
...
使用规则：命令名只作触发别名；若用户只给一个阶段，就只输出该阶段的 owner、交接物、验证与停止条件。若用户给完整 GStack 模板，按产研协同体系输出阶段链路和下一 owner，不复制外部命令体系。
...
- **产研协同体系编排者**：决定当前阶段、owner、交接物、验证门禁和停止条件；不替代 PRD、系分、代码、测试、CR 或发布审批。
