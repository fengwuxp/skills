...
本文定义产研协同体系中的受控执行方式。Loop 不是更长的 Prompt，也不是无条件自动执行；它只是围绕目标、状态、计划、动作、观察、判断、验证、预算、停止和人工接管组织起来的可审执行机制。

核心纲要：**Build the loop, stay the engineer**。混一总纲：以真实交付为体，以证据闭环为用；阴定边界，阳促推进，随事取最小可验证路径。体不清则不动，用无证则不收，阴不足则停，阳不足则问；神用无方就是按上下文取 Goal / Spec / Harness / TDD / CR / 发布 / 回流中的最小有效组合，不把它们写成并列菜单。产研协同体系工程师的高杠杆工作仍是设计能读取状态、调用 Agent、检查结果、决定下一步、保存记忆并在关键边界停下来的工程循环。

对外只推荐一个主入口：**产研协同体系**。它按交付编排、工程治理和知识演进组织能力；只读理解、产研交付、验证发布和知识回流只是场景视图，不是并列主流程。GSD、CAD、Goal、Harness 和 Grant 不再作为同级模式暴露，而是受控执行的内部映射：
...
- 需要判断某个自动化线程、定时任务、监控、Review Agent 或多 Agent 编排是否可以进入产研协同体系工作流。
...
- 用户希望减少 GSD、CAD、Goal 多模式选择成本，把它们统一纳入产研协同体系，并按只读理解、产研交付、验证发布或知识回流视图处理。
...
默认只产出一份最小 Loop 准入结论：不进入产研协同体系 / 进入只读理解视图 / 进入产研交付视图 / 进入验证发布视图 / 进入知识回流视图 / 必须人工主导；同时说明 owner、授权、验证和停止条件。
...
- GSD/CAD 内部层、Wave、Atomic Task、Plan Grant 和授权边界读 `planning-execution-admission.md`。
- Goal 目标层、状态、预算 / 时间盒、Ledger 和 GSD / CAD / Spec 关联读 `goal-governance.md`。
...
| 统一 GSD / CAD / Goal 为 Loop | `3. 内部层映射`，再读 `goal-governance.md` 和 `planning-execution-admission.md` | 不把内部层重新暴露成多个入口 |
...
生产可用 Loop 不是“少写 Prompt”，而是把一个可复现、可验证、可接管的小型工程系统装到产研协同体系流程里。可迁移的结构是：自动化提供心跳，隔离工作区承载并发执行，Skill 注入项目上下文，连接器接入真实工具链，Maker / Checker 解耦交付与验证，状态载体落盘保存进度和断点。缺任一关键件时，Loop 只能按候选或只读模式处理。
...
自我问询 / 自我回答 / `grill-me` 盘问: 触发原因 / 当前分支 / question_id / 已问问题 / 答案状态 / 问题 / 推荐答案 / 已自答证据 / 模糊回答 push back / 退出条件:
...
本节是 `grill-me` 盘问、问题去重、决策快照和执行前对账的唯一权威来源。`grill-me` 只在 Loop 推进受阻时升级决策澄清门禁。自动触发只看四类信号：关键分叉会改变业务目标、公共契约、状态机、验收样例、写入范围或发布风险；用户回答含糊；用户半答；同一方向连续返工。长链路、结果分叉多、返工成本高，或人的 Taste / 偏好 / 红线会影响方案时，也先把隐性判断转成可执行决策和可交接资产。显式提到 `grill-me` 触发不稳定、角色 Loop 自动触发或问询推进时，先审计这些信号。
命中后先做 Facts / Decisions 分流：Facts 用代码库、文档、测试和日志证据自答，Decisions 才沿一个决策树分支深度优先盘问。首轮只输出一张 `grill-me` 盘问卡，包含触发原因、当前分支、`question_id`、一个问题、推荐答案、依据和默认暂停点；每问必须等待 owner 反馈。下一问前检查已问问题和答案状态；候选问题若和已问问题同义，只能关闭当前 blocker、追问缺失项或对模糊回答 push back，不得重问原问题。
退出条件不是问完所有问题，而是形成决策快照：已确认选择、被排除方案、待确认项、下一阶段输入和写回位置；写回 Product Context、Engineering Handoff、Shared Context / Issue / PR / hand-off prompt 或项目允许的状态载体。`grill-me` 退出只形成 shared understanding 候选；owner 确认或完成执行前对账前，不进入执行、写文件或生成计划。执行前必须说明本轮依据哪条已确认选择，是否触碰被排除方案或待确认项；被排除方案不得复活，待确认项不得脑补，不一致时停止并问 owner。红线、底线、不能碰、不可、禁止、必须等字眼必须记录到决策快照或待确认项。决策快照进入下一阶段前再做领域知识分流：只有 owner 已确认，且能回链正式材料、代码或测试证据的领域事实，才进入知识回流；术语与已有知识或代码语义不一致时，先记录冲突证据与影响范围并停在待确认，不把当前代码自动认定为正确业务事实。确认后按业务域或模块写入术语与对象表、证据地图或领域知识卡。只有工程决定同时难以逆转、缺少背景会令人困惑、且源于真实方案取舍时，才路由 `资深架构师` 的 `adr-and-tradeoff.md`；其余保留在决策快照、现有产品文档或交接卡。未获写入授权时只输出候选回流位置，不创建 `CONTEXT.md`、ADR 或知识库目录。
不得把 `grill-me` 变成无休止访谈、替代 owner 决策、替代测试 / CR / 发布准出，或把问询过程写进正式 PRD / 系分正文。
## 3. 内部层映射

目标层、计划层、原子执行层、执行契约和授权边界统一归入产研协同体系的产研交付视图；默认输出不展示内部模式名，只展示当前阶段、owner、交接物、授权策略、验证与停止条件。
...
- 对外推荐使用“进入产研协同体系”“进入产研交付视图”“按目标计划推进”“做目标计划 / 原子执行准入”；输出应显示为产研协同体系的产研交付视图，而不是要求用户在多模式之间选择。
...
- 如果只是产研协同体系流程中的一个执行循环，留在本 reference。
