...
3. **先跑自我问询**：把问题拆成已知事实、可自答问题、需 owner 判断的问题；能凭证据自答且低风险的，直接进入设计、落地或验证下一环。
...
- **治理与回流**：Loop 取舍校准 / Wisdom Lens、Skill 类型路由、知识回流、Skill 自我改进外循环和 source-map；读 `wisdom-loop-lens.md`、`skill-type-owner-routing.md`、`superpowers-skill-library.md` 和 `source-map.md`。`huaxia-wisdom` 只做取舍校准，不替代事实、证据、测试、CR、授权或上线审批。
...
- Engineering Loop Contract、小闭环 Ask-or-Decide Card、自主交付控制卡、Knowledge-to-Execution Card、非标问题处理包、Coding Loop Contract、Verification Cluster Gate。
