...
8. **任务结束责任闭环**：每个任务或小闭环结束后，先做交付责任自检，再给 Ask-or-Decide 结论；如果可以继续，只给下一任务计划问询或最小计划草案，不把续作写成已授权；用户说“按你建议推进”时，先关闭当前 blocker，写入下一阶段输入，再挑下一最高价值 blocker，不重开全局规划。
