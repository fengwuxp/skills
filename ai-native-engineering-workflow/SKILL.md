...
description: |
  用户要求用 AI Native Engineering Loop 编排跨角色产研交付、角色 Loop 问询推进或问询修复、grilling 盘问门禁、Loop Engineering、可复用工作资产、协作门禁、授权边界、验证交接和停止条件时触发。普通 PRD、代码修改、测试评审或 Java Service 生成优先路由专门 Skill。
---
...
4. **轻量问询先于长方案，最小交付优先**：先确认用户、主体、目标、证据、失败成本、边界和验收种子；问决门禁是小闭环总门禁，`grilling` 是命中升级条件后的升级盘问；复杂或模糊任务一次只问一个问题，能从材料、源码或测试自答的问题先自答，普通问询不带 `question_id`、决策快照和执行前对账等加压字段。Loop 推进中遇到关键分叉未决、回答含糊、方案依赖链没走到底或连续返工时，进入 `grilling` 式盘问：目标是把人的 Taste / 偏好 / 红线 / 关键分叉显式化为可执行决策和可交接资产；沿决策树深挖一个分支、给推荐答案，每问记录 `question_id`、当前分支、已问问题和答案状态；下一问先查已问问题，命中同义重复问题时关闭当前 blocker、只追问缺失项或 push back 模糊点，不重问原问题；每问等待 owner 反馈或证据自答后再进入下一问，并把决策记录写入下一阶段输入。`grilling` 退出后形成 grilling 决策快照，只保留已确认选择、被排除方案、待确认项、下一阶段输入和写回位置；执行 / 写文件 / 生成计划前必须先做执行前对账，说明本轮执行依据哪条已确认选择，是否触碰被排除方案或待确认项；被排除方案不得复活，待确认项不得脑补，不一致时停止并问 owner；问询中出现红线、底线、不能碰、不可、禁止、必须等字眼时重点关注并记录。不知道就问，目标清楚时没要求的不写，只改被要求的范围；目标清楚但路径不是最短时，直接说明更短路径。Loop 输出优先给验收标准、验证结果、停止条件和下一 owner。
...
- **角色协作 Loop 编排**：用户要求把产品 / 交互设计、设计评审、TDD / 测试设计、编码实现、编码评审、可用性 / 安全性 / 可靠性评估、验证发布和复盘回流串成端到端交付闭环，或要求“混一 AI 工作流”“一个入口 / 一个契约 / 一个准出”“自主交付闭环”“角色 Loop 问询推进 / 问询修复”“grilling 触发不稳定 / 自动触发”。
...
3. **先跑问决门禁**：把问题拆成已知事实、关键分叉、可自答问题和需 owner 判断的问题；问决门禁是小闭环总门禁，`grilling` 是命中升级条件后的升级盘问；复杂 / 模糊任务一次只问一个主 blocker，并给出建议答案、依据、影响和默认暂停点；显式提到 `grilling` 触发不稳定、角色 Loop 自动触发或问询推进时，先按 `grilling` 盘问门禁审计；命中时首轮只输出一张 `grilling` 盘问卡：触发原因、当前分支、`question_id`、一个问题、推荐答案、依据和默认暂停点，不列第二个待答问题、完整方案或执行清单；下一问必须先检查已问问题和答案状态，遇到同义重复问题时只追问缺失项、关闭当前 blocker 或 push back 模糊点，不重问原问题；Loop 推进中如发现当前 blocker 的依赖问题没走到底，切入 `grilling` 盘问卡，直到得到可写入下一阶段输入的决策记录、被排除方案和待确认项；能凭证据自答且低风险的，直接进入设计、落地或验证下一环。
...
7. 不把外部 Superpowers skill、Matt Pocock skills / `grilling`、Ponytail、Open Code Review、WorkBuddy、Karpathy-style Guidelines、Gemini CLI、AgentRC、Understand Anything、华夏经世智慧、`huaxia-wisdom` 或其他工具 / 判断框架写成默认安装、默认联网、默认写文件、默认 Git 操作、默认审批、项目事实、工程证据、CR 结论或生产授权。
