...
Skill 自我改进外循环遵循仓库 `AGENTS.md`。本技能只编排基于执行记录、验证结果、CR 结论和人工反馈的最小 Skill 改进；不保存个人长期偏好或私有轨迹，不自动读取历史对话或私有目录，不自动提交、同步或发布。
...
2. **Loop 自主推进统一外显，旧术语内部化**：对外只呈现 Loop 阶段、Ask-or-Decide、owner、验证和停止条件；Goal、GSD、CAD、Harness、Grant 只作内部实现层。
...
9. **授权、角色和理解分离**：Loop 只能在已声明的低风险本地授权范围内推进；越权、验证失败或高风险业务必须停下。每个小闭环完成后先用已读证据自我问询和回答；证据足够则写入下一阶段输入，仍缺 owner 责任判断时只问 1-3 个关键问题。设计、实现、检查、准出必须区分主责、协作、AI Maker、AI Checker 和人工 owner；人必须能解释目标、当前状态、关键变更、证据链、残余风险和停止理由。
...
- **入口别名**：用户说“进入 AI Native 交付 Loop”“只读理解 Loop”“验证发布 Loop”“知识回流 Loop”“GSD 产研协同研发流程”“GSD + Goal”“先计划再创建 Goal”“按任务计划推进”“三卡交接”“做 GSD/CAD 准入”时，统一收敛到角色协作 Loop；旧术语只作触发别名，不在默认输出中展示为外部流程名。
- **专项门禁**：轻量问询 / Grilling、任务树真相源、PRD / 系分预审、MAGI 合议、AI 预扫描、Spec/SDD 模板、代码交付闭环、质量 / 测试门禁、代码库理解、设计-代码对齐、最小正确实现 / 过度设计 CR、工具准入、Wisdom Lens、知识表达、非标问题、实际项目编码 Loop、反馈闭环成熟度、验证簇、架构排熵 Loop 或腐朽门禁。
- **SDD / Harness 升级**：用户要求升级 SDD/SSD 套件、Superpowers 6.0、Harness 版本、Task Reviewer、文件化交接或 progress ledger 时，先读 `superpowers-skill-library.md`、`agentic-engineering-governance.md` 和 `code-delivery-closed-loop.md`，只吸收方法契约，不默认运行外部脚本。
...
- Superpowers、Matt Pocock skills / Grilling、Ponytail、Gemini CLI、AgentRC、Understand Anything 或其他外部工具：本技能只做准入、权限、联网/认证/写入边界和交接格式判断；不自动安装、联网、写配置、运行外部脚本或采用外部 Git 默认动作。

## 运行时流程

1. **识别任务层级**：流程设计、产品发现、工程交接、设计评审、TDD、AI 编码执行、编码评审、安全 / 可用性评估、验证发布、知识回流或组织治理。
2. **默认进入角色协作 Loop**：按设计、设计评审、TDD / 测试设计、编码实现、编码评审、可用性 / 安全性 / 可靠性评估、验证发布、复盘回流推进；旧 GSD/CAD/Goal 诉求映射为内部目标层、计划层或原子执行层，默认不外显模式名。
3. **先跑轻量问询**：把问题拆成已知事实、关键分叉、可自答问题和需 owner 判断的问题；复杂 / 模糊任务一次只问一个问题，并给出建议答案、依据和影响；能凭证据自答且低风险的，直接进入设计、落地或验证下一环。
4. **再定自主推进边界**：输出自主交付控制卡，区分可自我挖掘、可自我规划、可自动执行、必须人工确认和当前置信度。
5. **选择最小参考集**：按任务读取 `references/`，不一次加载所有方法论。
6. **路由协同 Skill**：产品语义不足时回 `产品架构专家`；工程边界和代码风险不足时回 `资深架构师`。
7. **输出流程产物**：给出当前阶段、角色视角、能力 / 约规来源、Loop Contract、门禁、验证和停止条件。
8. **保留证据边界**：区分事实 / 推断 / 待确认 / 范围外不做；证据不足时只输出补齐清单，不扩写任务或实现。
...
内部实现层：默认不对外展开；需要审计时只写目标层 / 计划层 / 原子执行层 / 执行契约 / 授权边界
授权策略：只读 / 计划内低风险执行 / 受控执行授权 / 显式确认
验证门禁：
停止条件：
残余风险 / 需要确认：
```
...
- **只读理解视图**：用于阅读分析代码库、设计-代码对齐、外部工具准入、事实边界检查和影响范围识别；Gemini CLI 可用且读取范围获授权时，优先只读启动；默认不写文件、不安装、不生成执行任务。
...
- **角色协作主流程**：从意图 / 需求收集到生产交付，或按轻量问询、自我挖掘、确认、设计、需求 CR、TDD、编码、CR、可用性安全性评估推进；读 `intent-to-production-loop.md`，输出 Role Collaboration Loop Map，并写明每阶段能力 / 约规来源。
- **大项目 Loop 编排**：目标层、计划切片、原子执行、授权策略；读 `goal-composition.md`、`gsd-cad-admission.md` 和 `agentic-engineering-governance.md`，输出目标组合包、Loop 准入或执行授权缺口。
...
- Engineering Loop Contract、小闭环 Ask-or-Decide Card、轻量问询卡、自主交付控制卡、Knowledge-to-Execution Card、非标问题处理包、Coding Loop Contract、Verification Cluster Gate。
...
7. 不把外部 Superpowers skill、Matt Pocock skills、Grilling、Ponytail、Karpathy-style Guidelines、Gemini CLI、AgentRC、Understand Anything、`huaxia-wisdom` 或其他工具 / 判断框架写成默认安装、默认联网、默认写文件、默认 Git 操作、默认审批、项目事实、工程证据或生产授权。
