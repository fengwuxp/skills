...
2. **编排**：定义当前阶段、角色视角、AI Maker / Checker、Goal、State、Plan、Action、Observation、Decision、Self-Question、Self-Answer、Verification、Stop/Handoff、owner、授权策略和交接物。
...
- 必要时只输出 Engineering Loop Contract、Loop 推进卡、目标层摘要、计划切片建议、原子执行缺口、授权边界卡、验证矩阵草案、Engineering Handoff Card 或 Production Loop Card。
...
7. **授权、角色和理解分离**：Loop 只能在已声明的低风险本地授权范围内推进；越权、验证失败或高风险业务必须停下。每个小闭环完成后先自我问询，用已读证据自我回答；证据足够则自动写入下一阶段输入，仍缺 owner 责任判断时只问 1-3 个关键问题，越权则停下。设计、实现、检查、准出必须区分主责、协作、AI Maker、AI Checker 和人工 owner；人必须能解释目标、当前状态、关键变更、证据链、残余风险和停止理由。
...
7. **输出流程产物**：给出当前阶段、角色视角、能力 / 约规来源、Goal、State、Plan、Action、Observe、Decide、Ask-or-Decide、Verify、Stop-Handoff、门禁、验证和停止条件。
...
唯一外显主入口是 **角色协作 Loop**。下面四类不是并列流程，而是角色协作 Loop 的场景视图，用来判断当前材料成熟度、处理深度、写入边界和验证要求；旧的 GSD、CAD、Goal 说法只作为触发别名和内部实现层，默认输出不展示旧流程名：
...
- `references/agentic-engineering-governance.md`：OpenSpec、Superpowers、Harness、AI Native Harness Contract v2、权限边界、多 Agent 协作和事实边界。
...
7. 不把外部 Superpowers skill、Matt Pocock skills、Grilling、Ponytail、Gemini CLI、AgentRC、Understand Anything、`huaxia-wisdom` 或其他工具 / 判断框架写成默认安装、默认联网、默认写文件、默认 Git 操作、默认审批、项目事实、工程证据或生产授权。
