...
- Gemini CLI、AgentRC、Understand Anything 或同类 AI 代码理解 / 上下文工程 / 知识图谱工具由本技能先做安装与调用准入：判断何时可用、是否只读、是否联网/认证/写文件、输出如何回链源码和验证；具体源码级判断、测试和代码修改仍交给 `资深架构师`。
...
- 问题核心诊断由本技能编排：当输入混有情绪、方案、工具、文章观点或战术动作时，先判断核心问题、症状证据、系统结构、可验证实验和停止条件；产品专家负责把需求病机收敛为产品定义，架构师负责把系统病机落到边界、接口、数据、测试和发布风险。
...
12. **先定核心再定流程**：不要用流程、工具、模型、模板或执行勤奋掩盖问题未定；先抓住核心矛盾、业务事实、系统病机和验证路径，再决定进入 PRD、OpenSpec、GSD、CAD、Loop 或工具准入。
13. **顶层原则先行**：当用户要求“提炼原则”“处事方式”“方法论沉淀”或外部文章吸收时，先判断哪些应上移到仓库级 `AGENTS.md`，哪些只应留在本技能或对应 `references/`；不把一次性启发写成跨技能硬规则。

## 使用时机

- 用户要求设计或优化 AI 时代产品、研发、编码、测试、CR、发布或复盘流程。
- 用户要求提炼顶层原则、处事方式、方法论纲要、跨 Skill 共同约束，或希望把外部文章经验上升为仓库级规则。
...
- 用户说“阅读 / 分析代码”时，只有目标是代码库级理解、设计-代码对齐、AI 生成变更影响、上下文工程、知识图谱或 Gemini CLI / AgentRC / Understand Anything 准入，才进入本技能；只读某段代码、具体 CR、Bug、测试或实现问题优先交给 `资深架构师`。
- 用户要求安装、评估或调用 Gemini CLI、AgentRC、Understand Anything、AI 快速阅读代码工具、上下文工程工具、代码库知识图谱工具，或要求把代码阅读结论、agent instructions、eval、MCP 配置、AI-readiness 和上下文漂移纳入流程。
...
- 用户要求抓住问题核心、判断需求是否无止境、区分价值与意义、做定向/定性/定位/定量、或用整体/系统/科学三层视角审查 AI 流程、产品方案、系分或 GSD/CAD 计划。
...
- Gemini CLI / AgentRC / Understand Anything 安装、调用或工具辅助代码阅读可以从本技能进入，但本技能只判断安装准入、权限边界、只读/写入范围、隐私/联网/认证要求和工具输出交接；实际安装、调用、源码判断和生成物采纳必须经用户授权并交给合适 owner。
...
- 只要求阅读/分析某个文件、函数、类、报错、测试失败或 PR diff 时，不把 Gemini CLI / AgentRC / Understand Anything 作为默认入口；除非用户明确要求工具准入、代码库级理解、知识图谱或团队共享结构视图，否则直接交给 `资深架构师`。
...
- **问题核心诊断模式**：用户输入混有情绪、方案、工具、文章观点或战术动作时，先输出核心问题、症状证据、整体边界、系统结构、可验证实验、owner 和停止条件；证据不足时停在 Round 0，不把战术动作包装成产研任务。
...
- **工具准入模式**：用户点名 Gemini CLI、AgentRC、Understand Anything 或类似代码理解 / 上下文工程 / 知识图谱工具，或要求代码库级阅读 / 分析、设计-代码对齐、AI-readiness、团队共享图谱时，先输出安装与调用准入、只读范围、禁止事项、工具输出交接格式和人工替代路径。
...
- `references/code-understanding-tools.md`：Gemini CLI、AgentRC、Understand Anything 等 AI 代码理解 / 上下文工程 / 知识图谱工具的触发入口、安装准入、只读/写入边界、设计-代码对齐和工具输出交接。
...
- AI 代码理解工具准入包：Gemini CLI / AgentRC / Understand Anything 是否值得安装或调用、只读范围、联网/认证/写入边界、输出格式、人工替代路径和交接 owner。
