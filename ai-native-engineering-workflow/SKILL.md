...
- **AI 编码框架分层**：用户要求比较或吸收 Superpowers、GSD、GStack、Trellis 等框架时，只判断方法纪律、上下文状态、角色链审查和仓库级记忆如何纳入角色协作 Loop，不把外部框架变成并列流程或默认依赖。
- **GStack 角色链模板**：用户提到 `/office-hours`、CEO Review、Eng Review、Design Review、`/review`、`/qa`、`/ship` 等模板时，只映射为产品思考、范围收敛、工程评审、交互评审、编码实现、源码 CR、QA 验证和发布准出；命令名只是触发别名，不作为外部工具、默认命令菜单或新流程。
- **生产交付审查**：用户要求“生产交付审查”“发布前评审”“能不能上线 / 交付生产”或 `/ship` 时，进入验证发布视图，输出生产交付审查卡；只判断价值、范围、设计、实现、CR、QA、观测、回滚、人工接管和下一 owner，不替代测试通过、Git/PR/merge/部署或上线审批。
- **生产可用混一模型**：用户要求“混一 AI 工作流”“统一 AI 流程”“一个入口 / 一个契约 / 一个准出”或提升生产可用性时，只把多套模式收成角色协作 Loop、Engineering Loop Contract 和生产可用准出卡；不新增流程名、命令菜单或工具依赖。
...
- **推荐提示语**：优先使用“进入角色协作 Loop”“进入产研交付视图”“进入只读理解视图”“做生产交付审查”“做源码质量评审”“进入知识回流视图”；内部目标、计划、原子执行和授权边界由 Loop 自主归位，不展开模式菜单。
