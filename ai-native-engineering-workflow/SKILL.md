...
description: |
  用户要求用 AI Native Engineering Loop 编排跨角色产研交付、角色 Loop 问询推进或问询修复、grill-me 盘问门禁、Loop Engineering、可复用工作资产、协作门禁、授权边界、验证交接和停止条件时触发。普通 PRD、代码修改、测试评审或 Java Service 生成优先路由专门 Skill。
---
...
Goal、GSD、CAD、Harness、Grant 等术语只作为角色协作 Loop 的内部实现层；对外只呈现 Loop 自主推进、问决门禁、owner、验证和停止条件。需要审计内部实现时，用“目标层 / 计划层 / 原子执行层 / 执行契约 / 授权边界”表述，不要求用户在多个模式间选择。
...
2. **Loop 自主推进统一外显，内部层只做支撑**：对外只呈现角色协作 Loop、场景视图、问决门禁、owner、验证和停止条件；目标、计划、原子执行、执行契约和授权边界只在审计内部层时出现。
...
4. **轻量问询先于长方案，最小交付优先**：先确认用户、主体、目标、证据、失败成本、边界和验收种子；问决门禁是小闭环总门禁，`grill-me` 是命中升级条件后的升级盘问；复杂或模糊任务一次只问一个问题，先区分 Facts / Decisions：Facts 用材料、源码、测试和日志自答，Decisions 才问 owner，普通问询不带 `question_id`、决策快照和执行前对账等加压字段。Loop 推进中遇到关键分叉未决、回答含糊、方案依赖链没走到底或连续返工时，进入 `grill-me` 式盘问：目标是把人的 Taste / 偏好 / 红线 / 关键分叉显式化为可执行决策和可交接资产；沿决策树深挖一个分支、给推荐答案，每问记录 `question_id`、当前分支、已问问题和答案状态；下一问先查已问问题，命中同义重复问题时关闭当前 blocker、只追问缺失项或 push back 模糊点，不重问原问题；每问等待 owner 反馈或证据自答后再进入下一问，并把决策记录写入下一阶段输入。`grill-me` 退出只形成 shared understanding 候选；owner 确认达成 shared understanding 或完成执行前对账前，不进入执行、写文件或生成计划。`grill-me` 退出后形成 grill-me 决策快照，只保留已确认选择、被排除方案、待确认项、下一阶段输入和写回位置；执行 / 写文件 / 生成计划前必须先做执行前对账，说明本轮执行依据哪条已确认选择，是否触碰被排除方案或待确认项；被排除方案不得复活，待确认项不得脑补，不一致时停止并问 owner；问询中出现红线、底线、不能碰、不可、禁止、必须等字眼时重点关注并记录。不知道就问，目标清楚时没要求的不写，只改被要求的范围；目标清楚但路径不是最短时，直接说明更短路径。Loop 输出优先给验收标准、验证结果、停止条件和下一 owner。
...
7. **授权、角色和理解分离**：Loop 只能在已声明的低风险本地授权范围内推进；越权、验证失败或高风险业务必须停下。每个小闭环统一走问决门禁；任务结束必须先做任务结束责任闭环和交付责任自检，继续推进时给出下一任务计划问询或最小计划草案。设计、实现、检查、准出必须区分主责、协作、AI Maker、AI Checker 和人工 owner；人必须能解释目标、当前状态、关键变更、证据链、残余风险和停止理由。
...
- **外部框架或工具吸收**：用户要求参考 Loop Engineering、CDD / Capability-Driven Development、Prompt Engineering 转 Loop、Superpowers、GSD、GStack 角色链模板、Trellis、AI 代码交付闭环、Spec / SDD / OpenSpec、渐进式 SDD / Harness 规范化、Matt Pocock skills / `grill-me`、Ponytail、Open Code Review、WorkBuddy、Gemini CLI、AgentRC、Understand Anything、Wisdom Lens、华夏经世智慧、`huaxia-wisdom` 或老祖宗智慧时，只把方法纪律、上下文状态、角色链审查、仓库级记忆、独立裁判、轻量问询、阶段性盘问和工具准入纳入分角色 Loop；读 `references/agent-loop-engineering.md`、`references/code-delivery-closed-loop.md`、`references/wisdom-loop-lens.md` 等最小 reference；对 `grill-me` 盘问门禁只吸收“决策树深挖、一次一问、推荐答案、等待反馈、Facts 自查、Decisions 等 owner、shared understanding 确认、模糊回答 push back、结构化决策摘要”；对可复用工作资产只吸收触发器、动作、证据、记忆和停止条件；对实现前能力发现只吸收“非 T0 写入 / 交付任务先查已有 Skill / Tool / Workflow / 脚本 / 自动化和验收证据”；对渐进式 SDD 只吸收强度分级、独立裁判、证据链、漂移检查和失败回流；周易 / 道德经 / 庄子 / 论语 / 中医 / 五德终始类取舍校准只落为工程反偏问题，周易、道德经、庄子、论语、中医、五德终始不替代事实、证据、测试、CR、授权或上线审批；不新增并列流程、命令菜单、默认安装，不默认运行外部脚本。
...
3. **先跑问决门禁**：把问题拆成已知事实、关键分叉、可自答问题和需 owner 判断的问题；问决门禁是小闭环总门禁，`grill-me` 是命中升级条件后的升级盘问；复杂 / 模糊任务一次只问一个主 blocker，并给出建议答案、依据、影响和默认暂停点；显式提到 `grill-me` 触发不稳定、`grill-me`、角色 Loop 自动触发或问询推进时，先按 `grill-me` 盘问门禁审计；命中时先做 Facts / Decisions 分流，Facts 用代码库、文档、测试、日志自答，Decisions 才进入首轮 `grill-me` 盘问卡：触发原因、当前分支、`question_id`、一个问题、推荐答案、依据和默认暂停点，不列第二个待答问题、完整方案或执行清单；下一问必须先检查已问问题和答案状态，遇到同义重复问题时只追问缺失项、关闭当前 blocker 或 push back 模糊点，不重问原问题；Loop 推进中如发现当前 blocker 的依赖问题没走到底，切入 `grill-me` 盘问卡，直到得到可写入下一阶段输入的决策记录、被排除方案和待确认项；未达成 shared understanding 或未完成执行前对账，不进入执行、写文件或生成计划；能凭证据自答且低风险的，直接进入设计、落地或验证下一环。
...
8. **任务结束责任闭环**：每个任务或小闭环结束后，先做交付责任自检，再给出问决门禁结论；如果可以继续，只给下一任务计划问询或最小计划草案，不把续作写成已授权；用户说“按你建议推进”时，先关闭当前 blocker，写入下一阶段输入，再挑下一最高价值 blocker，不重开全局规划。
...
混一口径：角色协作 Loop 是唯一入口；一言以蔽之，以真实交付为体，以证据闭环为用；阴定边界，阳促推进，随事取最小可验证路径。Engineering Loop Contract 承接执行，生产可用准出卡只看真实入口、证据、owner、回滚和回流；GSD、CAD、GStack、Goal、Harness 和工具链只在内部归位。
...
- **产品到工程 / 角色主流程**：读 `product-to-engineering-lifecycle.md` 或 `intent-to-production-loop.md`，只输出补齐清单、业务架构规划路由、产品判断动作链准入、阶段 owner、交接物、角色协作判断矩阵和能力来源。
...
- `references/agent-loop-engineering.md`：统一 Loop Contract、四类角色 Loop 场景视图、CDD 能力发现门禁、可复用工作资产 Loop、实际项目编码 Loop、反馈闭环成熟度、验证簇准入、架构排熵 Loop / 腐朽门禁、`/goal`、`/loop`、auto mode、后台 Agent、多 Agent 监督、生产可用 Loop 门禁和 Skill 复用单位。
- `references/wisdom-loop-lens.md`：华夏经世智慧、huaxia-wisdom、Loop 取舍校准、老祖宗智慧和东方判断层触发别名、周易、道德经、庄子、论语、中医、五德终始、阴阳平衡、先为不可胜、庖丁解牛、治未病、中庸之道、循名责实、无为而治、每日三省、知行合一和一张一弛在 Loop 准入、拆解、执行核验、AI bug / 补丁门禁、授权纠偏和复盘回流中的工程化映射。
...
- **收口卡**：交付责任自检、问决门禁结论、下一任务计划问询或停止交接。
