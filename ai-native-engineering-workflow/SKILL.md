...
description: |
  用户要求用 AI Native Engineering Loop 编排跨角色产研交付、Loop Engineering、可复用工作资产、协作门禁、授权边界、验证交接和停止条件时触发。普通 PRD、代码修改、测试评审或 Java Service 生成优先路由专门 Skill。
---
...
4. **轻量问询先于长方案，最小交付优先**：先确认用户、主体、目标、证据、失败成本、边界和验收种子；复杂或模糊任务一次只问一个问题，能从材料、源码或测试自答的问题先自答。Loop 推进中遇到关键分叉未决、回答含糊、方案依赖链没走到底或连续返工时，进入 Grill-Me 式盘问：沿决策树深挖一个分支、给推荐答案、push back 模糊回答，并把决策记录写入下一阶段输入。不知道就问，目标清楚时没要求的不写，只改被要求的范围；目标清楚但路径不是最短时，直接说明更短路径。Loop 输出优先给验收标准、验证结果、停止条件和下一 owner。
...
- **外部框架或工具吸收**：用户要求参考 Loop Engineering、CDD / Capability-Driven Development、Prompt Engineering 转 Loop、Superpowers、GSD、GStack 角色链模板、Trellis、AI 代码交付闭环、Spec / SDD / OpenSpec、渐进式 SDD / Harness 规范化、Matt Pocock skills / Grilling / Grill-Me、Ponytail、Open Code Review、WorkBuddy、Gemini CLI、AgentRC、Understand Anything、Wisdom Lens 或 `huaxia-wisdom` 时，只把方法纪律、上下文状态、角色链审查、仓库级记忆、独立裁判、轻量问询、阶段性盘问和工具准入纳入分角色 Loop；读 `references/agent-loop-engineering.md`、`references/code-delivery-closed-loop.md`、`references/wisdom-loop-lens.md` 等最小 reference；对 Grill-Me 只吸收“决策树深挖、一次一问、推荐答案、能自答先自答、模糊回答 push back、结构化决策摘要”；对可复用工作资产只吸收触发器、动作、证据、记忆和停止条件；对实现前能力发现只吸收“非 T0 写入 / 交付任务先查已有 Skill / Tool / Workflow / 脚本 / 自动化和验收证据”；对渐进式 SDD 只吸收强度分级、独立裁判、证据链、漂移检查和失败回流；周易 / 道德经 / 庄子 / 论语 / 中医 / 五德终始类取舍校准只落为工程反偏问题，周易、道德经、庄子、论语、中医、五德终始不替代事实、证据、测试、CR、授权或上线审批；不新增并列流程、命令菜单、默认安装，不默认运行外部脚本。
...
3. **先跑 Ask-or-Decide**：把问题拆成已知事实、关键分叉、可自答问题和需 owner 判断的问题；复杂 / 模糊任务一次只问一个主 blocker，并给出建议答案、依据、影响和默认暂停点；Loop 推进中如发现当前 blocker 的依赖问题没走到底，切入 Grill-Me 盘问卡，直到得到可写入下一阶段输入的决策记录、被排除方案和待确认项；能凭证据自答且低风险的，直接进入设计、落地或验证下一环。
