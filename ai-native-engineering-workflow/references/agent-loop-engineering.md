...
- 一份授权、验证和失败回写设计：只读 / 计划内低风险执行 / Plan Grant / Wave Grant / CAD Grant / Codex 替我审批通道 / 显式确认；测试、lint、CR、源码锚点、用户反馈、eval、发布监控或人工验收如何进入下一轮；验证失败、需求不清、边界冲突、授权不足或连续无进展时回写到哪里。
- 一份自主交付控制卡：可自我挖掘、可自我规划、可自动执行、必须人工确认、置信度、状态回写和停止条件。
- 一份小闭环 Ask-or-Decide Card：每个阶段完成后，判断 Auto-Decide Next、Auto-Ask Owner、Auto-Loop 或 Stop-Handoff，并把结论写入下一阶段输入或状态载体。
...
Loop 契约只保留决策必须项，细节回写到 Goal Ledger、GSD Wave、Harness Plan、OpenSpec tasks、验证矩阵或任务文档：
...
| 产研交付视图 | 产品专家到架构师的端到端交付、大项目分波、真实代码任务推进。 | 先补 Goal / GSD / Harness，再按授权进入 CAD 原子子循环。 |
...
```
决策规则：
- `Auto-Decide Next`：证据足以支撑下一阶段、仍在 Goal / Grant / 写入范围内、低风险可回滚、没有 owner 冲突，且下一阶段有可执行验证或 Checker。必须把决定写回 Goal Ledger、GSD Wave、Harness、验证矩阵或任务文档，再让下一阶段消费。
...
自驱不是让 Loop 自己拍板业务结论，而是把每个小闭环后的问询或决策显式写入下一阶段输入。能凭证据决定的不要反复打扰 owner；需要 owner 负责的不能用 AI 置信度替代确认。

## 3. 内部层映射

旧说法 `GSD + Goal + Loop`、`GSD/CAD`、`CAD + Goal` 统一映射为角色协作 Loop 的产研交付视图：
