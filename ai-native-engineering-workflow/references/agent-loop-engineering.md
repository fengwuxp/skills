...
- 命中 Grill-Me 盘问门禁：补 Grill-Me 盘问卡，说明触发原因、当前决策树分支、推荐答案、已自答证据、模糊回答 push back、退出条件和写回位置。
...
自我问询 / 自我回答 / Grill-Me 盘问: 触发原因 / 当前分支 / 问题 / 推荐答案 / 已自答证据 / 模糊回答 push back / 退出条件:
...
Grill-Me 只在 Loop 推进受阻时升级 Ask-or-Decide：关键分叉会改变业务目标、公共契约、状态机、验收样例、写入范围、发布风险，或用户回答含糊、半答、连续返工时进入；沿一个决策树分支深度优先盘问，一次只问一个问题，每问给推荐答案和依据；能从代码库、文档、测试或日志自答的问题先自答，再让 owner 确认。退出条件是形成可写回产品上下文卡、工程交接卡、任务树 / 计划切片或验证矩阵的决策记录：已确认选择、被排除方案、剩余待确认项、下一阶段输入。不得把 Grill-Me 变成无休止访谈、替代 owner 决策、替代测试 / CR / 发布准出，或把问询过程写进正式 PRD / 系分正文。
