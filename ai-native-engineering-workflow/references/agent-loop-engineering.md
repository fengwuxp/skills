...
判定: Auto-Decide Next / Auto-Ask Owner / Auto-Loop / Stop-Handoff
...
先自问再问人：Loop 先从目标、事实、源码、测试、约规和交接物中自问“我已经知道什么、能凭证据回答什么、下一环缺什么”；能自答且低风险就推进设计、落地或验证，不能自答且影响责任边界才 Auto-Ask。自驱不是让 Loop 自己拍板业务结论，而是把每个小闭环后的问询或决策显式写入下一阶段输入。用户接受建议或说“按你建议推进”时，先关闭当前主 blocker，再推进下一最高价值 blocker；不要同时摊开多个 blocker。
...
- 实现前先过最小正确实现门禁：确认是否无需新增、是否已有代码可复用、标准库 / 平台原生 / 已安装依赖是否覆盖、是否存在一行或更小实现；没有证据时不新建抽象、配置、依赖、脚手架或未来扩展点。
