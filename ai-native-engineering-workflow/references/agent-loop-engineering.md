...
- 一份 Loop 准入结论：不进入角色协作 Loop / 进入只读理解视图 / 进入产研交付视图 / 进入验证发布视图 / 进入知识回流视图 / 必须人工主导。
- 一份最小 Loop 契约：角色 Loop 场景视图、Goal、任务树真相源、状态载体、计划层、决策输入、可调用 Skill / 工具、允许/禁止动作、反馈源、验证者、预算、最大轮次、无进展检测、停止条件、恢复入口、交接物和人类理解检查。
- 一份授权、验证和失败回写设计：只读 / 计划内低风险执行 / 受控执行授权 / Codex 替我审批通道 / 显式确认；测试、lint、CR、源码锚点、用户反馈、eval、发布监控或人工验收如何进入下一轮；验证失败、需求不清、边界冲突、授权不足或连续无进展时回写到哪里。
- 一份自主交付控制卡：可自我挖掘、可自我规划、可自动执行、必须人工确认、置信度、状态回写和停止条件。
- 一份小闭环 Ask-or-Decide Card：每个阶段完成后，先自我问询并用证据自答，再判断 Auto-Decide Next、Auto-Ask Owner、Auto-Loop 或 Stop-Handoff，并把结论写入下一阶段输入或状态载体。
- 一份生产可用 Loop 门禁：自动化心跳、隔离执行、Skill 上下文、连接器权限、Maker / Checker 解耦、可复现状态、观测审计、人工接管、发布/回滚、理解债控制和责任 owner。
- 面向真实代码库时，必须额外产出 Coding Loop Contract：任务 ID、代码写入范围、只读范围、失败测试 / 验收样例、实现顺序、最小正确实现门禁、验证命令、独立 Checker、状态回写位置、提交切片、回滚方式和停止条件。
- 面向 AI 生成代码或多轮编码时，必须额外产出复杂度投资门禁：是否新增浅模块、直通包装、公共知识泄露、多文件小改动、AI 注释噪声或只为过测试的战术实现；命中时路由 `资深架构师` 做设计 / CR 复核。
- 面向高风险业务 Loop 时，必须额外产出 Verification Cluster Gate：业务不变量、验证簇 ID、证据类型、生产重放边界、独立 Checker、预算 / CI 分层、置信度、状态回写和停止条件。
- 面向架构排熵时，必须额外产出 Architecture Entropy Card：可删除性、局部推理边界、承重行为、废弃 API / dead path、概念膨胀、事实源分裂、治理自腐、守卫自检、可执行约束 / 可追溯理由链、时间边界三问、扫描输入、状态回写、低风险动作、Maker / Checker、人工 triage、架构师路由和停止条件。
...
- `Task Tree / 任务树`：把模糊意图拆成可执行节点，每个节点写清目标、输入、owner、验收标准、依赖、状态和停止条件；可用 Trellis 等工具承载，但本仓库默认只吸收任务树结构，不默认安装或依赖外部服务。
...
完成判断：
- `Complete`：Goal 成功标准已被独立证据覆盖，验证通过，状态已回写，残余风险有 owner。
- `Loop`：证据新增且缺口收敛，仍在授权、预算和写入范围内。
- `Stop`：触发失败、风险、成本、信息不足或权限边界。
- `Human handoff`：需要人确认目标、取舍、授权、发布或高风险责任。
自主推进的价值是减少机械追问和重复搜索，不是减少人的关键判断。只要不能证明“正在接近目标且没有越界”，就停止并交接。

## 2E. 小闭环 Ask-or-Decide

每个小闭环完成后，不默认停下来等用户，也不默认继续执行；先把新增证据、未收敛缺口、风险和授权边界转成下一步决策。小闭环包括只读侦察、需求事实分层、产品 / 交互设计、设计评审、TDD 候选、编码实现、编码评审、验证发布和知识回流。小闭环 Ask-or-Decide Card：
