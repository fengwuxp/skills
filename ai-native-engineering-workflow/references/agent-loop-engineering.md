...
- 每个小闭环结束：补 Ask-or-Decide Card，判断 Auto-Decide Next、Auto-Ask Owner、Auto-Loop 或 Stop-Handoff。
...
当前主 blocker / 建议答案 / 依据 / 影响 / 默认暂停点: 判定 Auto-Decide Next / Auto-Ask Owner / Auto-Loop / Stop-Handoff
下一阶段输入 / 下一任务计划问询 / 状态回写位置:
```
决策规则：
- `Auto-Decide Next`：证据足以支撑下一阶段、仍在 Goal / Grant / 写入范围内、低风险可回滚、没有 owner 冲突，且下一阶段有可执行验证或 Checker。必须把决定写回目标账本、计划切片、执行契约、验证矩阵或任务文档，再让下一阶段消费。
- `Auto-Ask Owner`：缺口会影响业务价值、优先级、公共契约、数据 / 状态 / 权限 / 安全 / 合规 / 发布责任、多方案取舍、成本预算或授权边界。只问 1-3 个聚焦问题，每个问题说明影响和默认暂停点；不要用“要不要继续”这类宽泛问题替代判断。
- `Auto-Loop`：本轮证据新增、缺口收敛，且继续一轮仍在预算、授权和写入范围内。下一轮必须缩小假设或验证范围，不得扩张目标。
- `Stop-Handoff`：连续两轮无新增证据、范围 / 风险 / 授权越界、工具输出无法回链源码或材料、Checker 无法解释证据，或人类 owner 无法理解目标、变更和风险。
先自问再问人：Loop 先从目标、事实、源码、测试、约规和交接物中自问“我已经知道什么、能凭证据回答什么、下一环缺什么”；能自答且低风险就推进设计、落地或验证，不能自答且影响责任边界才 Auto-Ask。用户接受建议或说“按你建议推进”时，先关闭当前主 blocker，再推进下一最高价值 blocker；不要同时摊开多个 blocker。
`grilling` 只在 Loop 推进受阻时升级 Ask-or-Decide：自动触发只看四类信号：关键分叉会改变业务目标、公共契约、状态机、验收样例、写入范围、发布风险；用户回答含糊；用户半答；同一方向连续返工。显式提到 `grilling` 触发不稳定、角色 Loop 自动触发或问询推进时，先审计这四类信号；命中后沿一个决策树分支深度优先盘问，首轮只输出一张 `grilling` 盘问卡，包含触发原因、当前分支、`question_id`、一个问题、推荐答案、依据和默认暂停点，不列第二个待答问题、完整方案或执行清单；每问必须等待 owner 反馈，或用代码库、文档、测试、日志证据自答并让 owner 确认后，再进入下一问。下一问前先检查已问问题和答案状态；候选问题若和已问问题同义，只能关闭当前 blocker、只追问缺失项或 push back 模糊点，不得重问原问题。退出条件是形成可写回产品上下文卡、工程交接卡、任务树 / 计划切片或验证矩阵的决策记录：已确认选择、被排除方案、剩余待确认项、下一阶段输入。不得把 `grilling` 变成无休止访谈、替代 owner 决策、替代测试 / CR / 发布准出，或把问询过程写进正式 PRD / 系分正文。
