...
- 一份最小 Loop 契约：Loop Profile、Goal、状态载体、计划层、决策输入、可调用 Skill / 工具、允许动作、禁止动作、反馈源、验证者、预算、最大轮次、无进展检测、停止条件、恢复入口和交接物。
...
| 让 Loop 生产可用 | `7. 生产可用 Loop 门禁`，再读 `code-delivery-closed-loop.md` 和 `verification-review-release.md` | 不把能自动跑当能上线 |
