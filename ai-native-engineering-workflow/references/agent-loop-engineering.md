...
先自问再问人：Loop 先从目标、事实、源码、测试、约规和交接物中自问“我已经知道什么、能凭证据回答什么、下一环缺什么”；能自答且低风险就推进设计、落地或验证，不能自答且影响责任边界才 Auto-Ask。自驱不是让 Loop 自己拍板业务结论，而是把每个小闭环后的问询或决策显式写入下一阶段输入。

## 3. 内部层映射

旧说法 `GSD + Goal + Loop`、`GSD/CAD`、`CAD + Goal` 统一映射为角色协作 Loop 的产研交付视图；默认输出不展示旧模式名，只展示目标层、计划层、原子执行层、执行契约和授权边界。
...
- 用户仍可说“进入 GSD 产研协同研发流程”“GSD + Goal 按任务计划推进”“做 GSD/CAD 准入”，但输出应显示为角色协作 Loop 的产研交付视图，而不是要求用户继续在多模式之间选择。
