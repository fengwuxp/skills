...
`grill-me` 只在 Loop 推进受阻时升级问决门禁：自动触发只看四类信号：关键分叉会改变业务目标、公共契约、状态机、验收样例、写入范围、发布风险；用户回答含糊；用户半答；同一方向连续返工。长链路、结果分叉多、返工成本高，或人的 Taste / 偏好 / 红线会影响方案时，也先进入 `grill-me` 把隐性判断显式化。显式提到 `grill-me` 触发不稳定、角色 Loop 自动触发或问询推进时，先审计这些信号；命中后先做 Facts / Decisions 分流，Facts 用代码库、文档、测试、日志证据自答，Decisions 才沿一个决策树分支深度优先盘问。首轮只输出一张 `grill-me` 盘问卡，包含触发原因、当前分支、`question_id`、一个问题、推荐答案、依据和默认暂停点，不列第二个待答问题、完整方案或执行清单；每问必须等待 owner 反馈，或用证据自答 Fact 并让 owner 确认后，再进入下一问。下一问前先检查已问问题和答案状态；候选问题若和已问问题同义，只能关闭当前 blocker、只追问缺失项或 push back 模糊点，不得重问原问题。退出条件不是问完所有问题，而是形成可写回产品上下文卡、工程交接卡、任务树 / 计划切片、验证矩阵或 Shared Context / Issue / PR / hand-off prompt 的决策记录：已确认选择、被排除方案、剩余待确认项、下一阶段输入。`grill-me` 退出只形成 shared understanding 候选；owner 确认达成 shared understanding 或完成执行前对账前，不进入执行、写文件或生成计划。不得把 `grill-me` 变成无休止访谈、替代 owner 决策、替代测试 / CR / 发布准出，或把问询过程写进正式 PRD / 系分正文。
