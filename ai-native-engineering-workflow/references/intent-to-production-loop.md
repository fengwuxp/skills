...
目标、计划、原子执行、执行契约和授权边界都是本流程的内部层。对外默认只问“当前处于哪个角色协作阶段”，再由 Loop 自主归位内部实现，不把内部层展示成用户需要选择的流程。
...
- 小闭环问决门禁结论：每个阶段完成后先自我问询并用证据自答，再给出 自决推进、询问 owner、继续收敛或停止交接，并写清下一阶段输入。
...
轻量问询优先于长方案审查：问决门禁是小闭环总门禁，`grill-me` 是命中升级条件后的升级盘问。复杂或模糊任务不要先生成大段方案让用户审；先沿设计树一次只问一个关键问题，每问给出建议答案、依据和影响。先做 Facts / Decisions 分流：Facts 用已有材料、源码、测试、日志或约规自答，Decisions 才交给 owner 等待反馈。问询结论必须写入产品上下文卡、工程交接卡、任务树 / 计划切片或下一阶段输入，不进入正式 PRD / 系分正文的讨论过程。

问询推进只处理一个主 blocker。每次问询都写清：问题、建议答案、依据、影响和默认暂停点；用户接受建议或说“按你建议推进”时，先把当前 blocker 关闭并写入下一阶段输入，再挑下一最高价值 blocker，不重新摊开全局方案。

Loop 推进中自动按需触发 `grill-me` 只看四类信号：关键分叉未决、回答含糊 / 半答、连续返工、下一阶段会改变公共契约 / 状态机 / 验收样例 / 写入范围 / 发布风险。长链路、结果分叉多、返工成本高，或人的 Taste / 偏好 / 红线会影响方案时，也先进入 `grill-me` 把隐性判断显式化。显式提到 `grill-me` 触发不稳定时，先审计这些信号，再决定是否升级。

Loop 推进中适时进入 `grill-me`：命中上述信号时，先暂停执行，沿一个决策树分支深挖；首轮只输出一张 `grill-me` 盘问卡，包含触发原因、当前分支、`question_id`、一个问题、推荐答案、依据和默认暂停点，不列第二个待答问题、完整方案或执行清单；每次只问一个 Decision 并给推荐答案，等待 owner 反馈，Facts 用代码库、文档、测试、日志证据自答并让 owner 确认后再进入下一问。下一问前先检查已问问题和答案状态；候选问题若和已问问题同义，只能关闭当前 blocker、只追问缺失项或 push back 模糊点，不得重问原问题。退出条件不是问完所有问题，而是形成可写回的结构化决策摘要，写回产品上下文卡、工程交接卡、任务树 / 计划切片、下一阶段输入、Shared Context / Issue / PR / hand-off prompt；不把过程写进正式 PRD / 系分，不替代产品 owner、架构 owner 或发布 owner。

grill-me 退出只形成 shared understanding 候选；owner 确认达成 shared understanding 或完成执行前对账前，不进入执行、写文件或生成计划。grill-me 退出后形成决策快照，字段只保留已确认选择、被排除方案、待确认项、下一阶段输入和写回位置。执行、写文件或生成计划前必须做执行前对账：说明本轮执行依据哪条已确认选择，是否触碰被排除方案或待确认项；被排除方案不得复活，待确认项不得脑补；如果当前执行意图与决策快照不一致时停止并问 owner，不靠上下文记忆或模型推测覆盖已确认结论。问询交互中出现红线、底线、不能碰、不可、禁止、必须等高风险字眼时，重点关注并记录到决策快照或待确认项。
...
| 自我挖掘与确认边界 | AI Native 编排，业务 / 产品 / 架构 owner 确认关键判断 | 读取现有材料、代码、测试、历史反馈和约规，形成候选需求与边界 | 自主交付控制卡、事实/推断/待确认、人工确认点 | 无来源、冲突事实、目标取舍、范围扩张或高风险动作时停止转人工 |
...
| 发布准出 | 发布 owner / 架构师 | AI Native + `资深架构师` | GStack `/ship`、Harness 证据链、验证发布 reference | 生产交付审查卡、生产生效验证卡、监控 / 回滚 / 人工接管证据 | 不替代 Git / PR / merge / 部署授权 |
...
GStack 的 `/office-hours`、`/plan-ceo-review`、`/plan-eng-review`、`/plan-design-review`、开发、`/review`、`/qa`、`/ship` 只作为角色链审查模板接入角色协作 Loop。它解决“同一任务要被不同角色视角连续挑战”的问题，不新增对外主流程、不要求用户记命令，也不替代产品专家、资深架构师、质量门禁或发布 owner。

GStack 的价值不是命令名，而是连续逼问：先问为什么做，再砍范围，再审工程和交互，最后用独立 CR、QA 和发布证据准出。

| GStack 模板段 | AI Native 阶段 | 主责与能力来源 | 必须回答的问题 | 准出 / 停止 |
| --- | --- | --- | --- | --- |
...
| `/plan-ceo-review` 范围收敛 | 产品方案 / 目标层 Review | AI Native 编排 + 产品 owner | 是否过大、是否可逆、MVP 是否足够、非目标和成功指标是否清楚？ | 业务取舍或优先级不明时回 owner。 |
...
| `/qa` QA 验证 | 质量 / 测试门禁 | AI Native 质量门禁 + 资深架构师 testing 能力 | 正常、边界、失败、回归、安全、可靠性验证是否足够？ | 失败用例、残余风险或不可复现结果必须回写。 |
...
使用规则：命令名只作触发别名；若用户只给一个阶段，就只输出该阶段的 owner、交接物、验证与停止条件。若用户给完整 GStack 模板，按角色协作 Loop 输出阶段链路和下一 owner，不复制外部命令体系。
