...
轻量问询优先于长方案审查：问决门禁是小闭环总门禁，`grilling` 是命中升级条件后的升级盘问。复杂或模糊任务不要先生成大段方案让用户审；先沿设计树一次只问一个关键问题，每问给出建议答案、依据和影响。能从已有材料、源码、测试、日志或约规自答的问题先自答，只把影响业务取舍、公共契约、授权或高风险边界的问题交给 owner。问询结论必须写入产品上下文卡、工程交接卡、任务树 / 计划切片或下一阶段输入，不进入正式 PRD / 系分正文的讨论过程。

问询推进只处理一个主 blocker。每次问询都写清：问题、建议答案、依据、影响和默认暂停点；用户接受建议或说“按你建议推进”时，先把当前 blocker 关闭并写入下一阶段输入，再挑下一最高价值 blocker，不重新摊开全局方案。

Loop 推进中自动按需触发 `grilling` 只看四类信号：关键分叉未决、回答含糊 / 半答、连续返工、下一阶段会改变公共契约 / 状态机 / 验收样例 / 写入范围 / 发布风险。长链路、结果分叉多、返工成本高，或人的 Taste / 偏好 / 红线会影响方案时，也先进入 `grilling` 把隐性判断显式化。显式提到 `grilling` 触发不稳定时，先审计这些信号，再决定是否升级。
...
grilling 退出后形成决策快照，字段只保留已确认选择、被排除方案、待确认项、下一阶段输入和写回位置。执行、写文件或生成计划前必须做执行前对账：说明本轮执行依据哪条已确认选择，是否触碰被排除方案或待确认项；被排除方案不得复活，待确认项不得脑补；如果当前执行意图与决策快照不一致时停止并问 owner，不靠上下文记忆或模型推测覆盖已确认结论。问询交互中出现红线、底线、不能碰、不可、禁止、必须等高风险字眼时，重点关注并记录到决策快照或待确认项。
