...
本文定义从业务意图、需求收集、产品澄清、工程设计、真实编码、验证发布到反馈回流的端到端角色 Loop。它用于 AI Native 编排，不替代产品专家 PRD、架构师系分、源码级 CR、测试实现、发布审批或生产 owner 决策。
...
- Intent-to-Production Role Loop Map：阶段、主责 owner、协作角色、AI 角色、输入、交接物、验证门禁和停止条件。
...
| 产品澄清 | 产品专家 / UED | 辅助梳理流程、用例、交互状态和验收种子 | 产品上下文卡、流程 / 用例 / 页面状态、验收种子 | 产品只传话、未解释业务问题或影响面时停止 |
| 规格准入 | 产品专家 + 架构师 | 预扫描完整性、一致性、可测试性、二义性 | PRD / OpenSpec 输入、AC、待确认项 | 事实 / 推断 / 待确认混在一起时停止 |
| 合议预审 | AI Native 编排，产品 / 架构 owner 决策 | MAGI / Checker 提出挑战与风险 | ACCEPT/REJECT/PENDING、风险清单、owner | AI 预审不能替代 owner 决策 |
| 系分与计划 | 资深架构师 | 辅助形成方案、Harness 摘要和任务拆分 | 系分、ADR、Harness / Plan、写入范围 | 缺少模块边界、数据、测试或发布风险时停止 |
| 编码执行 | 资深架构师 / 工程 owner | AI Maker 在授权范围内实现，按 TDD 推进 | 失败测试、实现、提交切片、状态回写 | 不允许模拟模块、内存版业务 Service 或无业务入口 demo |
| 独立检查 | AI Checker + 架构师 | 对照 Spec、测试和源码锚点找问题 | CR 发现、测试结果、残余风险 | 写代码的 Agent 不能自证通过 |
| 发布交付 | 发布 owner / 架构师 / 运维 owner | 汇总证据、监控、回滚和交接 | 发布计划、监控指标、回滚方案、Runbook | 没有观测、回滚、人工接管或审批时停止 |
