...
- Loop 取舍校准准入结论：需要 / 不需要；如果不需要，说明直接走哪个 Loop Profile 或专门 Skill。
...
| --- | --- | --- | --- |
| 阴阳平衡 | Loop 准入 | 速度收益和治理代价是否同时可见；AI 自治和人工确认是否互为条件 | Loop Profile、授权策略、风险边界 |
...
触发原因：
Loop 阶段：准入前 / 拆解时 / 执行中 / 复盘后 / 授权纠偏 / 协作冲突
推荐框架：
反偏问题：
底线问题：
证据要求：
回写位置：
不替代项：事实 / 测试 / CR / Execution Grant / 上线审批 / 专业确认
下一 owner：
```
