...
| 升级 SDD / Superpowers 6.0 / Harness 版本 | `2. OpenSpec / Superpowers / Harness`、`2A. AI Native Harness Contract v3`，再读 `superpowers-skill-library.md` 和 `code-delivery-closed-loop.md` | 不把 v6 脚本或 `.superpowers/sdd/` 写成默认安装态 |
...
- `external-superpowers/subagent-driven-development` 已替换为 Superpowers v6.0.3 vendored skill，但 `task-brief`、`review-package`、`sdd-workspace` 只在明确进入 SDD 执行且用户授权写入工作区时运行。
