...
| 升级 SDD / Superpowers 6.x / Harness 版本 | `2. OpenSpec / Superpowers / Harness`、`2A. AI Native Harness Contract v3`，再读 `superpowers-skill-library.md` 和 `code-delivery-closed-loop.md` | 不把 v6 脚本或 `.superpowers/sdd/` 写成默认安装态 |
...
## 2A. AI Native Harness Contract v3

`AI Native Harness Contract v3` 是本仓库内部执行契约版本，不等同于外部 Harness、插件或 `.superpowers/sdd/` 默认运行态。它在原有 Harness v1（owner、顺序、写入范围、验证命令、停止条件和交接）之上，吸收 Superpowers v6 SDD 的可迁移纪律，并把执行状态机、文件化交接和恢复入口写成准入条件：
