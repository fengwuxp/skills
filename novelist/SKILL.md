...
用户指出想象力匮乏、模板化、脸谱化、又像前案，或同一项目正在连续设计一批承重人物、事件、势力与神异时，读 `references/creative-inertia.md`；孤立成品的牵强因果先读“成品纠错”，跨轮重复再启用识骨与负空间。在需要去同构的生成、选择或幻想归来前，先执行其中的因果发动机门禁，比较最近相关设计的匿名因果骨架，携带持续负空间，完成至少两条高杠杆轴差异及替换／删除测试；承重候选同构或无法承担声称作用时退回天马行空，不得继续换名词、换载体或补规则；测试范围按 reference 执行，生活感与审美不因没有外部转折而被判失败。该 reference 只约束本轮生成，不取得修改正典的授权，也不把作者有意保留的母题回声判成缺陷。普通一次性构思或作者明确要求镜像时不启用识骨与负空间；已经选定分支时不重开发散，但需要执行层去同构或场景贴合时仍读取对应小节。
...
- 作者侧正典档案、设定集、人物档案、时间线、伏笔/承诺台账、出版提案、编辑梗概和读者附录需要正式整理或 DOCX/PDF：相关故事事实及其状态已由作者确认，或在作者明确授予的自决范围内由 `novelist` 确认后，由当前 Agent 使用可用文档能力整理；`document-authoring` 仅在准入通过、运行时可用且符合调用策略时协同。整理和交接均须保留授权范围、逐项设定状态与确认证据 / 权威指针，缺失或冲突的条目保持待确认。
