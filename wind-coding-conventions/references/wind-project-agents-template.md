...
- 代码格式默认优先采用项目实际选用的版本化 IDEA Project Code Style，按语言核对；项目明确采用其他 formatter / CI 规则时遵从其权威。样式来源与缺口处理读取 `wind-coding-conventions/references/code-style.md`；缺少可确认样式时告知用户需要配置，不能把附近代码当成已确认方案。不得使用个人 IDE 默认设置覆盖项目约规，也不得格式化本次修改范围外的代码。格式化后审查 diff，并运行项目已有格式检查。
...
- 涉及资金、权限、配置、审批、敏感数据、内网 secure API、外部通道、MQ/Webhook/定时任务的操作，必须记录业务 ID、主体、动作、结果、失败原因和外部请求/响应摘要。
- 已由统一日志链路 / MDC 注入的 `traceId`、应用名称、用户 ID、租户标识和客户端信息不得在业务日志中重复打印；异步、MQ、定时或批处理缺失上下文时修复入口、传播和清理，不在每条日志中手动补字段。
- 业务日志只记录定位和追溯所需的最小上下文，不打印无意义流水账，不打印完整 Request/Response、Entity、SQL、密钥、token、证件号、银行卡、手机号等敏感信息。
- 可恢复失败、重试、降级、幂等拦截、补偿和人工兜底必须有日志或审计入口，依靠业务 ID 和 MDC 中的 traceId 串起时间线。
...
| 场景 | 必用 Skill | 产物边界 |
| --- | --- | --- |
| 端到端角色协作、项目执行规范/Loop/GSD/CAD 编排、owner/交接物/授权/验证/停止条件 | `wise-agent` | 只做流程准入、角色协作和交接闭环；旧 Goal 输入先迁移到项目执行规范，不替代产品、架构、代码、测试和 Git 授权。 |
