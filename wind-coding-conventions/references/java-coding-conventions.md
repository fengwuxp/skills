...
- 任意 Java/JVM 项目的编码红线、模型分层、契约、异常日志、安全、测试代码或数据库访问约规。
...
- 命中 Wind/Nobe 依赖、包名、类型、模块结构或明确上下文时，模块、服务和模型规则读 `wind-coding-conventions.md`；深度 Review、项目治理和测试交给 `资深架构师`。
...
- 【强制】新增或调整编码标准时，每条强制 / 推荐规则必须说明原因、适用范围、反例或示例、验证方式（测试、静态检查、CR）。
...
- 阿里手册只作为本通用 Java 层的外部来源，不单独创建 Skill；固定测试命名、统一覆盖率、机械作者日期和服务器运行参数等旧版或环境规则不升级为通用编码门禁。
...
- 【推荐】测试类和测试方法命名优先服从项目既有约定；没有约定时，测试类可使用被测类名 + `Tests`，测试方法直接表达行为，不强制使用 `test` 前缀。
...
- 【推荐】公共契约、扩展点和配置属性在名称与类型不足以说明职责、边界或兼容语义时补 Javadoc；简单 DTO/Request/Query 及自解释公有方法不机械补文档。
...
- 【推荐】测试命名遵循 4.1 节和项目既有约定，不为统一形式批量重命名已有测试。
- 【强制】每个测试方法至少一个断言。
...
| 单元测试 | 增强采纳 | 补充测试资产治理、方法级注释、Given/When/Then；按项目风险设置门禁，不设置统一覆盖率百分比。 |
