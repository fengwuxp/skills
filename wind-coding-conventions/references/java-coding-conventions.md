...
| 任务 | 优先读取 | 跳过 |
| --- | --- | --- |
| 快速约规核对或编码红线 | `1. 编码规约总纲`、`4. 编程规约`、`5. 契约规约` | 阿里采纳矩阵细节 |
| 命名、枚举、集合、并发、方法抽取、最小正确实现 | `4. 编程规约`、`18. 阿里规约采纳矩阵`、`19. 设计质量回看` | 数据库和测试章节 |
| 单行注释、块注释、Javadoc 的编写、修改或 CR | `4.8 注释与 Javadoc`；格式配置冲突时再读 `4.6 控制语句与代码格式` | 不扫描或重排无关注释 |
| 契约、异常、日志、安全 | `5. 契约规约`、`6. 异常与日志规约`、`14. 安全编码规约` | Lombok/MapStruct 细节 |
| Java/Spring、依赖治理、Lombok、MapStruct、不可变对象 | `7. Java/Spring 专项规约`、`7A. Maven / Gradle 依赖治理`、`8. Lombok 与 MapStruct 规约` | MySQL/MyBatis 细节 |
| 数据库、MyBatis Flex、模型分层 | `11. 数据库与 MySQL 规约`、`12. MyBatis / MyBatis Flex 规约`、`13. 分层与模型规约` | 并发和 Lombok 细节 |
| 测试代码和自动化检查 | `15. 测试代码规约`、`17. 自动化检查映射`、`20. 与其他规范的关系` | 业务命名细节 |

## 1. 编码规约总纲

- **可读优先**：代码主要供人阅读和维护，顺便用于执行；命名、结构和控制流应直接表达业务意图，正确性和必要的失败保护不能因精简而降低。
...
- 【强制】面向接口编程：已有可表达所需能力的稳定接口时，调用方的依赖字段、构造参数和对外协作签名使用接口类型；不得注入具体实现类、向下转型或依赖实现专有方法绕过契约。只有一个实现不是改用实现类型的理由；多实现通过项目既有限定符或装配配置选择，不能用实现类型规避歧义。
- 【说明】实现类的创建与选择留在组合根、工厂、`@Bean` / `@Import` 等装配位置；纯值对象、DTO、算法类和实现专属单元测试不机械提取接口。接口是否新增仍按能力价值与稳定边界裁决，“不滥建接口”不能反过来否定已成立接口的消费约束。
- 【检查边界】离线守卫仅对扫描目录内可解析的直接 `XxxServiceImpl implements XxxService`，检查调用方带访问修饰符的依赖字段与显式构造参数；Spring 注入测试同样检查，配置装配与纯实现单测除外。继承、泛型、多接口、依赖 JAR、通配 import、工厂方法和向下转型仍需源码级评审，扫描无发现不代表接口依赖全面合规。
...
- 【强制红线】先执行 `code-style.md` 的“阅读与维护优先”：复合断言与条件须能直接读出业务规则，不能以堆叠否定、取值链或注释换取短代码；拆分时保持短路、空值、区间边界和异常契约。
- 【强制】`if`、`for`、`while`、`switch`、`do` 等保留字与括号之间必须有空格。
- 【强制】二目、三目运算符左右两边必须有空格。
- 【强制】注释的双斜线与注释内容之间保留一个空格。
- 【强制】行宽与换行服从已确认的 Java Code Style；未配置时告知用户需要配置或确认，不采用固定行宽兜底，也不为凑行宽拆碎短表达式、参数或方法调用。
- 【强制】普通类型引用优先使用 `import`，不要在字段、方法参数、返回值、泛型和局部变量中随意写全限定类名。
- 【推荐】控制语句避免深层嵌套，优先使用卫语句、提取方法或策略对象表达分支。
- 【强制红线】单个表达式中的三目运算符最多一层；禁止嵌套、连锁或在同一表达式中组合多个三目。三目只用于条件简单、两支短小且无副作用的单一值选择，不得承载状态变化、校验、异常、依赖选择或长调用链。
- 【强制红线】不得在同一方法或同一分支流程中大量、重复使用三目来替代可读的控制流；出现多个业务分支时，使用有业务含义的局部变量、卫语句或 `if/else`，必要时提取职责清晰的方法。不为消除一个简单三目机械抽取策略类，也不禁止清晰的 `enabled ? ENABLED : DISABLED`。
- 【强制】样式来源优先级、IDEA scheme 选用证据、缺配置提示与工具冲突处理统一读取 `code-style.md`。Java 另核对 import 排序与分组、普通 / static 通配符阈值及例外包、链式调用与续行缩进；不得从其他项目或个人 Default 推定本项目采用的设置。
...
- 【强制】不得保留无业务语义的单行透传方法；公有或私有方法如果只是原样转调、原样传参、改名包装或返回下游结果，且没有新增业务命名、边界校验、权限/审计、事务语义、异常转换、观测埋点、稳定契约或可测试隔离，应删除并内联调用。单调用方方法若仅重命名 JDK / 项目 helper、getter 或原样转发相同参数，且不承载策略、归一化、复用、测试替身、副作用边界或稳定业务语义，也应内联。
...
- 【推荐】时间方法按职责而不是名字裁决：单调用方方法若只包装 `LocalDateTime.now()` 通常直接内联；封装 `Clock`、统一时区、可测试时间源或业务时间策略时应保留。该判断依赖调用语境，不做静态扫描。
...
先区分声明的契约文档与实现说明，再按表达长度换行。`//` 的“单行”指每个标记到行末结束，不限制一段说明只能占一行；连续多行 `//` 可以完整说明一段实现。不能只按行数决定注释类型。

以下格式在项目无明确相反约定时采用；项目已提交的格式规则按 4.6 节执行。实现说明默认优先 `//`、传统 Javadoc 即使一句也展开多行、普通块注释起止符独占一行，以及带独立注释的字段之间留空行，均是本 Skill 的团队默认风格，不是 Java 语法要求或统一行业标准。Google 允许短且无块标签的单行 Javadoc、连续多行 `//`，并允许连续字段按需留空行；阿里手册的对应约定并不完全相同，核验版本与采纳边界见 `source-map.md` 的“Javadoc 与注释格式”。

| 类型 | 使用场景与位置 | 格式 |
| --- | --- | --- |
| 单行注释 `//` | 实现说明的默认选择：局部原因、分支约束，以及一段代码共同适用的多行算法或兼容说明。放在相关代码上方，连续使用时共同构成一个说明段。 | 每行 `//` 后空一格，与被说明代码同级缩进。行尾注释只用于很短且无歧义的局部说明，代码与 `//` 之间至少一格，并服从项目行宽。 |
| 普通块注释 `/* ... */` | 实现说明的可选形式：沿用项目既有段落风格，或需要项目 formatter 按段落自动重排长说明时使用；连续 `//` 通常不作同样的段落重排，具体以工具配置为准。两者可以承载同样的内容，不因跨行而强制选用块注释。 | 使用时将 `/*`、正文、`*/` 分行；正文每行以对齐的 ` * ` 开头，空行保留 ` *`，结束行写 ` */`。不把普通说明嵌入表达式或参数列表。 |
| 传统文档注释 `/** ... */` | 类型、方法、构造器、字段等声明的职责和调用契约，说明使用者需要知道的边界；按本节必要性规则选用，私有方法有需要长期保留的契约时也可使用。放在声明及其声明注解之前。 | `/**`、正文、`*/` 分行，即使只有一句；正文星号与结束星号对齐，星号后空一格。摘要先写职责，必要时分段补边界；必要标签按下文完整性规则提供，不填写空标签或只重复名称。 |
...
```java
// 旧协议按字节长度截断，新协议按字符数校验。
// 两条路径暂时分别保留，避免改变已发布客户端的边界行为。
```
...
```java
/**
 * 原子创建独立计划和默认版本一，不解析发布引用或生成物理绑定。
 */
```

- 【强制】采用本节默认风格时，新增或修改的普通说明不写成 `/** 说明。 */` 或 `/* 说明。 */`；前者展开成文档注释，后者按用途改为 `// 说明。` 或展开的块注释。项目明确采用其他合法格式时遵循项目约规，不将其判为语法错误。
- 【强制】已编写的方法文档按所用 JDK 的 Javadoc 规范提供职责摘要、类型参数与形参的 `@param`、非 `void` 返回值的返回说明，以及 `throws` 子句各异常的 `@throws`；不适用的项目不填，可合法继承的文档不重复。不能以“不机械填标签”为由省略必要契约，也不能为消除告警填空标签；调用者需知道但未在 `throws` 子句声明的异常同样说明触发条件。
- 【强制】Javadoc 不放进方法体冒充局部说明；调用者需要知道的事务、异常、幂等等契约也不能只藏在方法体注释中。接口已说明的契约不在实现类原样复制；继承契约沿用项目方式，有新增实现限制时只补差异。
- 【推荐】注释换行按语义与项目行宽处理，不把一句话机械拆成多行，也不为省行数拼接多个边界。格式化指令、工具标记和用户明确保留的样例按其用途处理，不套用普通说明规则改坏语义。

#### 类、方法、成员变量的注释归属与分隔

以下约定说明“需要写注释时如何归属”，不要求为每个声明补注释；是否需要由本节必要性规则与项目约规决定。命中 Wind 时，新增命名类型的文档和元信息继续遵循 Wind 专项，不在通用层复制类型头模板。

| 对象 | 注释内容与类型 | 位置与分隔 |
| --- | --- | --- |
| 类、接口、枚举、record 等类型 | Javadoc 描述整体职责、使用边界和重要约束；不把成员说明堆在类型头。 | 放在该类型的声明注解之前；顶层类型与前面的 import 或其他类型之间留一空行，嵌套类型按成员单元分隔。 |
| 方法与构造器 | Javadoc 描述调用契约、必要的参数、返回、异常和副作用；方法体内的局部取舍用 `//`。 | Javadoc、声明注解、签名、方法体构成一个单元，与前一个成员之间留一空行；文档与声明注解、声明注解与签名之间不插空行。 |
| 成员变量、常量与枚举值 | 字段的含义、单位、取值、默认值、空值或不变量属于声明文档，需说明时用 Javadoc；只解释内部存储或实现选择时可用 `//`。不重复字段名，也不把声明契约写成行尾注释。 | 放在对应字段或枚举值的声明注解之前。每份注释只归属紧随其后的声明；各自带注释的成员单元之间留一空行，无独立注释且紧密相关的字段可连续排列。 |
| 局部变量与代码片段 | 必要的实现说明默认用 `//`，不用 Javadoc。 | 注释紧贴被说明代码；切换到另一逻辑段时，在新注释之前留空行，不在注释与对应代码之间插空行。 |

- 【强制】同一声明按“注释 → 声明注解（如有）→ 声明”连续排列，独占行时保持同级缩进；不要把 Javadoc 放在声明注解之间、声明之后或上一方法尾部，让归属含糊。`TYPE_USE` 注解留在被标注类型之前，例如返回类型上的 `public @Nullable Person findPerson(...)`；不得为统一排列而把返回类型、参数、泛型实参或数组维度上的类型注解移到声明头。
- 【推荐】成员之间用空行表达分隔，不新增 `// =====`、`/* ----- */` 等装饰性横幅。字段确需分组时，用独占一行的简短 `//` 组说明；组与组之间空一行，组说明不替代各字段必要的文档。
- 【推荐】Javadoc 内部按“职责摘要 → 必要的边界说明 → 标签”组织；不同语义段之间用只含 ` *` 的空行分隔，需在生成文档中分段时按项目 Javadoc 风格使用 `<p>`。标签与正文之间留一条注释空行，不为凑格式补无意义段落或标签。

#### 使用时机与审查

- **写代码前**：需要声明公共契约或复杂边界时，先明确职责与约束，再起草必要的 Javadoc；无需注释也能清楚表达的代码不预填模板。
- **实现过程中**：确有局部设计取舍、兼容原因或不能从代码读出的约束时，在相应位置优先补 `//`，需要多行时连续书写；块注释按项目风格选用。不逐行翻译代码，不将任务计划、执行步骤或会话状态写进源码。
- **修改代码时**：行为、参数、异常、副作用或约束变化时，同步更新本轮相关注释；仍有作用的历史原因和兼容依据保留，不借格式修正清理无关内容。
- **CR 与交付前**：先确认项目采用的约规，再核对必要性、类型、声明归属、声明注解与类型注解的位置、文档及继承契约的完整性、空行分隔、星号与换行、内容是否匹配实际行为；项目允许连续多行 `//` 时，不能仅因行数判为类型错误。纯格式偏差按项目检查处理，不夸大为架构或正确性缺陷。使用项目已有 formatter / Checkstyle 等检查，并人工核对语义；没有对应工具时说明人工检查，不为此新增解析器、依赖或全仓格式化。
...
- 【强制】日志维度必须有明确的排障、审计、告警、统计或检索消费价值；删除无人消费的维度，只记录稳定业务 ID、动作、结果和必要失败摘要，不因“以后可能有用”堆积对象字段。
...
- 【强制】接口、Service、ApplicationService、Facade、Adapter 的拆分必须承载真实业务职责、业务抽象或稳定契约，例如用例编排、事务边界、权限/审计、状态转换、跨资源协调、异常聚合或对外协议隔离；不得新增没有独立责任的透传包装或改名转发，也不按单实现、短方法或接口数量裁决。既有服务能在自身责任、数据与事务边界内承接时，优先复用、增强或合并重复职责。纯约规检查以本条为依据；涉及具体保留、合并或删除的架构取舍时再协同 `senior-software-architect`，按其 `references/project-governance-service-api-modeling.md` 的“服务职责裁决”判断。该能力不可达时保留规则结论并标明架构判断待定，不把包外 reference 当成本 Skill 独立使用的必读依赖。
...
- 【强制】写测试前读取项目已有测试基类、配置和同层测试样例，复用适用底座；已有样例违反当前约规时不照抄。Spring 服务契约测试优先注入接口，具体实现仅在测试装配处选择；纯实现单元测试可以直接构造被测实现，不能据此宣称验证了 Spring 装配或事务。
...
| 复杂条件与断言的可读性 | 源码 CR 核对业务命名、条件适用范围和失败出口，行为测试核对边界与短路；不按运算符数量、行宽或守卫通过机械判为可读 |
