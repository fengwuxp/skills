...
- 【强制】空值处理按边界分层：内部 Java 契约沿用项目已采用的空值注解体系；API 数据模型在项目使用 Bean Validation 时用其表达输入约束；业务前置条件、状态条件和不可达分支使用项目统一断言工具或清晰的标准库检查。
...
- 【强制】已由空值注解标为非空的参数、返回值和字段，不再额外添加无业务语义的重复空判断；只有业务前置条件、状态条件、集合内容约束、查不到数据、不可达分支等运行时业务事实才使用断言。
