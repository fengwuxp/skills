...
- 空值责任只在所属边界处理一次：受本服务治理的数据库字段先核对实际 schema 与映射，`NOT NULL` 成立时按非空契约处理；项目已使用 Bean Validation 时，API 入参沿用对应注解并确保校验入口生效；项目已依赖 JSpecify 时，内部 Java 空值契约使用 JSpecify；业务前置条件和状态条件使用项目断言工具。不得对已经证明非空的值层层判空；数据库约束不能替代不可信输入校验，外部 / 历史数据、外连接、反序列化和可能绕过校验入口的路径仍在最近责任边界检查。若 `ServiceImpl` 会为 Request 字段补默认值，该字段的 Bean Validation 必须允许缺省，默认值应在服务入口归一化后再参与幂等查询、唯一键判断、转换和落库；不得把基础服务内部默认值泄露给调用方或上层 ApplicationService。
