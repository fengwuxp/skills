...
- 【强制】项目已提供 `com.wind.integration.operator.WindOperator` 时，新增或修改的公共 Service、ApplicationService、Command 和 Event 需要操作人上下文必须复用该类型；不得在业务模块再造私有 Operator、OperationUser 或复制其身份、来源字段的操作人模型。
...
- 纯约规判断直接使用本节的服务职责与包位规则。任务还需要裁决能力是否值得独立提供、服务保留或合并等架构取舍时，再协同 `senior-software-architect`，读取其 `references/project-governance-service-api-modeling.md` 的“能力价值与架构裁决”和“服务职责裁决”；该能力不可达时只将架构取舍标为待定，继续完成本包可独立回答的规则判断，不自动安装、不建立反向必需依赖。已有基础服务或应用服务能承接时优先增强或合并，不为分层另造 `AggregateService` / `Loader`；需要对照保留与合并反例时读 `wind-coding-examples.md` 的示例 1—3。
...
- 【强制】项目已确认由统一日志链路 / MDC 注入 `traceId`、应用名称、用户 ID、租户标识和用户请求客户端信息（如 IP、User-Agent、设备）时，业务日志不得在消息模板或参数中重复打印这些字段；只补充业务 ID、动作、结果、失败原因和必要的外部调用摘要。没有 MDC 配置、日志格式或运行证据时，不得机械删除现有上下文字段。
- 【强制】异步任务、线程池、MQ、定时任务或批处理缺少上述上下文时，应修复入口初始化、上下文传播和执行后清理，优先使用项目已有 Trace / TaskDecorator / 消息上下文能力；不得在每条业务日志中手动追加字段来掩盖传播缺口。
- 审计记录、业务事件或可回放消息若契约明确要求用户、租户或客户端字段，应写入结构化审计 / 事件 schema；这不等于在日志字符串中重复 MDC。所有上下文仍遵守最小必要和敏感信息脱敏规则。
