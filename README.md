...
轻量问询只服务 Ask-or-Decide：复杂或模糊任务一次只问一个主 blocker，并给建议答案、依据、影响和默认暂停点；能从材料、源码或测试自答的先自答。角色 Loop 自动按需触发 `grilling` 只看四类信号：关键分叉未决、回答含糊 / 半答、连续返工、下一阶段会改变公共契约 / 状态机 / 验收样例 / 写入范围 / 发布风险。用户说“按你建议推进”时，先关闭当前 blocker，写入下一阶段输入，再挑下一最高价值 blocker，不重开全局规划。

`grill` / `grilling` 有两种用法：直接用 `grilling` 适合在开工前盘计划或设计；AI Native 里的 `grilling` 盘问门禁适合角色 Loop 已经在推进、但当前 blocker 还有关键分叉。使用时先给计划、材料路径、当前 blocker 和不想被反复问的已知答案；每轮只回答 `接受建议 / 改答案 / 补材料 / 停止`，如果问题重复，直接指出已回答和缺失项。盘问结束后要求写回已确认选择、被排除方案、待确认项、下一阶段输入和写回位置。
...
- 做 `grilling` 盘问 / `grill 一下这个方案`：开工前或关键分叉处只盘一个决策分支；每轮只问一个问题并给推荐答案，回答接受、改答案、补材料或停止。
...
用 AI Native Engineering Loop 做 `grilling` 盘问门禁：AI 工作流中 `grilling` 触发不稳定，请在角色 Loop 中按四类信号判断是否自动触发；命中时一次只问一个问题、给推荐答案，能证据自答先自答；我每轮只回答接受建议、改答案、补材料或停止，结束后请写回已确认选择、被排除方案、待确认项、下一阶段输入和写回位置。
