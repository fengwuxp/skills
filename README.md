...
仓库默认加载知止者 `wise-agent/SKILL.md`，由它统一承接目标、选择专业 Skill 并完成交付，用户无需点名。简单任务直接完成，单一领域任务装载对应专业能力；跨专业、跨阶段或跨轮时按实际需要增加协作和状态管理，不先写规划文档。
...
直接描述任务即可：知止者默认承接，再按需装载专业能力。`$wise-agent：`、`wise-agent，<目标>` 和“知止者，<目标>”仍可使用，但不是触发前提；直接点名专业 Skill 表示优先使用该能力，目标和交付仍由同一 Agent 负责。
...
| 小说视觉设计 | `$fiction-visual-designer：继承 <小说正典或独立创意>，为 <人物/服饰/建筑/兵器/灵药/灵物> 形成视觉设计契约；区分视觉候选、已确认设计和已验收图像。已确认人物可继续编排审美基准、身份锚图与定向修订，已确认器物可编排多视图与拼版；图片仍由 imagegen 执行。` |
...
小说任务也由知止者默认承接，装载 `$novelist` 作为故事主能力；单次构思、正文、重写或评审直接完成，跨轮、多稿权威、状态恢复或需要组合文档与考据能力时才增加相应控制。`huaxia-practical-wisdom` 是小说家的必要叙事校准依赖，用户无需另行点名；`document-authoring` 仅用于显式候选评估，`hanzi-philology` 只在真实证据问题出现时加载。
...
默认取消“先做文档规划，再执行”：目标、范围和授权明确就读取事实、按约规实施并验证，复杂或多轮任务同样适用。文档按用户或项目明确要求交付，实际恢复与交接记录随工作补充。决策寻路只在用户要求路线规划时启用；目标大致明确，但路线仍模糊时可先取证，不要生成 Spec、计划或执行任务来替代关闭真实决策缺口。
