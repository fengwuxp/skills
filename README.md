...
当教程、演示、代码、规范和真实产物共同描述一项做事能力，而你希望把它变成可重复执行、可验证、可维护的 Agent 能力时，使用 `$resource-capability-distiller`。它解决的是“这些材料能沉淀出什么能力、应放到哪里”，不是“把材料总结成一篇文章”。

**适用与不适用**

| 任务 | 是否使用 | 正确处理 |
| --- | --- | --- |
| 多种材料共同描述一项可复用操作或判断 | 使用 | 提炼能力单元，处理冲突并逐项归位 |
| 需要判断应新建 Skill，还是增强 reference、script、asset 或 fixture | 使用 | 选择能承载职责的最小落点 |
| 只需摘要、翻译、润色或正式报告 | 不使用 | 直接成文，必要时使用 `document-authoring` |
| 只需建立某个业务域或工程域模型 | 不使用 | 交给产品或工程专业能力 |
| 已明确要修改哪个文件和哪条规则 | 不使用 | 直接做局部修改和验证 |
| 材料没有稳定复用价值或无法验证 | 可以评估 | 允许以零能力或任务内结论结束 |

**材料准备**

开始前尽量提供以下信息；未知内容可以明确标为待确认，不要猜：

- 可实际读取的正文、源码、规范、日志、成功产物和失败样例，以及各自路径或链接。
- 希望减少的重复错误、目标使用者、复用场景和目标环境。
- 材料的版本或日期、适用范围、许可、隐私和敏感信息边界。
- 允许的只读或写入范围，以及是否允许联网、安装、Git、同步或发布。
- 验收方式，例如 fixture、脚本断言、产物对比、行为样例或人工评审。

**可直接使用的指令**

```text
使用 $resource-capability-distiller 实际读取以下材料：
- 教程或演示：<路径、链接或时间戳>
- 源码与规范：<仓库、文件、符号或版本>
- 成功与失败产物：<路径或样例>

提炼目标：减少 <重复错误>，服务 <复用场景>，目标环境是 <环境>。
许可与敏感边界：<已知边界或待确认项>。
授权范围：只读 <路径>；不要联网、安装、提交、同步或发布。

请先说明资源整体目标、结构和局限，再建立来源与冲突矩阵；
提炼零个或多个完整能力单元，分别判断应新建 Skill、增强既有 Skill、
进入 reference / script / asset / fixture，还是不沉淀；
最后给出行为验证、拒绝项、待确认项和停止条件。
```

没有明确写出 Skill 名称也可以直接描述目标，例如：`这组教程、CLI 源码、官方规范和失败日志存在版本冲突，请提炼可复用操作能力，说明证据、落点和验证，不要默认新建 Skill。`

**如何审阅结果**

1. 先确认整体理解是否覆盖资源目标、结构、局限和可能力化范围，而不是从单个片段直接下结论。
2. 检查每项事实能否回到来源锚点；版本、平台或场景冲突是否被保留并解释。
3. 检查能力单元是否具备触发、前置条件、步骤、失败分支、停止条件、输出和验收。
4. 检查建议落点是否最小；已有 Skill 或 reference 能承载时，不应重复创建顶层 Skill。
5. 检查正例、硬负例和相邻能力混淆；模型自述、文件数量和篇幅不能作为通过证据。

| 状态 | 含义 | 下一步 |
| --- | --- | --- |
| `candidate` | 稳定职责已识别，但证据、步骤或验收尚未闭合 | 补证据或保留在任务内 |
| `accepted` | 已通过结构和行为门禁，可进入最小落点评审 | 仍需 Owner 确认和对应资产验证 |
| `rejected` | 缺少复用价值、证据不足、无法验证或与既有职责重复 | 保留失败门禁和理由，不重复提炼 |
| 零能力 | 当前材料不能形成合格能力单元 | 正常停止，不用模板填充或强造 Skill |

候选不是安装结果，`accepted` 也不等于已经安装、同步、提交、发布或获得领域批准。

**结构化候选校验**

需要让候选 JSON 进入确定性结构检查时，在仓库根目录运行：

```bash
resource-capability-distiller/scripts/check_capability_candidate.py --file <candidate.json>
```

使用已安装 Skill 时，可将脚本路径替换为 `~/.codex/skills/resource-capability-distiller/scripts/check_capability_candidate.py`。退出码 `0` 表示结构门禁通过，`1` 表示候选字段或来源关系不合格，`2` 表示文件读取或 JSON 解析失败。完整字段和状态规则见[提炼契约](./resource-capability-distiller/references/distillation-contract.md)；脚本不判断领域事实、来源真实性或能力价值。

**停止与授权边界**

- 无法取得正文、源码或真实产物时，停止写成已确认能力。
- 来源许可不明或含 token、客户数据、生产配置时，停止复制和持久化，只保留脱敏边界。
- 同版本来源冲突且没有决定性证据时，标为待确认，不拼接成一套流程。
- 没有稳定复用场景、可执行步骤或验收证据时，以任务内结论或零能力结束。
- 联网、读取私有目录、执行来源脚本、安装、Git、同步、部署和发布始终需要单独授权。
...
工作拓扑投影不是新的 `Graph Mode`。只有三个以上节点存在分支 / 汇合依赖、并行 Worker、跨 Wave 交接或动态取消 / 重排时，才在现有 Goal 状态契约中附加 `work_graph`；简单、线性和单文件任务不生成。友好指令：`这个 Goal 存在多分支依赖，请生成可校验的工作拓扑投影；Loop 留在节点内，并检查并行写入冲突和高风险 Checker。`
