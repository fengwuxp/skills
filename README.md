...
AI Native 的默认输出骨架固定为：结论、角色 Loop 场景视图、当前阶段、角色视角、Owner / 下一步分派、交接物、Loop Contract、自我问询 / 回答、证据边界、内部实现层、授权策略、验证门禁、停止条件、残余风险 / 需要确认。证据边界必须区分事实、推断、待确认和范围外不做；内部实现层默认不展开旧模式名，授权策略必须区分只读、计划内低风险执行、受控执行授权和显式确认。
...
- `进入自主交付闭环`：让 AI 先自我挖掘材料、自我问询 / 回答、确认边界、规划低风险下一步，再明确哪些事项必须人工确认。
...
日常使用优先看上面的快速指南。本节只保留技能边界、典型触发和常见组合。
...
- [obra/superpowers v6.0.3](https://github.com/obra/superpowers/releases/tag/v6.0.3)：2026-06-30 已通过 GitHub Releases API 核验 latest release 为 v6.0.3，并读取 v6.0.0 release 的 SDD 套件变化。本仓库只吸收 `task-reviewer-prompt.md` 单一评审、pre-flight plan review、文件化 handoff、progress ledger、final broad review 和 model/cost policy 等 SDD v6 方法契约；不整包替换 vendored Markdown 基线，不复制或运行 `task-brief`、`review-package`、`sdd-workspace`、visual companion server、hooks 或安装脚本，也不默认创建 `.superpowers/sdd/`。
