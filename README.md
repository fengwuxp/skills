...
- 跨角色产研交付 / 目标拆解 / 任务计划 / Spec / Harness / 质量发布 / 知识生产闭环：用 `AI Native Engineering Loop`。它默认进入角色协作 Loop，按设计、设计评审、TDD、编码、编码评审、可用性/安全性评估、验证发布和复盘回流分角色推进；目标层、计划层、原子执行层和执行契约是内部层，不替代 PRD、系分、代码或发布审批。
...
- **只读理解视图**：阅读分析代码库、设计-代码对齐、工具准入、事实边界检查和影响范围识别；Gemini CLI 可用且读取范围获授权时优先只读启动，默认不写文件、不安装。
...
AI Native 的默认最小输出只保留：结论、当前阶段、owner、交接物、授权策略、验证与停止条件；需要时补证据边界和残余风险。只有用户要求完整方案、评审报告或模板，才展开 Loop Contract、RACI、验证矩阵、Goal Ledger 或内部实现层。证据边界必须区分事实、推断、待确认和范围外不做；授权策略必须区分只读、计划内低风险执行、受控执行授权和显式确认。
...
1. 识别目标和材料：确认目标、输入材料、写入边界、风险和验证要求。
2. 判断场景视图：在只读理解、产研交付、验证发布和知识回流中选最小可用视图。
3. 轻量问询 / 自我回答：复杂或模糊任务一次只问一个关键问题，并给建议答案；能从材料、源码或测试自答的先自答，再列必须问 owner 的问题和范围外不做。
4. 分派角色和能力来源：产品 / 交互回到产品专家，系分、TDD、编码、CR 和发布回到架构师，结构化代码生成回到代码生成器。
5. 推进阶段：`意图 -> 产品/交互 -> 设计评审 -> TDD -> 编码 -> CR -> 质量评估 -> 验证发布 -> 复盘回流`。
6. 执行 Ask-or-Decide：能用证据闭环的进入 Auto-Decide Next；缺 owner 信息的进入 Auto-Ask Owner；失败可修复的 Auto-Loop；超过边界的 Stop-Handoff。
7. 交接收口：输出交接物、验证证据、停止条件、残余风险和下一步 owner。
...
- `Wind 项目编码约规`：如果某个项目要采用，在该项目 `AGENTS.md` 标明“本项目遵守 Wind 项目编码约规”；之后用 `wind-project-coding-conventions` 做规则判断，编码、TDD、源码级 CR 和生产风险仍交给 `资深架构师`。
...
- 适合：项目 `AGENTS.md` 已 opt-in Wind 约规后，判断 face/impl、服务分层、模型归位、Entity 不外露、ServiceImpl、MyBatis Flex 和 TDD/CR 是否符合项目约规。
