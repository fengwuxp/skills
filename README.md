...
- `进入 GSD 产研协同研发流程`：以交付生产可用能力为目标，从产品分析、系分、编码、TDD、CR 到验证发布。

### 2. 再会选：按交付物找入口

- 产品语义 / PRD / Backlog / 验收：用 `产品架构专家`，先把用户、主体、目标、对象、流程、规则、验收和待确认项讲清楚。
- 产研协同 / GSD / CAD / Goal / Spec / Harness：用 `AI Native 研发流程编排`，它负责 owner、顺序、交接物、门禁、授权策略和停止条件，不替代正文产物或源码实现。
...
- 先定体，再调度用。目标和验收不清时，不急着进入 GSD、CAD、代码生成或工具安装。
...
- `进入 GSD 产研协同研发流程`：产品专家先做需求分析、产品设计、确认和验收种子，架构师再做系分设计、编码、TDD、CR 和验证，AI Native 只编排流程和门禁。
- `进入知识表达门禁`：把口头目标、文章观点、AI 草稿或模糊想法整理成目标、对象、规则、约束、验收样例、反馈源和缺口 owner；不满足时不进入 GSD/CAD/Loop 或编码。
- `进入非标问题模式`：老板、客户、运营或工程问题没有标准答案时，先定义真实问题、影响面、关键不确定性、候选方案、最小可逆实验、验证标准和停止条件。
- `GSD + Goal 按任务计划推进`：形成 Plan Grant，范围内低风险本地任务按计划推进；Goal 不替代 Execution Grant，Git、联网、生产、密钥、部署和不可逆操作仍需显式授权。
...
4. Goal / GSD / Spec / Harness：中大型项目先固定目标、Wave、预算 / 时间盒、状态、写入范围、验证命令和交接节奏。
...
- **从想法推进到可开发方案**：先由 `产品架构专家` 产出 Product Context Card，再由 `AI Native 研发流程编排` 判断是否进入 GSD/CAD、Spec、Harness 或 Goal，最后交给 `资深架构师` 做系分、TDD、CR 和验证。
- **大项目要进入 GSD + Goal**：用 `AI Native 研发流程编排` 固定真实交付目标、成功标准、Wave、Atomic Task、CAD 候选、Plan Grant、验证证据和停止条件。最佳实践是按阶段形成可验证切片，Git 提交仍需授权；避免让 AI 推进模拟模块、无业务入口 demo 或内存版业务 Service。
- **需要持续自动推进或 Loop**：先做 `生产可用 Loop 门禁`。最佳实践是检查状态载体、隔离执行、Maker / Checker、预算 / 时间盒、独立验证、人工接管、发布 / 回滚和残余风险；能自动跑不等于能上线。
...
优先用它的场景是跨角色、跨阶段、跨工具的流程问题：从 AI 原型到 PRD-Lite / OpenSpec / Harness / GSD / CAD，产品专家和架构师如何接力，`GSD + Goal` 如何保持目标、状态、预算 / 时间盒、验证证据和交接节奏一致，编码提速如何转成最终代码交付能力，质量 / 测试 / CR / 发布门禁如何闭环，Gemini CLI / AgentRC / Understand Anything 等工具是否允许安装或调用。只写一份 PRD、只做产品方案或 Backlog 决策时，直接用 `产品架构专家`；只做系统设计、代码 CR、Bug、测试或生产变更时，直接用 `资深架构师`。
