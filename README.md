...
- 跨角色产研交付 / 目标拆解 / 任务计划 / Spec / Harness / 质量发布闭环：用 `AI Native Engineering Loop`。它默认进入角色协作 Loop，按设计、设计评审、TDD、编码、编码评审、可用性/安全性评估、验证发布和复盘回流分角色推进；目标层、计划层、原子执行层和执行契约是内部层，不替代 PRD、系分、代码或发布审批。
...
- **产研交付视图**：完整交付视图，从产品事实、PRD/Spec、设计评审、TDD、编码、编码评审、可用性/安全性评估到生产发布和反馈回流，目标是交付真实、可验证、可接手的生产可用能力。
...
- `进入自主交付闭环`：让 AI 先自我挖掘材料、确认边界、规划低风险下一步，再明确哪些事项必须人工确认。
...
- 常用说法：`进入角色协作 Loop`、`进入只读理解 Loop`、`进入验证发布 Loop`、`做质量门禁`、`按目标和任务计划推进`、`评估 Gemini CLI / AgentRC / Understand Anything`。
...
- 从 AI 原型到工程化：`AI Native 研发流程编排` 先进入角色协作 Loop，定义设计、设计评审、TDD、编码、编码评审、可用性/安全性评估、验证发布、owner、门禁和验证矩阵；目标拆解、任务计划、原子执行和执行契约只作为内部层；`产品架构专家` 补产品上下文包、Backlog 和 PRD-Lite；`资深架构师` 补 OpenSpec、Harness、测试策略、TDD、源码级 CR 和发布风险。
...
`产品架构专家` 是产品语义和产品交付物的主入口，不负责工程实现、代码 CR、生产排障或工程执行授权。典型场景：想法 / 口头需求 / 原型候选、PRD / 产品方案、需求评审前置扫描、AI 生成方案 / 多方争议、客户访谈 / 工单 / 竞品 / 行业资料、支付与资金产品、产品图形化交付。
...
`AI Native Engineering Loop` 是编排入口，不是万能执行入口。唯一外显主流程是角色协作 Loop，再按需选择只读理解、产研交付、验证发布或知识回流视图；目标拆解、任务计划、Spec/Harness、质量门禁、代码库理解、工具准入和 AI 代码交付闭环都只是内部路由。
...
授权策略：只读 / 计划内低风险执行 / 分波执行授权 / 原子任务授权 / 显式确认；Codex 替我审批仅限当前授权范围内低风险动作
```
...
- 能用短句表达时，先让 `AI Native 研发流程编排` 判断最小流程；不要一开始就要求完整执行计划、完整 Harness 或大而全模板。
