...
- 产研协同 / GSD / CAD / Goal / Spec / Harness：用 `AI Native Engineering Loop`。它负责 Loop Profile、owner、顺序、交接物、门禁、授权策略和停止条件；GSD/CAD/Goal 是内部层，不替代 PRD、系分、代码或发布审批。
...
- **只读理解 Loop**：阅读分析代码库、设计-代码对齐、工具准入、事实边界检查和影响范围识别；默认不写文件、不联网、不安装。
- **产研交付 Loop**：从产品事实、PRD/Spec、系分、任务计划、TDD、实现、CR 到生产发布和反馈回流；旧说法 `进入 GSD 产研协同研发流程` 映射到这里。
- **验证发布 Loop**：测试矩阵、质量门禁、CR 前置条件、失败回退、发布监控、残余风险和复盘。
- **知识回流 Loop**：把已验证经验沉淀到 Skill、reference、fixture、脚本、用户指南或 source-map。

AI Native 的默认输出骨架固定为：结论、Loop Profile、Owner / 下一步分派、交接物、Loop Contract、证据边界、内部层、授权策略、验证门禁、停止条件、残余风险 / 需要确认。证据边界必须区分事实、推断、待确认和范围外不做；授权策略必须区分只读、计划内低风险执行、Plan Grant、Wave Grant、CAD Grant 和显式确认。
...
4. 工程执行：`资深架构师` 承接系统设计、编码、TDD、补测试、源码级 CR 和发布风险。
5. 质量理解门禁：AI Native 编排测试矩阵、理解门禁、工具准入、影响范围和残余风险交接。
6. 交付复盘：检查一次通过率、返工率、CR 轮次、缺陷密度、知识回流和残余风险。
...
- `进入 AI Native 交付 Loop`：以交付生产可用能力为目标，产品专家先做需求分析、产品设计、确认和验收种子，架构师再做系分、编码、TDD、CR 和验证。
- `进入 GSD 产研协同研发流程`：兼容旧说法，映射到 AI Native 交付 Loop；目标仍是交付生产可用能力。
- `GSD + Goal 按任务计划推进`：作为交付 Loop 的 Goal + GSD 内部层和 Goal 目标层，串起生产可用能力、成功标准、Wave、CAD 候选、验证证据和停止条件；形成 Plan Grant 后，低风险本地任务按计划推进；Goal 不等于 Execution Grant，不要把 Goal 当 Execution Grant，Git、联网、生产、密钥、部署和不可逆操作仍需显式授权。
- `意图到生产交付角色 Loop`：输出 Intent-to-Production Role Loop Map，区分阶段 owner、角色边界、交接物、验证门禁和停止条件，把意图、需求、产品、架构、AI Maker / Checker、质量门禁、发布和复盘串成一张 Role Loop Map。
- `落地 Spec 模板最佳实践`：进入 Spec/SDD 模板最佳实践和 Spec 模板模式，先区分 PRD / SDD / 实现 Spec 三层是否齐备，再输出 Spec 模板落地包；`进入 SDD 生产代码模式` 时检查事实来源、结构化契约和五支柱验证。
- `进入架构排熵 Loop`：当系统长期只加不减、承重行为、废弃 API 或治理规则失效增多时，输出 Architecture Entropy Card；架构排熵 Loop / 腐朽门禁只给可删除性和治理建议，能自动扫描不等于能自动删除。
- `用 Loop 取舍校准增强 AI Native Loop`：在准入、拆解、授权、执行核验或复盘需要取舍时使用；Loop 取舍校准 / Wisdom Lens 不要替代证据、测试、CR、授权或上线审批。
- `进入只读理解 Loop`：读代码库、对齐设计和代码、评估工具准入或做事实边界检查。
...
- `整理最终 PRD / 系分`：正式交付文档只保留当前有效结论、范围、规则、风险、待确认和验收；讨论过程、迭代草稿、AI 推理轨迹和被拒方案进入过程资产。
...
- 适合：AI 时代产品到研发编码流程、Agentic Engineering、AI 原型 / 评测到 PRD-Lite/OpenSpec/Harness、统一 Loop 准入、GSD/CAD/Goal 内部层、PRD/系分合议预审、AI 代码交付闭环、质量/测试门禁、代码库理解结论包、变更可理解性/影响可视化门禁、设计-代码对齐、代码库知识图谱准入，以及 Gemini CLI/AgentRC/Understand Anything 等代码理解工具安装与调用准入。
- 边界：可以作为 PRD、Backlog、架构设计、代码 CR、测试、生产变更或 Java Service 代码生成的流程入口，但只负责成熟度、owner、交接物、停止条件、授权策略和工具边界；具体产物继续分派给产品专家、架构师或代码生成器。
- 常用说法：`进入 AI Native 交付 Loop`、`进入只读理解 Loop`、`进入验证发布 Loop`、`进入知识回流 Loop`、`做 GSD/CAD 准入`、`开启 GSD/CAD 计划内授权策略`、`GSD + Goal`、`落地 Spec 模板最佳实践`、`做 AI 代码交付闭环`、`做质量门禁`、`做理解门禁`、`评估 Gemini CLI / AgentRC / Understand Anything`、`用 Loop 取舍校准增强 AI Native Loop`。
...
- 从 AI 原型到工程化：`AI Native 研发流程编排` 先定义阶段、门禁、owner、GSD/CAD 编排准入、Goal 组合、AI 代码交付闭环、质量/测试门禁和验证矩阵；`产品架构专家` 补产品上下文包、Backlog 和 PRD-Lite；`资深架构师` 补 OpenSpec、Harness、测试策略、TDD、源码级 CR 和发布风险。
...
`AI Native Engineering Loop` 是编排入口，不是万能执行入口。优先用它处理跨角色、跨阶段、跨工具的流程问题：从 AI 原型到工程化、GSD/CAD/Goal、Spec/Harness、质量门禁、代码库理解、工具准入、AI 代码交付闭环和发布复盘。
...
经流程入口分派时这样用：可以让它先判断普通 PRD、产品方案或 Backlog 是否成熟，再把正文产物分派给 `产品架构专家`；可以让它先判断架构设计、代码 CR、Bug 修复、测试或生产变更是否需要 OpenSpec、Harness、验证矩阵和发布闭环，再把工程执行分派给 `资深架构师`；可以让它先判断是否值得安装或调用 Gemini CLI、AgentRC、Understand Anything 这类工具来阅读代码、生成上下文、对齐设计和代码、检查上下文漂移或生成代码库知识图谱。
...
- 微信文章 [《深度思考：架构腐朽 & Loop Engineering》](https://mp.weixin.qq.com/s/wINKSDQCroWBvf29h567zA)：作为 `ai-native-engineering-workflow`、`资深架构师` 和 `产品架构专家` 的架构排熵、可删除性、概念生命周期和治理自检参考来源。2026-06-17 `web.open` 未取得正文，随后通过移动端微信 UA `curl` 公开 HTML 读取标题、作者/账号、meta 描述和正文；作者字段为 `lencx`，账号字段为 `浮之静`。本仓库只吸收持续排熵、可删除性、承重行为、概念膨胀、规则外置、Maker / Checker、Memory、守卫自检和人工 triage 等可迁移门禁，不复制原文、图片、比喻、作者表达或标题传播话术，也不把自动扫描写成可以自动删除、迁移、重写、合并、测试通过、CR 结论、执行授权或上线审批。
