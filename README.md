...
需要让所有仓库默认继承知止者的最小行动原则时，可在明确授权后把 [全局行动内核](./wise-agent/assets/codex-global-agents.md) 安装为 `$CODEX_HOME/AGENTS.md`（默认 `~/.codex/AGENTS.md`）。它只常驻事实优先、最小行动、验证闭环、授权边界和能力升级规则；不会强制每轮加载完整 `$wise-agent`。已有非空全局规则时先合并，不直接覆盖；安装或修改后开启新会话验证。
