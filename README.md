...
- `GSD + Goal 按任务计划推进`：形成 Plan Grant，任务计划内低风险本地读写、测试、lint、文档/Skill 更新和小步代码修改默认推进，不再逐任务索要 Execution Grant；GSD 规划会给出阶段/任务提交切片，实际本地 commit 只有在授权 Git 策略后执行；联网、依赖安装、生产、密钥、部署、不可逆操作和高风险业务仍显式确认。
- `进入 Agent Loop Engineering 模式`：用于 `/goal`、`/loop`、auto mode、后台 Agent 或多 Agent 监督等持续推进场景，先确认关联 Goal、状态载体、反馈源、验证者、预算 / 最大轮次、无进展检测、停止条件和交接物；Loop 只是运行循环契约，不是无条件授权。
- `进入 GSD + Goal + Loop 协同流程`：先定位 Round 0、GSD Candidate、Wave Plan、Plan Grant Active、Loop Candidate、CAD Candidate、CAD Loop Active、Verified、Paused、Escalated 或 Closed，再输出 Execution Handoff Card；Plan Grant 必须绑定 Loop 预算、最大轮次、无进展检测和失败回写，CAD 只消费已选定原子任务。
- `做 GSD/CAD 准入`：判断是否进入 Round 0、Wave/Atomic Task、Superpowers 方法门禁、CAD 候选缺口和授权策略缺口。
- `开启 GSD/CAD 默认授权策略`：把“每个任务都问授权”收敛为只读、默认低风险授权、Wave Grant、CAD Grant、阶段提交策略和显式确认；Codex “替我审批”只用于当前 Grant 范围内的低风险工具审批。
...
- 涉及安装、联网、写文件、覆盖已有文件、Git、同步到 Codex、生产数据或不可逆操作：先明确授权、写入范围、dry-run 和停止条件；GSD/CAD 默认授权和 Codex 替我审批只覆盖 Grant 范围内低风险本地动作。
...
- 中大型项目要持续推进：用 `GSD + Goal`，先固定父 Goal、Wave Goal、成功标准、预算 / 时间盒、停止条件、验证证据、授权策略、提交切片和 Ledger；Goal 不替代 Execution Grant。若还需要 `/goal`、`/loop`、auto mode 或后台 Agent 持续运行，再进入 `Agent Loop Engineering` 补状态、反馈、验证、预算和停止条件。
- 想让 AI 写代码：先确认 Spec / Harness / 写入范围 / 验证命令 / 授权策略；具体实现、TDD、补测试和源码级 CR 用 `资深架构师`。
...
- PRD、系分或 Spec 已经混入多轮讨论：用 `整理最终 PRD / 系分`，把正文收敛为最终交付标准版本，把过程内容迁移到评审报告、Decision Log、Goal Ledger、ADR 或任务计划。
...
4. Agent Loop：需要 `/goal`、`/loop`、auto mode 或后台 Agent 时，先用 `Agent Loop Engineering` 固定状态载体、反馈源、验证者、预算 / 最大轮次、无进展检测、停止条件、授权策略和交接物；GSD + Goal + Loop 还要输出协同状态机和 Execution Handoff Card。
5. Spec / Harness：需要 AI 编码时，补 OpenSpec、Spec 模板、Harness 摘要、写入范围、验证命令、授权策略和显式确认缺口。
6. 工程执行：`资深架构师` 承接系统设计、编码、TDD、补测试、CAD 门禁、源码级 CR 和发布风险。
7. 质量理解门禁：AI Native 编排质量门禁、理解门禁、工具准入和验证矩阵，确保 Review 者看得懂影响范围并能复核证据。
8. 交付复盘：用 `做 AI 代码交付闭环` 检查一次通过率、返工率、CR 轮次、缺陷密度、知识回流和残余风险。
...
它承接 AI Native 产品研发流程，但不直接替代 PRD 正文、系分正文、代码实现、测试通过、CR 结论或上线审批；核心职责是把目标、owner、交接物、授权策略、验证门禁和停止条件排成可执行链路。
...
- `开启 GSD/CAD 默认授权策略`：把自动推进拆成只读、默认低风险授权、Wave Grant、CAD Grant、阶段提交策略和显式确认；Codex “替我审批”只作为已启用的低风险审批通道。
- `落地 Spec 模板最佳实践`：先区分 PRD / SDD / 实现 Spec 三层是否齐备，再输出 Spec 强度、五段式骨架、AC 与测试映射、spec-lint、AC 覆盖、漂移检查、风险自查和轻重切换。
...
- **Goal 组合模式**：说 `GSD + Goal`、`CAD + Goal` 或目标驱动推进时，输出 Goal 卡、GSD Wave / Goal 映射、CAD 候选关联、Spec / AC 映射、Goal 状态、预算 / 时间盒、验证证据、提交切片、Plan Grant 判断、授权策略、停止条件、交接节奏和 Ledger 更新；Goal 不等于 Execution Grant，只有 Plan Grant 字段齐备时才推进范围内低风险任务。
- **工程编排模式**：已有产品上下文、OpenSpec 或明确变更目标时，设计 GSD/CAD 编排准入结论、Harness 摘要、GSD Wave 建议、CAD 候选缺口、授权策略、显式确认缺口和验证矩阵草案。
- **授权策略模式**：用户要求 GSD/CAD 默认授权、按任务计划推进、自动推进、阶段提交或 Codex “替我审批”时，输出只读 / 默认低风险授权 / Plan Grant / Wave Grant / CAD Grant / Git 策略 / 显式确认分级；Plan Grant 字段齐备后，范围内低风险本地任务直接推进，Git 提交只有授权后才执行，联网、生产等硬边界仍显式确认。
...
默认输出骨架固定为：结论、当前模式、Owner / 下一步分派、交接物、证据边界、授权策略、验证门禁、停止条件、残余风险 / 需要确认。证据边界必须区分事实、推断、待确认和范围外不做；授权策略必须区分默认、自动和需显式确认；只有用户要求完整方案、评审报告或模板时，再展开阶段表、RACI、验证矩阵或 Goal Ledger。
...
- 可以让它先做 `开启 GSD/CAD 默认授权策略`，减少每个任务重复审批；Plan Grant 覆盖任务计划内低风险本地动作，但 Git、联网、依赖安装、生产、密钥、部署、不可逆操作和高风险业务仍必须显式确认。
...
```text
GSD + Goal：把目标、生产可用能力、成功标准、预算 / 时间盒、GSD Wave、CAD 候选、验证证据、提交切片、停止条件和交接节奏串起来；不要把 Goal 当 Execution Grant。
...
```text
开启 GSD/CAD 默认授权策略：把每个任务都问授权收敛成只读、默认低风险授权、Wave Grant、CAD Grant 和显式确认；Codex 替我审批只用于当前 Grant 范围内低风险本地动作。
```
