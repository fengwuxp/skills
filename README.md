...
问决门禁是小闭环总门禁，`grill-me` 是命中升级条件后的升级盘问，也是唯一入口。复杂或模糊任务一次只问一个主 blocker；Facts 先从材料、源码、测试或日志自答，Decisions 才问 owner；需要问 owner 时给建议答案、依据、影响和默认暂停点。角色 Loop 自动触发 `grill-me` 只看四类信号：关键分叉未决、回答含糊 / 半答、连续返工、下一阶段会改变公共契约 / 状态机 / 验收样例 / 写入范围 / 发布风险。

`grill-me` 使用方式：

- 适合：目标含糊、关键分叉未决、回答半答、连续返工，或下一步会影响公共契约、状态机、验收样例、写入范围、发布风险。
- 你只需要回答：接受建议、改答案、补材料或停止。
- 结束条件：确认 shared understanding，并形成决策快照；未确认前不进入执行。
- 问询中出现红线、底线、不能碰、不可、禁止、必须等字眼，会被重点记录到决策快照或待确认项。
...
- `做 grill-me 盘问` / `grill-me 一下这个方案` / `grill 一下这个方案`：只盘一个决策分支；每轮只回答接受建议、改答案、补材料或停止；未确认 shared understanding 不进入执行。
...
维护者复装或更新外部 `grill-me` 后，运行 `VALIDATE_GRILL_ME_INSTALL=1 ./scripts/validate.sh` 或 `scripts/validate-grill-me-install.py`，确认本地只保留 `grill-me`，且没有 `/grilling` 转发；普通使用 `grill-me` 不需要运行这些命令。
