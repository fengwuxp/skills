...
当 PRD、可操作原型和业务标注需要在同一评审入口中对账时，可增加 `single-page-review-projection`。它是产品事实、交互载体和标注证据的投影，不是新的 PRD 事实源，也不要求所有原型都使用三栏布局。

- 投影至少提供三个可回读的区域：产品摘要（目标、范围、确认 / 待确认项）、交互载体（页面、状态和操作）以及规则 / 标注面板（`annotation_id`、需求 / AC、Owner、revision 和证据）。桌面可并列，窄屏可用 Tab、抽屉或分段流；布局变化不能改变事实和任务状态。
- 投影声明 `review_revision`，并同时记录 `product_revision / prototype_revision / annotation_revision`；三者不一致时显示 `stale / conflict / pending`，不以展示页面覆盖上游冲突。
- 产品摘要只引用 PRD 和产品事实，不复制完整 PRD；交互载体只承载可验证任务；规则面板只连接既有标注，不在 HTML 中自行发明业务规则。审阅开关必须保持体验模式的输入、路由和状态。
- 最低证据包括静态三端映射、关键浏览器任务和标注锚点回读。页面存在、静态 checker 通过或文章示例中的“通过”不能单独证明交互可用、业务正确、可访问性完整或生产就绪。
- 只在评审沟通、早期方案验证或跨角色对账确有收益时启用；简单页面、事实未定或只需局部说明时保留普通 HTML / Figma / 文档载体。
