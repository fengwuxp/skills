...
- Ant Design 采用 contract 曾完成临时复评，但仓库未登记可复核 artifact 且题集随后变化，因此不保留精确分数，只记录 release gate 未通过并继续保持 `cases-only`。重新激活前须按当前 source 复采、盲评并通过全部高风险门禁，不以刷新 hash 或降低回归阈值代替。
...
- 补充来源：`https://mp.weixin.qq.com/s?__biz=MzkwMDY4MTQwMw==&mid=2247487188&idx=1&sn=3e4ea848126adb145187aa1b32bb5ac0&chksm=c0411500f7369c1653fc276ac2d6b443bd617bf53a1232e0b545493aee4a0c06f61d4504a306#rd`，《产品构建师 | 设计与审美：视觉元素的意图审查》，作者“秋哥聊成长”，发布时间 2026-08-26 21:11；2026-08-27 通过本机 Chrome / Playwright 从专辑逐步加载并实际读取正文。
- 使用范围：只吸收“注意顺序与操作顺序分开，视觉权重回指当前首先需要判断的信息”，以及逐项检查颜色、线、图标、动效、留白和视觉权重是否承担真实任务、状态、关系或品牌职责；不复制原文、示例、固定字号或作者表达。
- 边界：不把 `0.5 秒`、字号差两级或位置天然优先等经验写成跨场景硬规则，也不把模糊 / 眯眼、灰度或倒置检查当作眼动、可用性或目标用户证据。

### AI UI 风格名词：共同语言与场景边界

- 来源：`https://mp.weixin.qq.com/s/tZ6oXI5xOsinbTV4vWMVQw`，《别再对 AI 说高级了：50 个 UI 风格名词，让 AI 秒懂你想要什么》，账号“星途AI视界”，页面显示发布时间为 2026-08-13 16:29。
...
- 将常见风格名正交拆成布局、页面目标、数据任务、视觉语言、交互方式和横切约束，再以名实、时位、文质、知止和行验校准；名词只作检索坐标，最终必须落到可观察变量和真实任务验证。
