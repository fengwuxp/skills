...
它不负责定义产品业务语义、规则和验收口径，也不替代前端工程实现、源码 CR 或生产发布。跨产品、设计和工程推进仍由当前 Agent 持有目标，默认入口与能力装载遵循 `AGENTS.md`；产品事实由 `product-architecture-expert` 稳定，界面设计由本 Skill 负责，代码实现与工程验证由 `senior-software-architect` 负责。
